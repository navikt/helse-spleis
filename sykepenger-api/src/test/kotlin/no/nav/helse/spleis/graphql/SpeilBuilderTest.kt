...
internal class SpeilBuilderTest : AbstractE2ETest() {
    ...
    fun `totalgrad må jo være lik for avslag og utbetaling`() {
        ...
        speilApi().assertTotalgrad(
            15,
            17.januar til 19.januar,
            22.januar til 26.januar,
            29.januar til 31.januar
        )
    ...
    fun `lager NyeInntektsforhold-pølse for tilkommen inntekt`() {
        ...
        val nyeInntektsforholdPølse =
            speilApi().arbeidsgivere.find { it.organisasjonsnummer == "a24" }?.nyeInntektsforhold!!.single()
        assertEquals(
            1.februar til 28.februar,
            nyeInntektsforholdPølse.fom til nyeInntektsforholdPølse.tom
        )
        ...
        assertEquals(
            ...
            speilApi().arbeidsgivere.find { it.organisasjonsnummer == "a24" }?.ghostPerioder
        )
    }
    ...
    fun `kan lage NyeInntektsforhold-pølse med peker til riktig vilkårsgrunnlag før noe er utbetalt`() {
        ...
        val nyeInntektsforholdPølser =
            speilApi().arbeidsgivere.find { it.organisasjonsnummer == "a24" }?.nyeInntektsforhold!!.single()
        assertEquals(
            1.februar til 28.februar,
            nyeInntektsforholdPølser.fom til nyeInntektsforholdPølser.tom
        )

        val førstegangs =
            speilApi().arbeidsgivere.find { it.organisasjonsnummer == "a1" }!!.generasjoner.single().perioder.find { it.fom == 1.januar } as BeregnetPeriode
    ...
    fun `nav utbetaler agp`() {
        ...
        val tidslinje =
            speilJson.arbeidsgivere.single().generasjoner.single().perioder.single().sammenslåttTidslinje
    ...
    fun `Viser inntektsgrunnlag for arbeidsforhold som startet innen 3 måneder før skjæringstidspunktet, selvom vi ikke har inntekt`() {
        ...
        assertEquals(
            listOf(a1, a2).map(String::toString),
            personDto.arbeidsgivere.map { it.organisasjonsnummer })

        val vilkårsgrunnlagId =
            (personDto.arbeidsgivere.first().generasjoner.first().perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
        val arbeidsgiverInntektA2 =
            personDto.vilkårsgrunnlag[vilkårsgrunnlagId]?.inntekter?.first { it.organisasjonsnummer == a2 }
    ...
    fun `beregnet periode peker på vilkårsgrunnlagid for infotrygdvilkårsgrunnlag`() {
        ...
        val infotrygdVilkårsgrunnlag =
            dto().vilkårsgrunnlagHistorikk.historikk.first().vilkårsgrunnlag.first { it.skjæringstidspunkt == 1.januar }
    ...
    fun `refusjon ligger på vilkårsgrunnlaget`() {
        ...
        val speilVilkårsgrunnlagId =
            (personDto.arbeidsgivere.first().generasjoner.first().perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
        val vilkårsgrunnlag =
            personDto.vilkårsgrunnlag[speilVilkårsgrunnlagId] as? SpleisVilkårsgrunnlag
    ...
    fun `refusjon ligger på vilkårsgrunnlaget - også for infotrygd`() {
        ...
        val speilVilkårsgrunnlagId =
            (personDto.arbeidsgivere.first().generasjoner.first().perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
        val vilkårsgrunnlag =
            personDto.vilkårsgrunnlag[speilVilkårsgrunnlagId] as? InfotrygdVilkårsgrunnlag
        assertTrue(vilkårsgrunnlag!!.arbeidsgiverrefusjoner.isNotEmpty())
        val arbeidsgiverrefusjon = vilkårsgrunnlag.arbeidsgiverrefusjoner.single()
        assertEquals(a1, arbeidsgiverrefusjon.arbeidsgiver)
        val refusjonsopplysning = arbeidsgiverrefusjon.refusjonsopplysninger.single()
    ...
    fun `korrigert inntektsmelding i Avsluttet, velger opprinnelig refusjon for eldste generasjon`() {
        ...
        generasjoner.last().also { eldsteGenerasjon ->
            ...
            val vilkårsgrunnlagId =
                (eldsteGenerasjon.perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
            val vilkårsgrunnlag = speil.vilkårsgrunnlag[vilkårsgrunnlagId] as? SpleisVilkårsgrunnlag
            assertEquals(
                INNTEKT,
                vilkårsgrunnlag!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.single().beløp.månedlig
            )
        }
        generasjoner.first().also { nyesteGenerasjon ->
            assertEquals(1, nyesteGenerasjon.perioder.size)
            val vilkårsgrunnlagId =
                (nyesteGenerasjon.perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
            val vilkårsgrunnlag = speil.vilkårsgrunnlag[vilkårsgrunnlagId] as? SpleisVilkårsgrunnlag
            assertEquals(
                20000.månedlig,
                vilkårsgrunnlag!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.single().beløp.månedlig
    ...
    fun `Endring til ingen refusjon i forlengelsen`() {
        ...
        val januarVilkårsgrunnlagId =
            (speilApi().arbeidsgivere.first().generasjoner.last().perioder.last() as BeregnetPeriode).vilkårsgrunnlagId
        val februarVilkårsgrunnlagId =
            (speilApi().arbeidsgivere.first().generasjoner.first().perioder.first() as BeregnetPeriode).vilkårsgrunnlagId
        val vilkårsgrunnlag = speilApi().vilkårsgrunnlag

        assertEquals(
            1,
            vilkårsgrunnlag[januarVilkårsgrunnlagId]!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.size
        )
        assertEquals(
            2,
            vilkårsgrunnlag[februarVilkårsgrunnlagId]!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.size
        )

        val førsteRefusjonsopplysning =
            vilkårsgrunnlag[februarVilkårsgrunnlagId]!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.first()
        val sisteRefusjonsopplysning =
            vilkårsgrunnlag[februarVilkårsgrunnlagId]!!.arbeidsgiverrefusjoner.single().refusjonsopplysninger.last()
    ...
    fun `hendelser på uberegnet periode`() {
        ...
        val periode =
            speilApi().arbeidsgivere.single().generasjoner.single().perioder.single() as UberegnetPeriode
    ...
    fun `legger ved skjønnsmessig fastsatt sykepengegrunnlag`() {
        ...
        håndterSkjønnsmessigFastsettelse(
            ...
            opplysninger = listOf(
                OverstyrtArbeidsgiveropplysning(
                    a1,
                    inntektSkjønnsfastsatt.månedlig
                )
            )
        )
        håndterYtelserTilGodkjenning()
        ...
        val personDto = speilApi()
        val vilkårsgrunnlagId =
            (personDto.arbeidsgivere.single().generasjoner.single().perioder.single() as BeregnetPeriode).vilkårsgrunnlagId
        val vilkårsgrunnlag =
            (personDto.vilkårsgrunnlag[vilkårsgrunnlagId] as SpleisVilkårsgrunnlag)
        ...
        assertEquals(
            ...
            vilkårsgrunnlag.inntekter.single().skjønnsmessigFastsatt!!.årlig
        )

        assertEquals(
            inntektSkjønnsfastsatt,
            vilkårsgrunnlag.inntekter.single().skjønnsmessigFastsatt!!.månedlig
        )

        assertEquals(
            Inntektkilde.Inntektsmelding.name,
            vilkårsgrunnlag.inntekter.single().omregnetÅrsinntekt.kilde.name
        )
    ...
    fun `Skatteinntekt vises som AO`() {
        ...
        håndterPåminnelse(
            ...
            tilstandsendringstidspunkt = LocalDateTime.now().minusMonths(4L)
        )
