            ...
            ApiMetrikker.målDeserialisering(meterRegistry) {
                ...
                .let {
                    ApiMetrikker.målByggSnapshot(meterRegistry) {
                        serializePersonForSpeil(it, spekemat)
                    }
                }
                .let { person ->
                    mapTilDto(person, fnr, aktørId, hendelseDao.hentHendelser(fnr.toLong()))
                }
        }
}
...
private fun mapTilDto(
    ...
    GraphQLPerson(
            ...
            person.arbeidsgivere.map { arbeidsgiver ->
                GraphQLArbeidsgiver(
                    ...
                    generasjoner =
                        arbeidsgiver.generasjoner.map { generasjon ->
                            GraphQLGenerasjon(
                                id = generasjon.id,
                                perioder =
                                    generasjon.perioder.map { periode ->
                                        mapTidslinjeperiode(periode, hendelser)
                                    },
                    ...
                    nyeInntektsforholdPerioder =
                        arbeidsgiver.nyeInntektsforhold.map { periode ->
                            GraphQLNyttInntektsforholdPeriode(
                                id = periode.id,
                                organisasjonsnummer = arbeidsgiver.organisasjonsnummer,
                                fom = periode.fom,
                                tom = periode.tom,
                                dagligBelop = periode.dagligBeløp,
                                manedligBelop = periode.månedligBeløp,
                                skjaeringstidspunkt = periode.skjæringstidspunkt,
                            )
                        },
                )
        ...
        vilkarsgrunnlag =
            person.vilkårsgrunnlag.map { (id, vilkårsgrunnlag) ->
                mapVilkårsgrunnlag(id, vilkårsgrunnlag)
            },
    )
