...
data class ArbeidsgiverDTO(
    ...
    private companion object {
        fun List<ArbeidsgiverDTO>.sykefraværstilfeller() =
            this.flatMap { arbeidsgiver ->
    ...
    internal fun medGhostperioderOgNyeInntektsforholdperioder(
        ...
        val potensielleGhostperioder =
            vilkårsgrunnlagHistorikk.potensielleGhostsperioder(
                organisasjonsnummer,
        ...
        val ghostsperioder =
            fjernHelgepølser(
                potensielleGhostperioder.flatMap { ghostperiode ->
    ...
    private fun <Ting> fjernDagerMedSykdom(
        ...
        val tidslinjeperioderFraNyesteGenerasjon = generasjoner.first().perioder.sortedBy { it.fom }
        val oppslittetPølser =
            tidslinjeperioderFraNyesteGenerasjon.fold(listOf(ghostperiode)) {
                resultat,
                vedtaksperiode ->
                ...
                tidligereGhostperioder +
                    (sisteGhostperiode?.oppbrytningsfunksjon(tidslinjeperiode) ?: emptyList())
            }
        return oppslittetPølser
    }

    private fun fjernHelgepølser(ghostPerioder: List<GhostPeriodeDTO>) =
        ghostPerioder.filterNot { ghostperiode ->
