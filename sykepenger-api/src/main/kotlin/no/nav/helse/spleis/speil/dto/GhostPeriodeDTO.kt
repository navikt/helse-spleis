...
data class GhostPeriodeDTO(
    ...
    internal fun brytOpp(tidslinjeperiode: ClosedRange<LocalDate>) =
        brytOpp(this, fom, tom, tidslinjeperiode, GhostPeriodeDTO::til, GhostPeriodeDTO::fra)

    internal fun til(other: ClosedRange<LocalDate>) =
        copy(id = UUID.randomUUID(), tom = other.start.forrigeDag)

    internal fun fra(other: ClosedRange<LocalDate>) =
        copy(id = UUID.randomUUID(), fom = other.endInclusive.nesteDag)
}

internal fun <Ting> brytOpp(
    ting: Ting,
    fom: LocalDate,
    tom: LocalDate,
    tidslinjeperiode: ClosedRange<LocalDate>,
    tilFunksjon: Ting.(ClosedRange<LocalDate>) -> Ting,
    fraFunksjon: Ting.(ClosedRange<LocalDate>) -> Ting,
): List<Ting> =
    when {
        // trimmer ingenting
        tidslinjeperiode.erUtenfor(fom, tom) -> listOf(ting)
        // trimmer i midten
        tidslinjeperiode.erInni(fom, tom) ->
            listOf(ting.tilFunksjon(tidslinjeperiode), ting.fraFunksjon(tidslinjeperiode))
        // trimmer i slutten
