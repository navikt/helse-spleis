                    ...
                    val hendelse =
                        hendelseDao.hentHendelse(meldingsReferanse)
                            ?: throw NotFoundException(
                                "Kunne ikke finne hendelse for hendelsereferanse = $hendelseId"
                            )
    ...
    routing {
        authenticate {
            get("/api/vedtaksperioder") {
                withContext(Dispatchers.IO) {
                    val fnr =
                        call.request.header("fnr")?.toLong()
...
private data class PersonRequest(val fødselsnummer: String)
