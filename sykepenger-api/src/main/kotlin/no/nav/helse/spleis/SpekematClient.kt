...
class SpekematClient(
    ...
    fun hentSpekemat(fnr: String, callId: String): SpekematDTO {
        ...
        if (statusCode != 200)
            throw SpekematClientException(
                "Fikk uventet http statuskode fra spekemat: $statusCode. Forventet HTTP 200 OK"
            )
    ...
    private fun lagRequest(fnr: String, callId: String): HttpRequest {
        @Language("JSON") val requestBody = """{ "fnr": "$fnr" }"""
...
private data class PølseradDto(val pølser: List<PølseDto>, val kildeTilRad: UUID)
...
private enum class Pølsestatus {
    ÅPEN,
    LUKKET,
    FORKASTET,
}
