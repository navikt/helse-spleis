...
internal class SykdomstidslinjeBuilder(private val dto: SykdomstidslinjeDto, periode: PeriodeDto) {
    private val tidslinje by lazy {
        (periode.fom.datesUntil(periode.tom.plusDays(1)).map { dag ->
            ...
            dto.dager.firstOrNull { it.dato == dag }?.let { tilDagDto(it) }
                ?: Sykdomstidslinjedag(
                    dagen = dag,
...
internal class UtbetalingstidslinjeBuilder(private val dto: UtbetalingstidslinjeUtDto) {
        ...
        dto.dager.map {
            when (it) {
                ...
                is UtbetalingsdagUtDto.FridagDto ->
                    UtbetalingstidslinjedagUtenGrad(
                        type =
                            if (it.dato.erHelg()) UtbetalingstidslinjedagType.Helgedag
                            else UtbetalingstidslinjedagType.Feriedag,
                        dato = it.dato,
                    )
                is UtbetalingsdagUtDto.NavDagDto ->
                    mapUtbetalingsdag(it.dato, UtbetalingstidslinjedagType.NavDag, it.økonomi)
    ...
    private fun mapUtbetalingsdag(
        dato: LocalDate,
        type: UtbetalingstidslinjedagType,
