...
private fun mapDag(dag: SammenslåttDag) =
    GraphQLDag(
        dato = dag.dagen,
        sykdomsdagtype =
            when (dag.sykdomstidslinjedagtype) {
                ...
                SykdomstidslinjedagType.ARBEID_IKKE_GJENOPPTATT_DAG ->
                    GraphQLSykdomsdagtype.ArbeidIkkeGjenopptattDag
                ...
                SykdomstidslinjedagType.ANDRE_YTELSER_FORELDREPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserForeldrepenger
                SykdomstidslinjedagType.ANDRE_YTELSER_AAP -> GraphQLSykdomsdagtype.AndreYtelserAap
                SykdomstidslinjedagType.ANDRE_YTELSER_OMSORGSPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserOmsorgspenger
                SykdomstidslinjedagType.ANDRE_YTELSER_PLEIEPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserPleiepenger
                SykdomstidslinjedagType.ANDRE_YTELSER_SVANGERSKAPSPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserSvangerskapspenger
                SykdomstidslinjedagType.ANDRE_YTELSER_OPPLÆRINGSPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserOpplaringspenger
                SykdomstidslinjedagType.ANDRE_YTELSER_DAGPENGER ->
                    GraphQLSykdomsdagtype.AndreYtelserDagpenger
                SykdomstidslinjedagType.UBESTEMTDAG -> GraphQLSykdomsdagtype.Ubestemtdag
            },
        utbetalingsdagtype =
            when (dag.utbetalingstidslinjedagtype) {
                UtbetalingstidslinjedagType.ArbeidsgiverperiodeDag ->
                    GraphQLUtbetalingsdagType.ArbeidsgiverperiodeDag
        ...
        kilde =
            GraphQLSykdomsdagkilde(
                id = dag.kilde.id,
                type =
                    when (dag.kilde.type) {
                        SykdomstidslinjedagKildetype.Inntektsmelding ->
                            GraphQLSykdomsdagkildetype.Inntektsmelding
                        SykdomstidslinjedagKildetype.Søknad -> GraphQLSykdomsdagkildetype.Soknad
                        SykdomstidslinjedagKildetype.Sykmelding ->
                            GraphQLSykdomsdagkildetype.Sykmelding
                        SykdomstidslinjedagKildetype.Saksbehandler ->
                            GraphQLSykdomsdagkildetype.Saksbehandler
                        SykdomstidslinjedagKildetype.Ukjent -> GraphQLSykdomsdagkildetype.Ukjent
                    },
            ),
        grad = dag.grad?.toDouble(),
        utbetalingsinfo =
            dag.utbetalingsinfo?.let {
                GraphQLUtbetalingsinfo(
                    inntekt = null, // deprecated: speil bruker ikke denne verdien
                    utbetaling =
                        it
                            .arbeidsgiverbeløp, // deprecated: verdien settes til det samme som
                                                // arbeidsgiverbeløp
                    ...
                    totalGrad =
                        it.totalGrad
                            .toDouble(), // double er deprecated: speil forventer float, med runder
                                        // ned før visning
                )
            },
        begrunnelser =
            dag.begrunnelser?.map {
                when (it) {
                    BegrunnelseDTO.SykepengedagerOppbrukt ->
                        GraphQLBegrunnelse.SykepengedagerOppbrukt
                    BegrunnelseDTO.SykepengedagerOppbruktOver67 ->
                        GraphQLBegrunnelse.SykepengedagerOppbruktOver67
                    ...
                    BegrunnelseDTO.EgenmeldingUtenforArbeidsgiverperiode ->
                        GraphQLBegrunnelse.EgenmeldingUtenforArbeidsgiverperiode
...
private fun mapOppdrag(oppdrag: SpeilOppdrag): GraphQLOppdrag =
    GraphQLOppdrag(
        ...
        simulering =
            oppdrag.simulering?.let { simulering ->
                GraphQLSimulering(
...
private fun mapUtbetaling(utbetaling: Utbetaling) =
    GraphQLUtbetaling(
        id = utbetaling.id,
        ...
        statusEnum =
            when (utbetaling.status) {
                ...
                Utbetalingstatus.GodkjentUtenUtbetaling ->
                    GraphQLUtbetalingstatus.GodkjentUtenUtbetaling
        ...
        arbeidsgiveroppdrag =
            utbetaling.oppdrag[utbetaling.arbeidsgiverFagsystemId]?.let { mapOppdrag(it) },
        personoppdrag = utbetaling.oppdrag[utbetaling.personFagsystemId]?.let { mapOppdrag(it) },
    )

private fun mapHendelse(hendelse: HendelseDTO) =
    when (hendelse.type) {
        HendelsetypeDto.NY_SØKNAD ->
            GraphQLSykmelding(
                id = hendelse.id,
        ...
        HendelsetypeDto.SENDT_SØKNAD_NAV ->
            GraphQLSoknadNav(
                id = hendelse.id,
                eksternDokumentId = hendelse.eksternDokumentId,
                fom = hendelse.fom!!,
        ...
        HendelsetypeDto.SENDT_SØKNAD_FRILANS ->
        ...
        HendelsetypeDto.SENDT_SØKNAD_SELVSTENDIG ->
        ...
        HendelsetypeDto.SENDT_SØKNAD_ARBEIDSLEDIG ->
        ...
        HendelsetypeDto.SENDT_SØKNAD_ARBEIDSGIVER ->
        ...
        HendelsetypeDto.INNTEKTSMELDING ->
        ...
        HendelsetypeDto.INNTEKT_FRA_AORDNINGEN ->
...
private fun mapPeriodevilkår(vilkår: BeregnetPeriode.Vilkår) =
    GraphQLPeriodevilkar(
        sykepengedager =
            vilkår.sykepengedager.let {
...
private fun mapPeriodetype(type: Tidslinjeperiodetype) =
    when (type) {
...
private fun mapInntektstype(kilde: UtbetalingInntektskilde) =
    when (kilde) {
    ...
    GraphQLBeregnetPeriode(
        ...
        periodevilkar =
            GraphQLPeriodevilkar(
                ...
                alder = GraphQLPeriodevilkar.Alder(alderSisteSykedag = 0, oppfylt = false),
            ),
        periodetilstand = mapTilstand(periode.periodetilstand),
...
private fun Set<UUID>.tilHendelseDTO(hendelser: List<HendelseDTO>): List<GraphQLHendelse> {
    return this.mapNotNull { dokumentId ->
            hendelser.firstOrNull { hendelseDTO -> hendelseDTO.id == dokumentId.toString() }
        }
        .mapNotNull { mapHendelse(it) }
}

private fun mapTilstand(tilstand: Periodetilstand) =
    when (tilstand) {
        ...
        Periodetilstand.UtbetaltVenterPåAnnenPeriode ->
            GraphQLPeriodetilstand.UtbetaltVenterPaAnnenPeriode
        Periodetilstand.AvventerInntektsopplysninger ->
            GraphQLPeriodetilstand.AvventerInntektsopplysninger
    }

private fun mapArbeidsgiverRefusjon(arbeidsgiverrefusjon: Arbeidsgiverrefusjon) =
    GraphQLArbeidsgiverrefusjon(
        arbeidsgiver = arbeidsgiverrefusjon.arbeidsgiver,
        refusjonsopplysninger =
            arbeidsgiverrefusjon.refusjonsopplysninger.map {
...
private fun mapInntekt(inntekt: Arbeidsgiverinntekt) =
    GraphQLArbeidsgiverinntekt(
        ...
        skjonnsmessigFastsatt =
            inntekt.skjønnsmessigFastsatt?.let {
                GraphQLSkjonnsmessigFastsatt(belop = it.årlig, manedsbelop = it.månedlig)
            },
...
private fun Inntekt.tilGraphQLOmregnetArsinntekt() =
    GraphQLOmregnetArsinntekt(
        ...
        inntekterFraAOrdningen =
            this.inntekterFraAOrdningen?.map {
                GraphQLInntekterFraAOrdningen(maned = it.måned, sum = it.sum)
            },
    )
        ...
        is SpleisVilkårsgrunnlag ->
            GraphQLSpleisVilkarsgrunnlag(
                ...
                sykepengegrunnlagsgrense =
                    mapSykepengergrunnlagsgrense(vilkårsgrunnlag.sykepengegrunnlagsgrense),
                ...
                arbeidsgiverrefusjoner =
                    vilkårsgrunnlag.arbeidsgiverrefusjoner.map { refusjon ->
                        mapArbeidsgiverRefusjon(refusjon)
                    },
            )
        is InfotrygdVilkårsgrunnlag ->
            GraphQLInfotrygdVilkarsgrunnlag(
                id = id,
                skjaeringstidspunkt = vilkårsgrunnlag.skjæringstidspunkt,
                omregnetArsinntekt =
                    vilkårsgrunnlag
                        .beregningsgrunnlag, // For infotrygd har vi ikke noe konsept for hvorvidt
                                             // en inntekt er skjønnsfastsatt
                sykepengegrunnlag = vilkårsgrunnlag.sykepengegrunnlag,
                inntekter = vilkårsgrunnlag.inntekter.map { inntekt -> mapInntekt(inntekt) },
                arbeidsgiverrefusjoner =
                    vilkårsgrunnlag.arbeidsgiverrefusjoner.map { refusjon ->
            ...
            throw IllegalStateException(
                "har ikke mapping for vilkårsgrunnlag ${vilkårsgrunnlag::class.simpleName ?: "[ukjent klassenavn]"}"
            )
    }
