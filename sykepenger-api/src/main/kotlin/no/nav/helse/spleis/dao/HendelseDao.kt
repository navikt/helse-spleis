...
internal class HendelseDao(
    ...
    fun hentHendelse(meldingsReferanse: UUID): String? {
        return sessionOf(dataSource())
            ...
            .also { PostgresProbe.hendelseLestFraDb(meterRegistry) }
    }
    ...
    fun hentHendelser(fødselsnummer: Long): List<HendelseDTO> {
        ...
        return sessionOf(dataSource())
                    ...
                    when (type) {
                        Meldingstype.NY_SØKNAD ->
                            HendelseDTO.nySøknad(
                                ...
                                rapportertdato =
                                    LocalDateTime.parse(node.path("@opprettet").asText()),
                            )
                        Meldingstype.NY_SØKNAD_FRILANS ->
                            HendelseDTO.nyFrilanssøknad(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.NY_SØKNAD_SELVSTENDIG ->
                            HendelseDTO.nySelvstendigsøknad(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.NY_SØKNAD_ARBEIDSLEDIG ->
                            HendelseDTO.nyArbeidsledigsøknad(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.SENDT_SØKNAD_FRILANS ->
                            HendelseDTO.sendtSøknadFrilans(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.SENDT_SØKNAD_SELVSTENDIG ->
                            HendelseDTO.sendtSøknadSelvstendig(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.SENDT_SØKNAD_ARBEIDSLEDIG ->
                            HendelseDTO.sendtSøknadArbeidsledig(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                        ...
                        Meldingstype.SENDT_SØKNAD_ARBEIDSGIVER ->
                            HendelseDTO.sendtSøknadArbeidsgiver(
                                id = node.path("@id").asText(),
                                eksternDokumentId = node.path("id").asText(),
                                fom = LocalDate.parse(node.path("fom").asText()),
                                tom = LocalDate.parse(node.path("tom").asText()),
                                rapportertdato =
                                    LocalDateTime.parse(node.path("@opprettet").asText()),
                                sendtArbeidsgiver =
                                    LocalDateTime.parse(node.path("sendtArbeidsgiver").asText()),
                            )
                        Meldingstype.INNTEKTSMELDING ->
                        ...
                        Meldingstype.SYKEPENGEGRUNNLAG_FOR_ARBEIDSGIVER ->
                            HendelseDTO.inntektFraAOrdningen(
                                id = node.path("@id").asText(),
                                mottattDato = LocalDateTime.parse(node.path("@opprettet").asText()),
                            )
                    }
            ...
            .onEach { PostgresProbe.hendelseLestFraDb(meterRegistry) }
    }
