...
internal object Api {
    ...
    private val String.fnr
        get() = objectMapper.readTree(this.replace(" ", "").replace("\n", "")).let { body ->
            val fraVariables = body.path("variables").fields().asSequence()
                .singleOrNull { (_, value) -> value.asText().matches(sifferRegex) }?.value?.asText()
            fraVariables ?: fraQueryRegex.find(
                body.path("query").asText()
            ...
            authenticate(optional = true) {
                post("/graphql") {
                    ...
                    try {
                        ...
                        val (_, aktørId) = speedClient.hentFødselsnummerOgAktørId(ident, callId)
                            .getOrThrow()

                        val person = personResolver(
                            spekematClient,
                            personDao,
                            hendelseDao,
                            ident,
