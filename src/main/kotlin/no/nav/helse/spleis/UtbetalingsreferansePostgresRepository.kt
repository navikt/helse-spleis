...
class UtbetalingsreferansePostgresRepository(private val dataSource: DataSource,
    ...
    override fun hentUtbetaling(utbetalingsreferanse: String): Utbetalingsreferanse? {
        return using(sessionOf(dataSource)) { session ->
            session.run(queryOf("SELECT id, aktor_id, orgnr, sakskompleksId FROM utbetalingsreferanse WHERE id = ? LIMIT 1", utbetalingsreferanse).map {
                Utbetalingsreferanse(
                        id = it.string("id"),
                        aktørId = it.string("aktor_id"),
                        orgnummer = it.string("orgnr"),
                        sakskompleksId = UUID.fromString(it.string("sakskompleksId"))
                )
            }.asSingle)
        }?.also {
