...
class Tidslinjedag(
    ...
    companion object {
        fun List<Tidslinjedag>.dager(
            periode: ClosedRange<LocalDate>? = null
        ): List<Map<String, Any?>> {
            return this.filter {
                    it.dato >= (periode?.start ?: LocalDate.MIN) &&
                        it.dato <= (periode?.endInclusive ?: LocalDate.MAX)
                }
                .sortedBy { it.dato }
        ...
        private fun List<Tidslinjeperiode>.dager() = map {
            mapOf("fom" to it.fom, "tom" to it.tom, "dagtype" to it.dagtype, "grad" to it.grad)
        }
    }
