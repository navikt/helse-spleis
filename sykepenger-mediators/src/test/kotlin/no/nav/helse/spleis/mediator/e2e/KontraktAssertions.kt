...
internal object KontraktAssertions {
    ...
    private fun Pair<ObjectNode, ObjectNode>.assertOgFjernUUIDTemplates() =
        assertOgFjernTemplates("<uuid>") { faktiskJson, key -> faktiskJson.assertOgFjernUUID(key) }
    ...
    internal fun ObjectNode.assertOgFjern(key: String, validation: (value: JsonNode) -> Unit) {
        ...
        val objekt =
            key.substringBeforeLast(".").split(".").fold(this as JsonNode) { result, nestedKey ->
                result.path(nestedKey)
            } as ObjectNode
