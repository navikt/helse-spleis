...
internal class KunEnArbeidsgiverMediatorTest : AbstractEndToEndMediatorTest() {
    ...
    fun `perioder påvirket av annullering-event blir forkastet men forblir i Avsluttet`() {
        ...
        sendUtbetaling()
        val utbetalingId =
            testRapid.inspektør.alleEtterspurteBehov(Utbetaling).last().path("utbetalingId")
                .asText()
    ...
    fun `Behandler ikke melding hvis den allerede er behandlet`() {
        ...
        val (meldingId, message) = meldingsfabrikk.lagNySøknad(
            SoknadsperiodeDTO(
                fom = 1.januar,
                tom = 25.januar,
                sykmeldingsgrad = 100
    ...
    fun `Behandler melding hvis den tidligere har prøvd å behandle melding, men kræsjet`() {
        ...
        val (_, message) = meldingsfabrikk.lagNySøknad(
            SoknadsperiodeDTO(
                fom = 1.januar,
                tom = 25.januar,
                sykmeldingsgrad = 100
