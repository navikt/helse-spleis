...
internal class SendtNavSøknaderRiverTest : RiverTest() {
    ...
    private fun validSøknad(
        ...
        fravar: List<FravarDTO> =
            listOf(
                FravarDTO(fom = LocalDate.now(), tom = LocalDate.now(), type = FravarstypeDTO.FERIE)
            ),
    ) =
        SykepengesoknadDTO(
    ...
    private val validSendtSøknadWithUnknownFieldsJson =
        validSøknad().asObjectNode().put(UUID.randomUUID().toString(), "foobar").toJson()
    private val ukjentFraværskode =
        validSøknad()
            .asObjectNode()
            .also { (it.path("fravar").first() as ObjectNode).put("type", "INVALID_FRAVÆRSTYPE") }
            .toJson()
    ...
    fun `parser søknad med egenmeldingsdager`() {
        ...
        assertNoErrors(
            validSøknad().copy(egenmeldingsdagerFraSykmelding = listOf(1.januar, 2.januar)).toJson()
        )
    ...
    fun `parser søknad med merknader (fra sykmelding)`() {
        ...
        assertNoErrors(
            validSøknad()
                .copy(merknaderFraSykmelding = listOf(MerknadDTO("UGYLDIG_TILBAKEDATERING", null)))
                .toJson()
        )
        assertNoErrors(
            validSøknad()
