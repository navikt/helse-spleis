...
internal abstract class AbstractEndToEndMediatorTest {
    ...
    protected fun antallPersonalias(fnr: String? = null) =
        sessionOf(dataSource.ds).use {
            it.run(
                queryOf("SELECT COUNT(1) FROM person_alias ${fnr?.let { "WHERE fnr=${fnr.toLong()}" } ?: "" }").map { it.long(1) }.asSingle,
            )
                ?: 0
        }
                ...
                Forespørsel(
                    ...
                    førsteFraværsdager =
                        node.path("førsteFraværsdager").map {
                            FørsteFraværsdag(it.path("organisasjonsnummer").textValue(), it.path("førsteFraværsdag").asLocalDate())
                        },
                    sykmeldingsperioder =
