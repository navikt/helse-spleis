...
internal class AvstemmingMediatorTest : AbstractEndToEndMediatorTest() {
    ...
    fun avstemming() {
        ...
        assertDoesNotThrow {
            LocalDateTime.parse(
                vedtaksperioder.path(0).path("opprettet").asText()
            )
        }
        assertDoesNotThrow {
            LocalDateTime.parse(
                vedtaksperioder.path(0).path("oppdatert").asText()
            )
        }
        ...
        assertDoesNotThrow {
            LocalDateTime.parse(
                vedtaksperioder.path(1).path("opprettet").asText()
            )
        }
        assertDoesNotThrow {
            LocalDateTime.parse(
                vedtaksperioder.path(1).path("oppdatert").asText()
            )
        }
        ...
        assertDoesNotThrow {
            UUID.fromString(
                forkastedeVedtaksperioder.path(0).path("id").asText()
            )
        }
        assertEquals("TIL_INFOTRYGD", forkastedeVedtaksperioder.path(0).path("tilstand").asText())
        assertDoesNotThrow {
            LocalDateTime.parse(
                forkastedeVedtaksperioder.path(0).path("opprettet").asText()
            )
        }
        assertDoesNotThrow {
            LocalDateTime.parse(
                forkastedeVedtaksperioder.path(0).path("oppdatert").asText()
            )
        }

        assertDoesNotThrow {
            UUID.fromString(
                forkastedeVedtaksperioder.path(1).path("id").asText()
            )
        }
        assertEquals("TIL_INFOTRYGD", forkastedeVedtaksperioder.path(1).path("tilstand").asText())
        assertDoesNotThrow {
            LocalDateTime.parse(
                forkastedeVedtaksperioder.path(1).path("opprettet").asText()
            )
        }
        assertDoesNotThrow {
            LocalDateTime.parse(
                forkastedeVedtaksperioder.path(1).path("oppdatert").asText()
            )
        }
        ...
        assertDoesNotThrow {
            LocalDateTime.parse(
                utbetalinger.path(0).path("vurdering").path("tidspunkt").asText()
            )
        }
