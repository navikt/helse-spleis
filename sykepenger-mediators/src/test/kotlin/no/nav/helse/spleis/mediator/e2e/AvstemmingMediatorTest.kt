...
internal class AvstemmingMediatorTest : AbstractEndToEndMediatorTest() {
    ...
    fun avstemming() {
        ...
        sendSøknad(
            perioder =
                listOf(SoknadsperiodeDTO(fom = 1.februar, tom = 27.februar, sykmeldingsgrad = 100))
        )
        sendSøknad(
            perioder =
        ...
        sendSøknad(
            perioder =
                listOf(SoknadsperiodeDTO(fom = 1.april, tom = 30.april, sykmeldingsgrad = 100))
        )
        sendInntektsmelding(
            listOf(Periode(fom = 1.april, tom = 16.april)),
            førsteFraværsdag = 1.april,
        )
        ...
        val forkastedeVedtaksperioder =
            avstemt.path("arbeidsgivere").path(0).path("forkastedeVedtaksperioder")
        ...
        assertEquals(
            ORGNUMMER,
            avstemt.path("arbeidsgivere").path(0).path("organisasjonsnummer").asText(),
        )
        ...
        assertDoesNotThrow {
            LocalDateTime.parse(vedtaksperioder.path(0).path("opprettet").asText())
        }
        assertDoesNotThrow {
            LocalDateTime.parse(vedtaksperioder.path(0).path("oppdatert").asText())
        }
        ...
        assertDoesNotThrow {
            LocalDateTime.parse(vedtaksperioder.path(1).path("opprettet").asText())
        }
        assertDoesNotThrow {
            LocalDateTime.parse(vedtaksperioder.path(1).path("oppdatert").asText())
        }

        assertEquals(2, forkastedeVedtaksperioder.size())
        assertDoesNotThrow {
            UUID.fromString(forkastedeVedtaksperioder.path(0).path("id").asText())
        }
        assertEquals("TIL_INFOTRYGD", forkastedeVedtaksperioder.path(0).path("tilstand").asText())
        assertDoesNotThrow {
            LocalDateTime.parse(forkastedeVedtaksperioder.path(0).path("opprettet").asText())
        }
        assertDoesNotThrow {
            LocalDateTime.parse(forkastedeVedtaksperioder.path(0).path("oppdatert").asText())
        }

        assertDoesNotThrow {
            UUID.fromString(forkastedeVedtaksperioder.path(1).path("id").asText())
        }
        assertEquals("TIL_INFOTRYGD", forkastedeVedtaksperioder.path(1).path("tilstand").asText())
        assertDoesNotThrow {
            LocalDateTime.parse(forkastedeVedtaksperioder.path(1).path("opprettet").asText())
        }
        assertDoesNotThrow {
            LocalDateTime.parse(forkastedeVedtaksperioder.path(1).path("oppdatert").asText())
        }
        ...
        assertEquals(
            false,
            utbetalinger.path(0).path("vurdering").path("automatiskBehandling").asBoolean(),
        )
        assertEquals(true, utbetalinger.path(0).path("vurdering").path("godkjent").asBoolean())
        assertDoesNotThrow {
            LocalDateTime.parse(utbetalinger.path(0).path("vurdering").path("tidspunkt").asText())
        }
