...
internal class TestRapid : RapidsConnection() {
    ...
    internal val inspektør
        get() = RapidInspektør(messages.toList())
    ...
    class RapidInspektør(private val messages: List<Pair<String?, String>>) {
        ...
        private val tilstander
            get() =
                mutableMapOf<UUID, MutableList<String>>().apply {
                    events("vedtaksperiode_endret") {
                        val id = UUID.fromString(it.path("vedtaksperiodeId").asText())
                        this.getOrPut(id) { mutableListOf() }
                            .add(it.path("gjeldendeTilstand").asText())
                    }
                }
        ...
        private val behov
            get() =
                mutableMapOf<UUID, MutableList<Pair<Aktivitet.Behov.Behovtype, TilstandType>>>()
                    .apply {
                        events("behov") {
                            val vedtaksperiodeIdString =
                                it.path("vedtaksperiodeId")
                            ...
                            this.getOrPut(id) { mutableListOf() }
                                .apply {
                                    it.path("@behov").onEach {
                                        add(
                                            Aktivitet.Behov.Behovtype.valueOf(it.asText()) to
                                                tilstand
                                        )
                                    }
                                }
        ...
        val vedtaksperiodeteller
            get() = vedtaksperiodeIder.size
        ...
        fun tilstandForEtterspurteBehov(
            ...
            behov
                .getValue(vedtaksperiodeId(vedtaksperiodeIndeks))
                .last { it.first == behovtype }
                .second
