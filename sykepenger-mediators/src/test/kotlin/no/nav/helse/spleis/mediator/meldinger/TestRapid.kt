...
internal class TestRapid : RapidsConnection() {
    ...
    class RapidInspektør(
        ...
        private val behov
            get() =
                mutableMapOf<UUID, MutableList<Pair<Aktivitet.Behov.Behovtype, TilstandType>>>().apply {
                    events("behov") {
                        val vedtaksperiodeIdString =
                            it
        ...
        private val behovmeldinger
            get() =
                mutableListOf<Pair<Aktivitet.Behov.Behovtype, JsonNode>>().apply {
        ...
        internal fun behovtypeSisteMelding(behovtype: Aktivitet.Behov.Behovtype) = melding(antall() - 1)["@behov"][0].asText() == behovtype.toString()
        ...
        fun etterspurteBehov(behovtype: Aktivitet.Behov.Behovtype) = behovmeldinger.last { it.first == behovtype }.second

        fun alleEtterspurteBehov(behovtype: Aktivitet.Behov.Behovtype) = behovmeldinger.filter { it.first == behovtype }.map { it.second }
