...
internal class ArbeidsopplysningerKorrigertTest : AbstractEndToEndMediatorTest() {
    ...
    fun `Sender ut arbeidsopplysninger_korrigert når arbeidsgiveropplysninger endres av en korrigerende IM`() {
        ...
        assertEquals(
            korrigertInntektsmelding.first,
            UUID.fromString(event["korrigertInntektsmeldingId"].asText()),
        )
        assertEquals(
            korrigerendeInntektsmelding.first,
            UUID.fromString(event["korrigerendeInntektsopplysningId"].asText()),
        )
