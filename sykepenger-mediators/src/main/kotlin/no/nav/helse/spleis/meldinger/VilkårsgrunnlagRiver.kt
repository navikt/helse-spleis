...
internal class VilkårsgrunnlagRiver(
    ...
    override fun validate(message: JsonMessage) {
        ...
        message.require(
            "${InntekterForSykepengegrunnlag.name}.skjæringstidspunkt",
            JsonNode::asLocalDate,
        )
        message.require(
            "${InntekterForOpptjeningsvurdering.name}.skjæringstidspunkt",
            JsonNode::asLocalDate,
        )
        ...
        message.interestedIn("@løsning.${Medlemskap.name}.resultat.svar") {
            require(
                it.asText() in listOf("JA", "NEI", "UAVKLART", "UAVKLART_MED_BRUKERSPORSMAAL")
            ) {
        ...
        message.requireArray("@løsning.${ArbeidsforholdV2.name}") {
            ...
            requireAny(
                "type",
                listOf("FORENKLET_OPPGJØRSORDNING", "FRILANSER", "MARITIMT", "ORDINÆRT"),
            )
