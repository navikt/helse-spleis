...
internal class Utboks(
    ...
    fun nyMelding(block: (personidentifikator: Personidentifikator) -> Utboksmelding) {
        val utboksmelding = block(personidentifikator)
        val utgåendeMelding = utboksmelding.utgåendeMelding
        ...
        tilstand.nyMelding(utboksmelding, this)
    }

    private fun leggTilMeldingOmMeldingHåndtert() {
        if (utboksmeldinger.any { it.utgåendeMelding.eventName == "melding_om_melding_ikke_håndtert_fordi_person_ikke_funnet" }) return
        nyMelding {
            Utboksmelding.ForkastEtterSending(
