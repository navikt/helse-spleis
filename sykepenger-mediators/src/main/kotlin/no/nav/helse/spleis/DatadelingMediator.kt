            ...
            aktivitetslogg.aktiviteter.map { aktivitet ->
                when (aktivitet) {
                    ...
                    is Aktivitet.FunksjonellFeil -> {
                        if (aktivitet.kode.avviklet)
                            sikkerlogg.warn(
                                "${aktivitet.kode} er ikke avviklet, men i bruk i spleis. Endre?"
                            )
                        aktivitetMap("FUNKSJONELL_FEIL", aktivitet) +
        ...
        mapOf(
            ...
            "kontekster" to
                aktivitet.kontekster.map {
                    mapOf("konteksttype" to it.kontekstType, "kontekstmap" to it.kontekstMap)
                },
        )
