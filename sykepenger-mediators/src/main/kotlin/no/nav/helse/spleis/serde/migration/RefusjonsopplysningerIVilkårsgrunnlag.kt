...
import no.nav.helse.spleis.serde.migration.RefusjonData.Companion.parseRefusjon
import no.nav.helse.spleis.serde.migration.RefusjonData.EndringIRefusjonData.Companion.parseEndringerIRefusjon
...
data class Refusjonshistorikk(
    private val refusjoner: List<RefusjonData> = emptyList()
) {
    val erTom get() = refusjoner.isEmpty()

    fun leggTiLRefusjon(refusjon: RefusjonData) {
        refusjoner.add(refusjon)
    }
}
