...
internal class PersonMediator(private val message: HendelseMessage) : PersonObserver {
        ...
        queueMessage(
            JsonMessage.newMessage(
                ...
                mapOf(
                    ...
                    "relevante_sykmeldingsperioder" to
                        event.relevanteSykmeldingsperioder.map { periode ->
                            mapOf("fom" to periode.start, "tom" to periode.endInclusive)
                        },
        ...
        queueMessage(
            JsonMessage.newMessage(
                ...
                mutableMapOf(
                    ...
                    "vedtaksperioder" to
                        event.overlappendeInfotrygdperioder.map { it ->
                            mapOf(
                                "organisasjonsnummer" to it.organisasjonsnummer,
    ...
    override fun vedtaksperiodePåminnet(
        ...
        queueMessage(
            JsonMessage.newMessage("vedtaksperiode_påminnet", påminnelse.toOutgoingMessage())
        )
    }

    override fun vedtaksperiodeIkkePåminnet(
        vedtaksperiodeId: UUID,
        organisasjonsnummer: String,
    ...
    private fun utbetalingAvsluttet(
        eventName: String,
        ...
        queueMessage(
            JsonMessage.newMessage(
                ...
                mapOf(
                    ...
                    "grunnlagForSykepengegrunnlagPerArbeidsgiver" to
                        event.omregnetÅrsinntektPerArbeidsgiver,
                        ...
                        when (val fakta = event.sykepengegrunnlagsfakta) {
                            is PersonObserver.UtkastTilVedtakEvent.FastsattEtterHovedregel ->
                                mapOf(
                                    ...
                                    "6G" to fakta.`6G`,
                                    "arbeidsgivere" to
                                        fakta.arbeidsgivere.map { arbeidsgiver ->
                                            mapOf(
                                                "arbeidsgiver" to arbeidsgiver.arbeidsgiver,
                                                "omregnetÅrsinntekt" to
                                                    arbeidsgiver.omregnetÅrsinntekt,
                                            )
                                        },
                                )
    ...
    override fun trengerArbeidsgiveropplysninger(
        ...
        queueMessage(
            JsonMessage.newMessage("trenger_opplysninger_fra_arbeidsgiver", event.toJsonMap())
        )
    }
    ...
    override fun trengerIkkeArbeidsgiveropplysninger(
        ...
        queueMessage(
            JsonMessage.newMessage("trenger_ikke_opplysninger_fra_arbeidsgiver", event.toJsonMap())
        )
    }
    ...
    override fun arbeidsgiveropplysningerKorrigert(
        ...
        queueMessage(
            JsonMessage.newMessage("arbeidsgiveropplysninger_korrigert", event.toJsonMap())
        )
    }
    ...
    override fun utkastTilVedtak(event: PersonObserver.UtkastTilVedtakEvent) {
        ...
        if (event.`6G` != null)
            utkastTilVedtak["sykepengegrunnlagsfakta"] = mapOf("6G" to event.`6G`)
        queueMessage(JsonMessage.newMessage("utkast_til_vedtak", utkastTilVedtak.toMap()))
    }

    private fun leggPåStandardfelter(outgoingMessage: JsonMessage) =
        outgoingMessage.apply { this["fødselsnummer"] = message.meldingsporing.fødselsnummer }

    private fun queueMessage(outgoingMessage: JsonMessage) {
        queueMessage(
            message.meldingsporing.fødselsnummer,
            leggPåStandardfelter(outgoingMessage).toJson(),
        )
    }
