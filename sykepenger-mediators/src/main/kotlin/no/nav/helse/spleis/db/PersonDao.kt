...
internal class PersonDao(
    ...
    fun hentEllerOpprettPerson(
        ...
        sessionOf(dataSource, returnGeneratedKey = true).use {
            it.transaction { txSession ->
                val (personId, person) = hentPersonEllerOpprettNy(
                    txSession,
                    subsumsjonslogg,
                    hendelseRepository,
                    personidentifikator,
    ...
    private fun gjenopprettFraTidligereBehandling(
        ...
        val (personId, serialisertPerson, tidligerePersoner) = hentPersonOgLåsPersonForBehandling(
            txSession,
            personidentifikator,
            historiskeFolkeregisteridenter
        )
            ...
            ?: return null
        val tidligereBehandlinger = tidligerePersoner.map { tidligerePerson ->
            Person.gjenopprett(
                subsumsjonslogg,
                tidligerePerson.tilPersonDto()
            )
        }
        val personInn = serialisertPerson.tilPersonDto {
    ...
    private fun knyttPersonTilHistoriskeIdenter(
        ...
        val identer =
            if (STØTTER_IDENTBYTTE) historiskeFolkeregisteridenter.plusElement(personidentifikator) else setOf(
                personidentifikator
            )
