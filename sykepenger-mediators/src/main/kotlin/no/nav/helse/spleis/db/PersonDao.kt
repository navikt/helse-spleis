...
internal class PersonDao(
    ...
    private fun hentPersonEllerOpprettNy(
        ...
        gjenopprettFraTidligereBehandling(
            txSession,
            subsumsjonslogg,
            hendelseRepository,
            personidentifikator,
    ...
    private fun gjenopprettFraTidligereBehandling(
        ...
        val (personId, serialisertPerson, tidligerePersoner) =
            hentPersonOgLåsPersonForBehandling(txSession, personidentifikator, historiskeFolkeregisteridenter)
    ...
    private fun hentTidligereBehandledeIdenter(
        ...
        return session
            .run(
                queryOf(statement, *historiskeFolkeregisteridenter.map { it.toLong() }.toTypedArray())
    ...
    private fun knyttPersonTilHistoriskeIdenter(
        ...
        val identer =
            if (STØTTER_IDENTBYTTE) {
                historiskeFolkeregisteridenter.plusElement(
                    personidentifikator,
                )
            } else {
                setOf(personidentifikator)
            }

        @Language("PostgreSQL")
        val statement = """INSERT INTO person_alias(fnr, person_id) VALUES ${identer.joinToString {
            "(?, $personId)"
    ...
    private fun <R> Collection<R>.singleOrNullOrThrow() =
        if (size < 2) {
            ...
            throw IllegalStateException("Listen inneholder mer enn ett element!")
        }
