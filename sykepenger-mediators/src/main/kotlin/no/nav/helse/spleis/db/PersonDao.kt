...
internal class PersonDao(
    ...
    private val STØTTER_IDENTBYTTE: Boolean,
) {
        ...
        */
        sessionOf(dataSource, returnGeneratedKey = true).use {
            it.transaction { txSession ->
                ...
                knyttPersonTilHistoriskeIdenter(
                    txSession,
                    personId,
                    personidentifikator,
                    historiskeFolkeregisteridenter,
    ...
    private fun hentPersonEllerOpprettNy(
        txSession: Session,
        subsumsjonslogg: Subsumsjonslogg,
        hendelseRepository: HendelseRepository,
        personidentifikator: Personidentifikator,
        ...
        return gjenopprettFraTidligereBehandling(
            txSession,
            subsumsjonslogg,
            hendelseRepository,
            personidentifikator,
            historiskeFolkeregisteridenter,
        ) ?: opprettNyPerson(txSession, lagNyPerson)
    }

    private fun gjenopprettFraTidligereBehandling(
        txSession: Session,
        subsumsjonslogg: Subsumsjonslogg,
        hendelseRepository: HendelseRepository,
        personidentifikator: Personidentifikator,
        ...
        val (personId, serialisertPerson, tidligerePersoner) =
            hentPersonOgLåsPersonForBehandling(
                txSession,
                personidentifikator,
                historiskeFolkeregisteridenter,
            )
                ?: hentPersonFraHistoriskeIdenter(
                    txSession,
                    personidentifikator,
                    historiskeFolkeregisteridenter,
                )
                ?: return null
        val tidligereBehandlinger =
            tidligerePersoner.map { tidligerePerson ->
                Person.gjenopprett(subsumsjonslogg, tidligerePerson.tilPersonDto())
            }
        val personInn =
            serialisertPerson.tilPersonDto {
                hendelseRepository.hentAlleHendelser(personidentifikator)
            }
        return personId to Person.gjenopprett(subsumsjonslogg, personInn, tidligereBehandlinger)
    }

    private fun opprettNyPerson(
        txSession: Session,
        lagNyPerson: () -> Person?,
    ): Pair<Long, Person>? {
    ...
    private fun hentTidligereBehandledeIdenter(
        session: Session,
        personId: Long,
        historiskeFolkeregisteridenter: Set<Personidentifikator>,
    ): List<SerialisertPerson> {
        ...
        return session
            .run(
                queryOf(
                        statement,
                        *historiskeFolkeregisteridenter.map { it.toLong() }.toTypedArray(),
                    )
                    .map { it.long("id") to SerialisertPerson(it.string("data")) }
                    .asList
            )
            .distinctBy { (id, _) -> id }
    ...
    private fun hentPersonOgLåsPersonForBehandling(
        session: Session,
        personidentifikator: Personidentifikator,
        historiskeFolkeregisteridenter: Set<Personidentifikator>,
    ): Triple<Long, SerialisertPerson, List<SerialisertPerson>>? {
        @Language("PostgreSQL")
        val statement = "SELECT id, data FROM person WHERE fnr = ? FOR UPDATE"
        val (personId, person) =
            session
                .run(
                    queryOf(statement, personidentifikator.toLong())
                        .map { it.long("id") to SerialisertPerson(it.string("data")) }
                        .asList
                )
        ...
        return Triple(
            personId,
            person,
            hentTidligereBehandledeIdenter(session, personId, historiskeFolkeregisteridenter),
        )
    }

    private fun hentPersonFraHistoriskeIdenter(
        session: Session,
        personidentifikator: Personidentifikator,
        historiskeFolkeregisteridenter: Set<Personidentifikator>,
    ): Triple<Long, SerialisertPerson, List<SerialisertPerson>>? {
        ...
        return session
            .run(
                queryOf(statement, *identer.map { it.toLong() }.toTypedArray())
                    .map { it.long("id") to SerialisertPerson(it.string("data")) }
                    .asList
            )
    ...
    private fun knyttPersonTilHistoriskeIdenter(
        session: Session,
        personId: Long,
        personidentifikator: Personidentifikator,
        historiskeFolkeregisteridenter: Set<Personidentifikator>,
        ...
        val identer =
            if (STØTTER_IDENTBYTTE) historiskeFolkeregisteridenter.plusElement(personidentifikator)
            else setOf(personidentifikator)
        @Language("PostgreSQL")
        val statement =
            """INSERT INTO person_alias(fnr, person_id) VALUES ${identer.joinToString { "(?, $personId)" }} ON CONFLICT DO NOTHING;"""
    ...
    private fun opprettNyPersonversjon(
        session: Session,
        personidentifikator: Personidentifikator,
        skjemaVersjon: Int,
        personJson: String,
        ...
        @Language("PostgreSQL")
        val statement =
            """ INSERT INTO person (fnr, skjema_versjon, data) VALUES (:fnr, :skjemaversjon, :data) """
        return checkNotNull(
            session.run(
                queryOf(
                        statement,
    ...
    private fun oppdaterAvstemmingtidspunkt(
        session: Session,
        message: HendelseMessage,
        personidentifikator: Personidentifikator,
    ) {
    ...
    internal fun manglerAvstemming(): Int {
        ...
        val statement =
            "SELECT count(1) FROM person WHERE sist_avstemt::date < now()::date - interval '31 DAYS'"
        return sessionOf(dataSource).use { session ->
            session.run(queryOf(statement).map { row -> row.int(1) }.asSingle) ?: 0
        }
    }

    private fun oppdaterPersonversjon(
        session: Session,
        personId: Long,
        skjemaVersjon: Int,
        personJson: String,
        ...
        @Language("PostgreSQL")
        val statement =
            """ UPDATE person SET skjema_versjon=:skjemaversjon, data=:data WHERE id=:personId; """
        check(
            1 ==
                session.run(
                    queryOf(
