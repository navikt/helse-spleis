...
internal sealed class HendelseMessage(private val packet: JsonMessage) : Aktivitetskontekst {
    ...
    final override fun toSpesifikkKontekst() =
        SpesifikkKontekst(
            kontekstnavn,
            mapOf("meldingsreferanseId" to meldingsporing.id.toString()),
        )
    ...
    internal fun logReplays(logger: Logger, size: Int) {
        logger.info(
            "som følge av $navn id=${meldingsporing.id} sendes $size meldinger for replay for fnr=${meldingsporing.fødselsnummer}"
        )
    }

    internal fun logOutgoingMessages(logger: Logger, size: Int) {
        logger.info(
            "som følge av $navn id=${meldingsporing.id} sendes $size meldinger på rapid for fnr=${meldingsporing.fødselsnummer}"
        )
    }
    ...
    internal fun secureDiagnosticinfo() = mapOf("fødselsnummer" to meldingsporing.fødselsnummer)

    internal fun tracinginfo() =
        additionalTracinginfo(packet) +
            mapOf("event_name" to navn, "id" to meldingsporing.id, "opprettet" to opprettet)
