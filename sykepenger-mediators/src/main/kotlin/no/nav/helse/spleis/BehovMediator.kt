        ...
        detaljerList
            ...
            .also { detaljer ->
                require(detaljer.size == 1) {
                    sikkerLogg.error(
                        "Forsøkte å sende duplikate behov på kontekst ${kontekst.entries.joinToString { "${it.key}=${it.value}" }}",
                    )
                    "Kan ikke produsere samme behov på samme kontekst med ulike detaljer. Forsøkte å be om ${behovliste.joinToString {
                        it.type.name
                    }}"
                }
            }.single()
