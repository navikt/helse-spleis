            ...
            ?.let {
                ...
                call.respond(serialisertPerson.apply {
                    putArray("hendelser").addAll(hendelser.map { hendelse ->
                        objectMapper.valueToTree<JsonNode>(tilDTO(hendelse.first, hendelse.second, aktivitetslogger))
                    })
                })
...
internal fun tilDTO(meldingstype: Meldingstype, hendelse: String, aktivitetslogger: Aktivitetslogger): HendelseDTO =
    when (meldingstype) {
        Meldingstype.NY_SØKNAD -> NySøknadMessage(hendelse, aktivitetslogger).asSpeilDTO()
        Meldingstype.SENDT_SØKNAD -> SendtSøknadMessage(hendelse, aktivitetslogger).asSpeilDTO()
        Meldingstype.INNTEKTSMELDING -> InntektsmeldingMessage(hendelse, aktivitetslogger).asSpeilDTO()
