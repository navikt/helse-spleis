...
internal class SykepengegrunnlagForArbeidsgiverMessage(
    packet: JsonMessage,
    ...
    private val inntekterForSykepengegrunnlag =
        VilkårsgrunnlagMessage.mapSkatteopplysninger(
            packet["@løsning.${InntekterForSykepengegrunnlagForArbeidsgiver.name}"]
        )

    private val skjæringstidspunkter =
        packet["${InntekterForSykepengegrunnlagForArbeidsgiver.name}.skjæringstidspunkt"]
            .asLocalDate()
    ...
    private val sykepengegrunnlagForArbeidsgiver
        get() =
            SykepengegrunnlagForArbeidsgiver(
                ...
                inntekter =
                    if (inntekterForSykepengegrunnlag.isEmpty())
                        ArbeidsgiverInntekt(organisasjonsnummer, emptyList())
                    else inntekterForSykepengegrunnlag.single(),
            )
