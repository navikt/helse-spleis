...
Når en melding er sendt OK flyttes den fra tabellen `utboks` til `sendt`. Selve meldingen har også fått et `@sendt`-tidspunkt satt på seg (UTC) som er det samme som i `sendt`-kolonnen i `sendt`-tabellen.
