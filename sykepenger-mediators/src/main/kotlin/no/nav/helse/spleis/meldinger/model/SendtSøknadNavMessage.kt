...
internal class SendtSøknadNavMessage(
    ...
    internal companion object {
        internal fun byggSendtSøknad(builder: SendtSøknadBuilder, packet: JsonMessage) {
            builder.permittert(
                packet["permitteringer"]
                    .takeIf(JsonNode::isArray)
                    ?.takeUnless { it.isEmpty }
                    ?.let { true } ?: false
            )
            builder.egenmeldinger(
                packet["egenmeldingsdagerFraSykmelding"]
                    .takeIf(JsonNode::isArray)
                    ?.map { LocalDate.parse(it.asText()) }
                    ?.grupperSammenhengendePerioderMedHensynTilHelg() ?: emptyList()
            )
            ...
            packet["inntektFraNyttArbeidsforhold"].takeIf(JsonNode::isArray)?.forEach {
                builder.tilkommenInntekt(
                    ...
                    beløp = it.path("belop").takeUnless(JsonNode::isMissingOrNull)?.asInt(),
                )
            }
        ...
        private fun andreInntektskilder(
            ...
            // fjerner ANDRE_ARBEIDSFORHOLD dersom <ikkeJobbetIDetSisteFraAnnetArbeidsforhold> er
            // satt til true
            return andreInntektskilder
                .map { it.path("type").asText() }
                .filterNot { kilde ->
                    kilde == "ANDRE_ARBEIDSFORHOLD" && ikkeJobbetIDetSisteFraAnnetArbeidsforhold
                }
        }

        private fun harSvartNeiOmIkkeJobbetIDetSisteFraAnnetArbeidsforhold(
            listeAvSpørsmål: JsonNode
        ): Boolean {
            val svarene = spørsmål(listeAvSpørsmål)
            val svarPåOmJobbetIDetSiste =
                svarene
                    .firstOrNull {
                        it.path("tag").asText() ==
