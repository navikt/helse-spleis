...
internal class VilkårsgrunnlagMessage(
    ...
    private val inntekterForSykepengegrunnlag =
        mapSkatteopplysninger(packet["@løsning.${InntekterForSykepengegrunnlag.name}"])

    private val inntekterForOpptjeningsvurdering =
        mapSkatteopplysninger(
            packet["@løsning.${Aktivitet.Behov.Behovtype.InntekterForOpptjeningsvurdering.name}"]
        )
    private val arbeidsforhold =
        packet["@løsning.${ArbeidsforholdV2.name}"]
                    ...
                    type =
                        when (it["type"].asText()) {
                            ...
                            "ORDINÆRT" -> Arbeidsforholdtype.ORDINÆRT
                                ...
                                error(
                                    "har ikke mappingregel for arbeidsforholdtype: ${it["type"].asText()}"
                                )
                        },
                )
            }

    private val medlemskapstatus =
        when (packet["@løsning.${Medlemskap.name}.resultat.svar"].asText()) {
    ...
    private val skjæringstidspunkter =
        listOfNotNull(
            packet["${InntekterForSykepengegrunnlag.name}.skjæringstidspunkt"].asLocalDate(),
            packet[
                    "${Aktivitet.Behov.Behovtype.InntekterForOpptjeningsvurdering.name}.skjæringstidspunkt"]
                .asLocalDate(),
    ...
    private val vilkårsgrunnlag
        get() =
            Vilkårsgrunnlag(
                ...
                medlemskapsvurdering = Medlemskapsvurdering(medlemskapstatus = medlemskapstatus),
                inntektsvurderingForSykepengegrunnlag =
                    InntektForSykepengegrunnlag(inntekter = inntekterForSykepengegrunnlag),
                inntekterForOpptjeningsvurdering =
                    no.nav.helse.hendelser.InntekterForOpptjeningsvurdering(
                        inntekter = inntekterForOpptjeningsvurdering
                    ),
    ...
    internal companion object {
        ...
        private fun JsonNode.asInntekttype() =
            when (this.asText()) {
            ...
            opplysninger
                ...
                .groupBy({ inntekt -> inntekt.arbeidsgiver() }) { inntekt ->
                    ArbeidsgiverInntekt.MånedligInntekt(
                        ...
                        fordel =
                            if (inntekt.path("fordel").isTextual) inntekt["fordel"].asText()
                            else "",
                        beskrivelse =
                            if (inntekt.path("beskrivelse").isTextual)
                                inntekt["beskrivelse"].asText()
                            else "",
                    )
                }
                .map { (arbeidsgiver, inntekter) -> ArbeidsgiverInntekt(arbeidsgiver, inntekter) }
    }
}
