...
internal class SubsumsjonMediator(
    ...
    fun ferdigstill(producer: Subsumsjonproducer) {
        ...
        logg.info(
            "som følge av hendelse id=${message.meldingsporing.id} sendes ${subsumsjoner.size} subsumsjonsmeldinger på rapid"
        )
        subsumsjoner
            ...
            .forEach {
                ...
                sikkerLogg.info(
                    "som følge av hendelse id=${this.message.meldingsporing.id} sender subsumsjon: $message"
                )
    ...
    private fun subsumsjonMelding(event: SubsumsjonEvent): JsonMessage {
        return JsonMessage.newMessage(
            ...
            mapOf(
                ...
                "@forårsaket_av" to mapOf("id" to message.meldingsporing.id),
                "subsumsjon" to
                    buildMap {
                        this["id"] = event.id
                        ...
                        this["sporing"] =
                            event.sporing.map { it.key.tilEkstern() to it.value }.toMap()
    ...
    private fun KontekstType.tilEkstern() =
        when (this) {
