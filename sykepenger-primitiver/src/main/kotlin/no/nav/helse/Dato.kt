...
class Ukedager(private val antallUkedager: Int) {
    private companion object {
        // tabellen er en sammenslått tabell på 5 kolonner og 7 rader (én for hver ukedag) som angir
        // hvor mange
        // dager man skal addere med gitt ukedagen til datoen og hvor mange ukedager man skal addere
        // feks lørdag + 1 ukedag => 2 fordi man skal først hoppe over søndag og deretter ukedagen
        // (mandag).
        // Et koordinat (x, y) i en 2D-tabell med w kolonner kan omgjøres til et punkt z i en
        // 1D-tabell ved formelen z = f(x, y, w) = wx + y
...
val LocalDate.forrigeDag
    get() = this.minusDays(1)
val LocalDate.nesteDag
    get() = this.plusDays(1)

fun LocalDate.førsteArbeidsdag(): LocalDate = this + 0.ukedager

val LocalDate.førsteArbeidsdagEtter
    get() = if (erHelg()) førsteArbeidsdag() else nesteArbeidsdag()
