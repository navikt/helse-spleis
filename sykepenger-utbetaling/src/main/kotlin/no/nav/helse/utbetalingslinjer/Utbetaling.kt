...
class Utbetaling private constructor(
    ...
    constructor(
        ...
        korrelerendeUtbetaling
            ?.takeIf {
                arbeidsgiverOppdrag.tilhører(it.arbeidsgiverOppdrag) || personOppdrag.tilhører(it.personOppdrag)
            }?.korrelasjonsId
            ?: UUID.randomUUID(),
        periode,
    ...
    private fun harHåndtert(hendelse: UtbetalingmodulHendelse) = (hendelse === forrigeHendelse).also { forrigeHendelse = hendelse }
    ...
    fun harUtbetalinger() = harOppdragMedUtbetalinger() || annulleringer.any { it.harOppdragMedUtbetalinger() }

    private fun harOppdragMedUtbetalinger() = arbeidsgiverOppdrag.harUtbetalinger() || personOppdrag.harUtbetalinger()
    ...
    fun håndter(
        hendelse: UtbetalingsavgjørelseHendelse,
    ...
    fun håndter(
        ...
        if (!relevantFor(utbetaling)) return håndterKvitteringForAnnullering(utbetaling, aktivitetslogg)
    ...
    private fun relevantFor(utbetaling: UtbetalingmodulHendelse) =
        utbetaling.utbetalingId == this.id &&
            (utbetaling.fagsystemId in setOf(this.arbeidsgiverOppdrag.fagsystemId, this.personOppdrag.fagsystemId))
    ...
    fun håndter(
        påminnelse: UtbetalingpåminnelseHendelse,
        aktivitetslogg: IAktivitetslogg,
    ) {
    ...
    fun gjelderFor(hendelse: UtbetalingmodulHendelse) = relevantFor(hendelse) || annulleringer.any { it.relevantFor(hendelse) }
    ...
    private fun tilstand(
        ...
        if (Oppdrag.ingenFeil(arbeidsgiverOppdrag, personOppdrag) &&
            !Oppdrag.synkronisert(
                arbeidsgiverOppdrag,
                personOppdrag,
            )
        ) {
    ...
    private fun nyUtbetaling(
        ...
        val nyttArbeidsgiveroppdrag =
            byggViderePåOppdrag(aktivitetslogg, vedtaksperiode, utbetalingsaken, this.arbeidsgiverOppdrag, kladd.arbeidsgiveroppdrag)
        val nyttPersonoppdrag =
            byggViderePåOppdrag(aktivitetslogg, vedtaksperiode, utbetalingsaken, this.personOppdrag, kladd.personoppdrag)
        ...
        val nyUtbetalingstidslinje =
            tidligereUtbetalingstidslinje + kladd.utbetalingstidslinje +
    ...
    fun erNyereEnn(other: LocalDateTime): Boolean = other <= tidsstempel
    ...
    internal object Godkjent : Tilstand {
        ...
        private fun vurderNesteTilstand(
            utbetaling: Utbetaling,
            aktivitetslogg: IAktivitetslogg,
        ) {
