...
class Utbetalingslinje(
    ...
    companion object {
        ...
        private fun sisteLinjeSkalIkkeTrekkesIHelg(
            linjer: List<Utbetalingslinje>
        ): List<Utbetalingslinje> {
        ...
        private fun nyeLinjer(fagsystemId: String, linjer: List<Utbetalingslinje>) =
            linjer.take(1).map { it.kopier(refFagsystemId = null) } +
                linjer.drop(1).map { it.kopier(refFagsystemId = fagsystemId) }
        ...
        // oppdraget utgjør på sett og vis en linket liste hvor hver linje har et nummer, og peker
        // tilbake på forrige linje
    ...
    fun dager() = fom.datesUntil(tom.plusDays(1)).filter { !it.erHelg() }.toList()
    ...
    fun markerUendret(tidligere: Utbetalingslinje) =
        kopier(
            endringskode = UEND,
    ...
    fun endreEksisterendeLinje(tidligere: Utbetalingslinje) =
        kopier(
            endringskode = ENDR,
            delytelseId = tidligere.delytelseId,
            refDelytelseId = null,
    ...
    fun slåSammenLinje(førsteLinjeIForrige: Utbetalingslinje): Utbetalingslinje? {
        if (
            this.beløp != førsteLinjeIForrige.beløp ||
                this.grad != førsteLinjeIForrige.grad ||
                !this.tom.erRettFør(førsteLinjeIForrige.fom)
        )
            return null
    ...
    internal fun kuttHelg(): Utbetalingslinje? {
        return when (tom.dayOfWeek) {
            DayOfWeek.SUNDAY ->
                tom.minusDays(2).takeUnless { it < fom }?.let { nyTom -> kopier(tom = nyTom) }
            DayOfWeek.SATURDAY ->
                tom.forrigeDag.takeUnless { it < fom }?.let { nyTom -> kopier(tom = nyTom) }
            else -> this
        }
    }
    ...
    fun behovdetaljer() =
        mapOf<String, Any?>(
            ...
            "grad" to
                grad
                    ?.toDouble(), // backwards-compatibility mot andre systemer som forventer
                                  // double: må gjennomgås
            ...
            klassekode =
                when (klassekode) {
                    ...
                    Klassekode.SykepengerArbeidstakerOrdinær ->
                        KlassekodeDto.SykepengerArbeidstakerOrdinær
                    Klassekode.SykepengerArbeidstakerFeriepenger ->
                        KlassekodeDto.SykepengerArbeidstakerFeriepenger
                },
