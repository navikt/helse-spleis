...
val UtbetalingsavgjørelseHendelse.avvist
    get() = !godkjent
...
private val UtbetalingsavgjørelseHendelse.manueltBehandlet
    get() = !automatisert
...
fun UtbetalingsavgjørelseHendelse.valider(aktivitetslogg: IAktivitetslogg) {
    when {
        avvist && manueltBehandlet -> {
            ...
            aktivitetslogg.info(
                "Utbetaling markert som ikke godkjent av saksbehandler ${saksbehandler()} $avgjørelsestidspunkt"
            )
        }
        avvist && automatisert -> {
            aktivitetslogg.funksjonellFeil(RV_UT_18)
            aktivitetslogg.info(
                "Utbetaling markert som ikke godkjent automatisk $avgjørelsestidspunkt"
            )
        }
        godkjent && manueltBehandlet ->
            aktivitetslogg.info(
                "Utbetaling markert som godkjent av saksbehandler ${saksbehandler()} $avgjørelsestidspunkt"
            )
        else ->
