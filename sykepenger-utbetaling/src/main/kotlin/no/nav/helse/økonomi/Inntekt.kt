...
class Inntekt private constructor(val årlig: Double) : Comparable<Inntekt> {
    ...
    init {
        require(
            this.årlig !in listOf(Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.NaN)
        ) {
    ...
    companion object {
        ...
        val Number.K
            get() = this.toDouble() * 1000

        val Number.månedlig
            get() = Inntekt(this.toDouble() * 12)

        val Number.årlig
            get() = Inntekt(this.toDouble())

        val Number.daglig
            get() = Inntekt(this.toDouble() * ARBEIDSDAGER_PER_ÅR)
    ...
    override fun compareTo(other: Inntekt) =
        if (this == other) 0 else this.årlig.compareTo(other.årlig)
