...
internal class UtbetalingTest {
    ...
    fun `forlenger seg ikke på en annullering`() {
        ...
        assertNotEquals(
            første.inspektør.arbeidsgiverOppdrag.inspektør
                .fagsystemId(),
            andre.inspektør.arbeidsgiverOppdrag.inspektør
                .fagsystemId(),
        )
    }
    ...
    fun `omgjøre en revurdert periode med opphør`() {
        ...
        assertEquals(
            første.inspektør.arbeidsgiverOppdrag.inspektør
                .fagsystemId(),
            tredje.inspektør.arbeidsgiverOppdrag.inspektør
                .fagsystemId(),
        )
        assertEquals(første.inspektør.korrelasjonsId, tredje.inspektør.korrelasjonsId)
        assertEquals(
            17.januar,
            tredje.inspektør.arbeidsgiverOppdrag
                .first()
                .inspektør.fom,
        )
        assertEquals(
        ...
        assertEquals(
            17.januar,
            tredje.inspektør.arbeidsgiverOppdrag.inspektør.periode
                ?.start,
        )
    }
    ...
    fun `forlenge en utbetaling uten utbetaling`() {
        ...
        assertEquals(
            NY,
            andre.inspektør.arbeidsgiverOppdrag[0]
                .inspektør.endringskode,
        )
        assertEquals(
            17.januar,
            andre.inspektør.arbeidsgiverOppdrag[0]
    ...
    fun `kan ikke forkaste utbetaling i spill`() {
        ...
        kvittèr(
            utbetaling,
            utbetaling.inspektør.arbeidsgiverOppdrag.inspektør
                .fagsystemId(),
        )
        assertFalse(Utbetaling.kanForkastes(listOf(utbetaling), listOf(utbetaling)))
    }
    ...
    fun `tar imot kvittering på det andre oppdraget selv om utbetalingen har feilet`() {
        ...
        assertEquals(
            AKSEPTERT,
            utbetaling.inspektør.personOppdrag.inspektør
                .status(),
        )
        assertEquals(OVERFØRT, utbetaling.inspektør.tilstand)
    }
    ...
    fun `annullere delvis refusjon`() {
        ...
        assertTrue(
            annullering.inspektør.arbeidsgiverOppdrag
                .last()
                .erOpphør(),
        )
        assertTrue(
            annullering.inspektør.personOppdrag
                .last()
                .erOpphør(),
        )
    }
    ...
    private fun opprettGodkjentUtbetaling() = opprettGodkjentUtbetaling(beregnUtbetalinger(tidslinjeOf(16.AP, 5.NAV(3000))))
