...
data class SpannerPersonDto(
    ...
    data class ArbeidsgiverData(
        ...
        data class VedtaksperiodeData(
            ...
            val gjeldende = behandlinger.last().endringer.last().let { gjeldendeEndring ->
                Gjeldende(
...
fun PersonUtDto.tilSpannerPersonDto(): SpannerPersonDto {
    ...
    return SpannerPersonDto(
        ...
        minimumSykdomsgradVurdering = minimumSykdomsgradVurdering.perioder.map {
            SpannerPersonDto.MinimumSykdomsgradVurderingPeriode(
                it.fom,
                it.tom
            )
        },
...
private fun ArbeidsgiverUtDto.tilPersonData(vilkårsgrunnlagHistorikk: List<VilkårsgrunnlagInnslagData>): SpannerPersonDto.ArbeidsgiverData {
    ...
    return SpannerPersonDto.ArbeidsgiverData(
        ...
        vedtaksperioder = this.vedtaksperioder.map { vedtaksperiode ->
            ...
            val vilkårsgrunnlag = gjeldendeEndring.vilkårsgrunnlagId?.let { vilkårsgrunnlagId ->
                vilkårsgrunnlagHistorikk.flatMap { it.vilkårsgrunnlag }
                    .first { it.vilkårsgrunnlagId == vilkårsgrunnlagId }
            }
            val utbetaling =
                gjeldendeEndring.utbetalingId?.let { utbetalingId -> utbetalingerDto.first { it.id == utbetalingId } }
