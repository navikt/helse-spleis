...
private fun migrateV2Task(arbeidId: String) {
    ...
    opprettOgUtførArbeid(arbeidId) { session, fnr ->
        session.transaction { txSession ->
            // låser ned person-raden slik at spleis ikke tar inn meldinger og overskriver mens
            // denne podden holder på
...
private fun fåLås(session: Session, arbeidId: String): Boolean {
    ...
    @Language("PostgreSQL") val query = "SELECT pg_try_advisory_lock(1337)"
            ...
            do {
                ...
                val arbeidsliste =
                    hentArbeid(session, arbeidId, size)
                        .also { if (it.isNotEmpty()) log.info("Fikk ${it.size} stk") }
                        .onEach { fnr ->
                            try {
                ...
                .use { session ->
                    session.run(
                        queryOf("SELECT fnr FROM person")
                            .map { row -> row.string("fnr").padStart(11, '0') }
                            .asList
                    )
                }
                .forEach { fnr ->
...
private fun avstemmingTask(factory: ConsumerProducerFactory, customDayOfMonth: Int? = null) {
    ...
    // spørringen funker slik at den putter alle personer opp i 28 ulike "bøtter" (fnr % 28 gir tall
    // mellom 0 og 27, også plusser vi på én slik at vi starter fom. 1)
    // hvor én bøtte tilsvarer én dag i måneden. Tallet 28 (pga februar) ble valgt slik at vi sikrer
    // oss at vi avstemmer
    // alle personer hver måned. Dag 29, 30, 31 avstemmes 0 personer siden det er umulig å ha disse
    // rest-verdiene
...
private class DataSourceConfiguration(dbUsername: DbUser) {
    ...
    fun dataSource(maximumPoolSize: Int = 2) =
        HikariDataSource(hikariConfig.apply { this.maximumPoolSize = maximumPoolSize })
}
