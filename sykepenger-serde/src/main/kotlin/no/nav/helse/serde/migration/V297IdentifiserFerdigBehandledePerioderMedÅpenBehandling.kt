        ...
        try {
            ...
            jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
                ...
                arbeidsgiver
                    ...
                    .filter {
                        it.path("tilstand").asText() in
                            setOf("AVSLUTTET", "AVSLUTTET_UTEN_UTBETALING")
                    }
                    .forEach { ferdigBehandletVedtaksperiode ->
                        val påminnelse =
                            if (
                                ferdigBehandletVedtaksperiode.path("tilstand").asText() ==
                                    "AVSLUTTET"
                            )
                                ferdigBehandletVedtaksperiode.validerVedtakIverksatt(
                                    aktørId,
                                    fødselsnummer,
                                    organisasjonsnummer,
                                )
                            else
                                ferdigBehandletVedtaksperiode.validerAvsluttetUtenVedtak(
                                    aktørId,
                                    fødselsnummer,
                                    organisasjonsnummer,
                                )
            ...
            val sisteBehandling =
                sisteBehandlingOrNull()
                    ?: return sikkerLogg
                        ...
                        .let { null }
            val tilstand =
                sisteBehandling.path("tilstand").asText().takeUnless {
                    it == "AVSLUTTET_UTEN_VEDTAK"
                } ?: return null
            sikkerLogg.error(
