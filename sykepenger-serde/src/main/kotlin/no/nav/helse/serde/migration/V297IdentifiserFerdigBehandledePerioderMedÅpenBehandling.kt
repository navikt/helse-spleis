...
internal class V297IdentifiserFerdigBehandledePerioderMedÅpenBehandling :
    JsonMigration(version = 297) {
        ...
        try {
            ...
            jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
                ...
                arbeidsgiver.path("vedtaksperioder")
                    .filter {
                        it.path("tilstand").asText() in setOf(
                            "AVSLUTTET",
                    ...
                    .forEach { ferdigBehandletVedtaksperiode ->
                        val påminnelse =
                            if (ferdigBehandletVedtaksperiode.path("tilstand")
                                    .asText() == "AVSLUTTET") ferdigBehandletVedtaksperiode.validerVedtakIverksatt(
                                aktørId,
                                fødselsnummer,
                                organisasjonsnummer
                            )
                            else ferdigBehandletVedtaksperiode.validerAvsluttetUtenVedtak(
                                aktørId,
                                fødselsnummer,
                                organisasjonsnummer
                            )
            ...
            sikkerLogg.info(
                "Påminnelse for å fikse behandlinger for {}:\n\t${snutePåminnelse}",
                keyValue("aktørId", aktørId)
            )
        } catch (exception: Exception) {
            sikkerLogg.error(
                "Feil ved migrering til versjon 297 for {}",
                keyValue("aktørId", aktørId),
                exception
            )
        }
    ...
    private companion object {
        ...
        private fun JsonNode.validerVedtakIverksatt(
            aktørId: String,
            fødselsnummer: String,
            organisasjonsnummer: String
        ): Pair<LocalDate, JsonNode>? {
            val vedtaksperiodeId = path("id").asText()
            val sisteBehandling = sisteBehandlingOrNull() ?: return sikkerLogg.error(
                "Avsluttet {} for {} på {} har ingen behandlinger",
                keyValue("vedtaksperiodeId", vedtaksperiodeId),
                keyValue("aktørId", aktørId),
                keyValue("organisasjonsnummer", organisasjonsnummer)
            ).let { null }
            val tilstand =
                sisteBehandling.path("tilstand").asText().takeUnless { it == "VEDTAK_IVERKSATT" }
                    ?: return null
            sikkerLogg.error(
                "Avsluttet {} for {} på {} har siste {} i {}",
                keyValue("vedtaksperiodeId", vedtaksperiodeId),
                keyValue("aktørId", aktørId),
                keyValue("organisasjonsnummer", organisasjonsnummer),
                keyValue("behandlingId", sisteBehandling.path("id").asText()),
                keyValue("tilstand", tilstand)
            )
            return sisteBehandling.fom() to påminnelse(
                aktørId,
                fødselsnummer,
                organisasjonsnummer,
                vedtaksperiodeId,
        ...
        private fun JsonNode.validerAvsluttetUtenVedtak(
            aktørId: String,
            fødselsnummer: String,
            organisasjonsnummer: String
        ): Pair<LocalDate, JsonNode>? {
            val vedtaksperiodeId = path("id").asText()
            val sisteBehandling = sisteBehandlingOrNull() ?: return sikkerLogg.error(
                "AvsluttetUtenUtbetaling {} for {} på {} har ingen behandlinger",
                keyValue("vedtaksperiodeId", vedtaksperiodeId),
                keyValue("aktørId", aktørId),
                keyValue("organisasjonsnummer", organisasjonsnummer)
            ).let { null }
            val tilstand = sisteBehandling.path("tilstand").asText()
                .takeUnless { it == "AVSLUTTET_UTEN_VEDTAK" } ?: return null
            sikkerLogg.error(
                "AvsluttetUtenUtbetaling {} for {} på {} har siste {} i {}",
                keyValue("vedtaksperiodeId", vedtaksperiodeId),
                keyValue("aktørId", aktørId),
                keyValue("organisasjonsnummer", organisasjonsnummer),
                keyValue("behandlingId", sisteBehandling.path("id").asText()),
