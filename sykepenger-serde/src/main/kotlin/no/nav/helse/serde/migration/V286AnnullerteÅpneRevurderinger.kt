...
internal class V286AnnullerteÅpneRevurderinger : JsonMigration(version = 286) {
    override val description =
        "smelter sammen til_infotrygd-generasjonen og nest siste generasjon hvis den er en åpen revurdering"
    ...
    override fun doMigration(jsonNode: ObjectNode, meldingerSupplier: MeldingerSupplier) {
        ...
        jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
            ...
            arbeidsgiver.path("vedtaksperioder")
                .forEach { migrerVedtaksperiode(aktørId, organisasjonsnummer, it) }
            arbeidsgiver.path("forkastede").forEach {
                migrerForkastetVedtaksperiode(
                    aktørId,
                    organisasjonsnummer,
    ...
    private fun migrerForkastetVedtaksperiode(
        ...
        val avsluttettidspunkt =
            sisteGenerasjon.path("avsluttet").asText().takeUnless(String::isBlank)
        if (avsluttettidspunkt == null) return sikkerLogg.info(
            "[V286] Ingen avsluttettidspunkt for generasjon ${
                sisteGenerasjon.path(
                    "id"
                ).asText()
            }", kv("aktørId", aktørId), kv("orgnr", orgnr)
        )
