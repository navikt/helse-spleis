    ...
    private fun migrerVedtaksperiode(
        ...
        if (sisteGenerasjon.path("tilstand").asText() != "TIL_INFOTRYGD")
            return sikkerLogg.info(
                "[V289] siste generasjon er ikke TIL_INFOTRYGD for ${vedtaksperiode.path("fom").asText()} - ${vedtaksperiode.path("tom").asText()} {}, {}, {}",
                kv("aktørId", aktørId),
                kv("orgnr", orgnr),
        ...
        sikkerLogg.info(
            "[V289] setter avsluttettidspunkt på generasjon ${sisteGenerasjon.path("id").asText()} for {}",
            kv("aktørId", aktørId),
        )
        sisteGenerasjon.put(
            "avsluttet",
            sisteGenerasjon.path("endringer").last().path("tidsstempel").asText(),
        )
    }
