...
internal class V281ForkasteAvsluttedePerioderMedUberegnetGenerasjon : JsonMigration(281) {
    ...
    private fun migrer(jsonNode: ObjectNode, aktørId: String) {
        jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
            ...
            forkastinger.forEach { periode ->
                ...
                val indeks = vedtaksperioder.indexOfFirst { vedtaksperiode ->
    ...
    private fun migrerVedtaksperiode(
        ...
        if (sisteIverksetting is ObjectNode) {
            sikkerlogg.info(
                "[V281] $aktørId Har UBEREGNET generasjon etter en VEDTAK_IVERKSATT ${
                    periode.path(
                        "fom"
                    ).asText()
                } - ${periode.path("tom").asText()} (${
                    periode.path("id").asText()
                }) og vedtaksperioden er $tilstandForVedtaksperiode"
            )
            return
        }

        sikkerlogg.info(
            "[V281] $aktørId Forkaster vedtaksperiode som er i AVSLUTTET, men med FORKASTET utbetaling og er UBEREGNET ${
                periode.path(
                    "fom"
                ).asText()
            } - ${periode.path("tom").asText()} (${periode.path("id").asText()})"
        )
