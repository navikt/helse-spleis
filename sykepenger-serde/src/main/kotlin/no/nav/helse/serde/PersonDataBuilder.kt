    ...
    PersonData(
        ...
        minimumSykdomsgradVurdering =
            minimumSykdomsgradVurdering.perioder.map {
                PersonData.MinimumSykdomsgradVurderingPeriodeData(it.fom, it.tom)
            },
...
private fun InntektsopplysningUtDto.InntektsmeldingDto.tilPersonData() =
    PersonData.ArbeidsgiverData.InntektsmeldingData(
        id = this.id,
        dato = this.dato,
        ...
        kilde =
            when (this.kilde) {
                InntektsopplysningUtDto.InntektsmeldingDto.KildeDto.Arbeidsgiver ->
                    PersonData.VilkårsgrunnlagElementData.ArbeidsgiverInntektsopplysningData
                        .InntektsopplysningData
                        .InntektsmeldingKildeDto
                        .Arbeidsgiver
...
private fun SykdomshistorikkElementDto.tilPersonData() =
    PersonData.SykdomshistorikkData(
        id = this.id,
        tidsstempel = this.tidsstempel,
        hendelseId = this.hendelseId,
...
private fun SykdomstidslinjeDto.tilPersonData() =
    PersonData.ArbeidsgiverData.SykdomstidslinjeData(
        dager = dager.map { it.tilPersonData() }.forkortSykdomstidslinje(),
        låstePerioder =
            this.låstePerioder.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
...
private fun SykdomstidslinjeDagDto.tilPersonData() =
    when (this) {
                ...
                type =
                    when (this.ytelse) {
                        SykdomstidslinjeDagDto.AndreYtelserDto.YtelseDto.AAP ->
                            PersonData.ArbeidsgiverData.SykdomstidslinjeData.JsonDagType
                                .ANDRE_YTELSER_AAP
        ...
        is SykdomstidslinjeDagDto.ForeldetSykedagDto ->
            DagData(
                type =
                    PersonData.ArbeidsgiverData.SykdomstidslinjeData.JsonDagType.FORELDET_SYKEDAG,
                kilde = this.kilde.tilPersonData(),
                grad = this.grad.prosent,
                other = null,
                melding = null,
...
private fun EndringIRefusjonDto.tilPersonData() =
    PersonData.ArbeidsgiverData.RefusjonData.EndringIRefusjonData(
        beløp = this.beløp.beløp,
        endringsdato = this.endringsdato,
    )

private fun SykmeldingsperioderDto.tilPersonData() =
    perioder.map { PersonData.ArbeidsgiverData.SykmeldingsperiodeData(it.fom, it.tom) }

private fun ForkastetVedtaksperiodeUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.ForkastetVedtaksperiodeData(
...
private fun VedtaksperiodeUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData(
        ...
        tilstand =
            when (tilstand) {
                ...
                VedtaksperiodetilstandDto.AVSLUTTET_UTEN_UTBETALING ->
                    TilstandTypeData.AVSLUTTET_UTEN_UTBETALING
                VedtaksperiodetilstandDto.AVVENTER_BLOKKERENDE_PERIODE ->
                    TilstandTypeData.AVVENTER_BLOKKERENDE_PERIODE
                VedtaksperiodetilstandDto.AVVENTER_GODKJENNING ->
                    TilstandTypeData.AVVENTER_GODKJENNING
                VedtaksperiodetilstandDto.AVVENTER_GODKJENNING_REVURDERING ->
                    TilstandTypeData.AVVENTER_GODKJENNING_REVURDERING
        ...
        egenmeldingsperioder =
            egenmeldingsperioder.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
        opprettet = opprettet,
        oppdatert = oppdatert,
    )

private fun BehandlingUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData(
        id = this.id,
        tilstand =
            when (this.tilstand) {
                BehandlingtilstandDto.ANNULLERT_PERIODE ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.TilstandData
                        .ANNULLERT_PERIODE
...
private fun BehandlingkildeDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.KildeData(
        meldingsreferanseId = this.meldingsreferanseId,
...
private fun AvsenderDto.tilPersonData() =
    when (this) {
        AvsenderDto.ARBEIDSGIVER ->
            PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.AvsenderData.ARBEIDSGIVER
        AvsenderDto.SAKSBEHANDLER ->
            PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.AvsenderData.SAKSBEHANDLER
        AvsenderDto.SYKMELDT ->
            PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.AvsenderData.SYKMELDT
        AvsenderDto.SYSTEM ->
            PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.AvsenderData.SYSTEM
    }

private fun BehandlingendringUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.BehandlingData.EndringData(
        id = id,
        ...
        arbeidsgiverperioder =
            arbeidsgiverperioder.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
        maksdatoresultat = maksdatoresultat.tilPersonData(),
    )

private fun MaksdatoresultatUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.MaksdatoresultatData(
        vurdertTilOgMed = vurdertTilOgMed,
        bestemmelse =
            when (bestemmelse) {
                MaksdatobestemmelseDto.IKKE_VURDERT ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.MaksdatobestemmelseData
                        .IKKE_VURDERT
                ...
                MaksdatobestemmelseDto.SYTTI_ÅR ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.MaksdatobestemmelseData.SYTTI_ÅR
            },
        startdatoTreårsvindu = startdatoTreårsvindu,
        startdatoSykepengerettighet = startdatoSykepengerettighet,
        forbrukteDager =
            forbrukteDager.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
        oppholdsdager =
            oppholdsdager.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
        avslåtteDager =
            avslåtteDager.map { PersonData.ArbeidsgiverData.PeriodeData(it.fom, it.tom) },
        maksdato = maksdato,
...
private fun DokumentsporingDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentsporingData(
        dokumentId = this.id,
        dokumenttype =
            when (type) {
                DokumenttypeDto.InntektsmeldingDager ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData
                ...
                DokumenttypeDto.OverstyrInntekt ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData.OverstyrInntekt
                DokumenttypeDto.OverstyrRefusjon ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData.OverstyrRefusjon
                ...
                DokumenttypeDto.Sykmelding ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData.Sykmelding
                DokumenttypeDto.Søknad ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData.Søknad
                DokumenttypeDto.AndreYtelser ->
                    PersonData.ArbeidsgiverData.VedtaksperiodeData.DokumentTypeData.AndreYtelser
            },
    )

private fun UtbetalingUtDto.tilPersonData() =
    PersonData.UtbetalingData(
        id = this.id,
        ...
        type =
            when (this.type) {
                UtbetalingtypeDto.ANNULLERING ->
                    PersonData.UtbetalingData.UtbetalingtypeData.ANNULLERING
                UtbetalingtypeDto.ETTERUTBETALING ->
                    PersonData.UtbetalingData.UtbetalingtypeData.ETTERUTBETALING
                UtbetalingtypeDto.FERIEPENGER ->
                    PersonData.UtbetalingData.UtbetalingtypeData.FERIEPENGER
                UtbetalingtypeDto.REVURDERING ->
                    PersonData.UtbetalingData.UtbetalingtypeData.REVURDERING
                UtbetalingtypeDto.UTBETALING ->
                    PersonData.UtbetalingData.UtbetalingtypeData.UTBETALING
            },
        status =
            when (this.tilstand) {
                UtbetalingTilstandDto.ANNULLERT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.ANNULLERT
                UtbetalingTilstandDto.FORKASTET ->
                    PersonData.UtbetalingData.UtbetalingstatusData.FORKASTET
                UtbetalingTilstandDto.GODKJENT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.GODKJENT
                UtbetalingTilstandDto.GODKJENT_UTEN_UTBETALING ->
                    PersonData.UtbetalingData.UtbetalingstatusData.GODKJENT_UTEN_UTBETALING
                UtbetalingTilstandDto.IKKE_GODKJENT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.IKKE_GODKJENT
                UtbetalingTilstandDto.IKKE_UTBETALT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.IKKE_UTBETALT
                UtbetalingTilstandDto.NY -> PersonData.UtbetalingData.UtbetalingstatusData.NY
                UtbetalingTilstandDto.OVERFØRT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.OVERFØRT
                UtbetalingTilstandDto.UTBETALT ->
                    PersonData.UtbetalingData.UtbetalingstatusData.UTBETALT
            },
...
private fun UtbetalingsdagData.kanUtvidesMed(other: UtbetalingsdagData): Boolean {
    // alle verdier må være like (untatt datoene)
    val utenDatoer = { dag: UtbetalingsdagData ->
        ...
        type =
            when (this) {
                is UtbetalingsdagUtDto.ArbeidsdagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.Arbeidsdag
                ...
                is UtbetalingsdagUtDto.ArbeidsgiverperiodeDagNavDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.ArbeidsgiverperiodedagNav
                is UtbetalingsdagUtDto.AvvistDagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.AvvistDag
                is UtbetalingsdagUtDto.ForeldetDagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.ForeldetDag
                is UtbetalingsdagUtDto.FridagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.Fridag
                is UtbetalingsdagUtDto.NavDagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.NavDag
                is UtbetalingsdagUtDto.NavHelgDagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.NavHelgDag
                is UtbetalingsdagUtDto.UkjentDagDto ->
                    PersonData.UtbetalingstidslinjeData.TypeData.UkjentDag
            },
...
private fun BegrunnelseDto.tilPersonData() =
    when (this) {
        BegrunnelseDto.AndreYtelserAap ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserAap
        BegrunnelseDto.AndreYtelserDagpenger ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserDagpenger
        BegrunnelseDto.AndreYtelserForeldrepenger ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserForeldrepenger
        BegrunnelseDto.AndreYtelserOmsorgspenger ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserOmsorgspenger
        BegrunnelseDto.AndreYtelserOpplaringspenger ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserOpplaringspenger
        BegrunnelseDto.AndreYtelserPleiepenger ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.AndreYtelserPleiepenger
        ...
        BegrunnelseDto.EgenmeldingUtenforArbeidsgiverperiode ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData
                .EgenmeldingUtenforArbeidsgiverperiode
        BegrunnelseDto.EtterDødsdato ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.EtterDødsdato
        BegrunnelseDto.ManglerMedlemskap ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.ManglerMedlemskap
        BegrunnelseDto.ManglerOpptjening ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.ManglerOpptjening
        BegrunnelseDto.MinimumInntekt ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.MinimumInntekt
        BegrunnelseDto.MinimumInntektOver67 ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.MinimumInntektOver67
        BegrunnelseDto.MinimumSykdomsgrad ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.MinimumSykdomsgrad
        BegrunnelseDto.NyVilkårsprøvingNødvendig ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.NyVilkårsprøvingNødvendig
        BegrunnelseDto.Over70 -> PersonData.UtbetalingstidslinjeData.BegrunnelseData.Over70
        BegrunnelseDto.SykepengedagerOppbrukt ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.SykepengedagerOppbrukt
        BegrunnelseDto.SykepengedagerOppbruktOver67 ->
            PersonData.UtbetalingstidslinjeData.BegrunnelseData.SykepengedagerOppbruktOver67
    }

private fun UtbetalingVurderingDto.tilPersonData() =
    PersonData.UtbetalingData.VurderingData(
        godkjent = godkjent,
        ident = ident,
        epost = epost,
        tidspunkt = tidspunkt,
        ...
        status =
            when (this.status) {
                ...
                OppdragstatusDto.AKSEPTERT_MED_FEIL ->
                    PersonData.OppdragData.OppdragstatusData.AKSEPTERT_MED_FEIL
...
private fun SimuleringResultatDto.tilPersonData() =
    PersonData.ArbeidsgiverData.VedtaksperiodeData.DataForSimuleringData(
...
private fun FeriepengeUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.FeriepengeutbetalingData(
...
private fun UtbetaltDagDto.tilPersonData() =
    PersonData.ArbeidsgiverData.FeriepengeutbetalingData.UtbetaltDagData(
        type =
            when (this) {
...
private fun InfotrygdFerieperiodeDto.tilPersonData() =
    PersonData.InfotrygdhistorikkElementData.FerieperiodeData(
        fom = this.periode.fom,
        tom = this.periode.tom,
    )

private fun InfotrygdArbeidsgiverutbetalingsperiodeUtDto.tilPersonData() =
    PersonData.InfotrygdhistorikkElementData.ArbeidsgiverutbetalingsperiodeData(
        orgnr = this.orgnr,
        fom = this.periode.fom,
        tom = this.periode.tom,
        grad = this.grad.prosent,
...
private fun InfotrygdPersonutbetalingsperiodeUtDto.tilPersonData() =
    PersonData.InfotrygdhistorikkElementData.PersonutbetalingsperiodeData(
        orgnr = this.orgnr,
        fom = this.periode.fom,
        tom = this.periode.tom,
        grad = this.grad.prosent,
...
private fun InfotrygdInntektsopplysningUtDto.tilPersonData() =
    PersonData.InfotrygdhistorikkElementData.InntektsopplysningData(
        ...
        type =
            when (this) {
                is VilkårsgrunnlagUtDto.Infotrygd ->
                    PersonData.VilkårsgrunnlagElementData.GrunnlagsdataType.Infotrygd
                is VilkårsgrunnlagUtDto.Spleis ->
                    PersonData.VilkårsgrunnlagElementData.GrunnlagsdataType.Vilkårsprøving
            },
                ...
                is VilkårsgrunnlagUtDto.Spleis ->
                    when (this.medlemskapstatus) {
                        ...
                        MedlemskapsvurderingDto.UavklartMedBrukerspørsmål ->
                            JsonMedlemskapstatus.UAVKLART_MED_BRUKERSPØRSMÅL
...
private fun OpptjeningUtDto.tilPersonData() =
    PersonData.VilkårsgrunnlagElementData.OpptjeningData(
...
private fun InntektsgrunnlagUtDto.tilPersonData() =
    PersonData.VilkårsgrunnlagElementData.InntektsgrunnlagData(
...
private fun NyInntektUnderveisDto.tilPersonData() =
    PersonData.VilkårsgrunnlagElementData.NyInntektUnderveisData(
        orgnummer = this.orgnummer,
        beløpstidslinje = this.beløpstidslinje.tilPersonData(),
    )

private fun InntektsopplysningUtDto.tilPersonData() =
    PersonData.VilkårsgrunnlagElementData.ArbeidsgiverInntektsopplysningData.InntektsopplysningData(
        id = this.id,
        dato = this.dato,
        hendelseId = this.hendelseId,
        beløp =
            when (this) {
                ...
                is InntektsopplysningUtDto.SkjønnsmessigFastsattDto ->
                    this.beløp.månedligDouble.beløp
            },
        kilde =
            when (this) {
...
private fun RefusjonsopplysningUtDto.tilPersonData() =
    PersonData.ArbeidsgiverData.RefusjonsopplysningData(
        meldingsreferanseId = this.meldingsreferanseId,
        fom = this.fom,
        tom = this.tom,
        beløp = this.beløp.månedligDouble.beløp,
        ...
        type =
            when (this.type) {
                ...
                InntekttypeDto.PENSJON_ELLER_TRYGD ->
                    SkatteopplysningData.InntekttypeData.PENSJON_ELLER_TRYGD
                InntekttypeDto.YTELSE_FRA_OFFENTLIGE ->
                    SkatteopplysningData.InntekttypeData.YTELSE_FRA_OFFENTLIGE
            },
        fordel = fordel,
...
private fun BeløpstidslinjeDto.tilPersonData() =
    PersonData.BeløpstidslinjeData(
        perioder =
            this.perioder.map {
...
private fun RefusjonsservitørDto.tilPersonData() =
    refusjonstidslinjer.mapValues { (_, beløpstidslinje) -> beløpstidslinje.tilPersonData() }
