...
internal class V282HengendeRevurderinger : JsonMigration(282) {
    ...
    private fun migrerVedtaksperiode(aktørId: String, periode: JsonNode) {
        ...
        if (tilstandForVedtaksperiode !in setOf(
        ...
        val åpneGenerasjoner = generasjoner.filter {
        ...
        if (åpneGenerasjoner.isEmpty()) {
            sikkerlogg.error(
                "[V282] $aktørId Perioden ${
                    periode.path("fom").asText()
                } - ${periode.path("tom").asText()} (${
                    periode.path("id").asText()
                }) mangler åpen generasjon! Perioden står i $tilstandForVedtaksperiode og siste generasjon er $tilstandForSisteGenerasjon"
            )
            return
        }
        ...
        sikkerlogg.info(
            "[V282] $aktørId Fjerner ${åpneGenerasjoner.size - 1} åpne generasjoner for ${
                periode.path(
                    "fom"
                ).asText()
            } - ${periode.path("tom").asText()} (${
                periode.path("id").asText()
            }) i $tilstandForVedtaksperiode"
        )
