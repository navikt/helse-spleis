...
internal class V282HengendeRevurderinger : JsonMigration(282) {
    override val description =
        "fjerner hengende uberegnede revurderinger på perioder som også har en beregnet revurdering"
    ...
    private fun migrer(jsonNode: ObjectNode, aktørId: String) {
        jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
            arbeidsgiver.path("vedtaksperioder").onEach { periode ->
                migrerVedtaksperiode(aktørId, periode)
            }
        }
    }
    ...
    private fun migrerVedtaksperiode(aktørId: String, periode: JsonNode) {
        ...
        val åpneGenerasjoner =
            generasjoner.filter {
                it.path("tilstand").asText() in
                    setOf("UBEREGNET_REVURDERING", "BEREGNET_REVURDERING")
            }
        if (åpneGenerasjoner.size == 1) return // alt ok
        if (åpneGenerasjoner.isEmpty()) {
            sikkerlogg.error(
                "[V282] $aktørId Perioden ${periode.path("fom").asText()} - ${periode.path("tom").asText()} (${periode.path("id").asText()}) mangler åpen generasjon! Perioden står i $tilstandForVedtaksperiode og siste generasjon er $tilstandForSisteGenerasjon"
            )
            return
        }
        ...
        sikkerlogg.info(
            "[V282] $aktørId Fjerner ${åpneGenerasjoner.size - 1} åpne generasjoner for ${periode.path("fom").asText()} - ${periode.path("tom").asText()} (${periode.path("id").asText()}) i $tilstandForVedtaksperiode"
        )
        generasjoner.removeAll { generasjon -> generasjon.path("id").asText().uuid in fjerne }
    }
