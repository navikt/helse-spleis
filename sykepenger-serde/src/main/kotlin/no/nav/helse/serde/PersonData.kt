...
data class PersonData(
    ...
    fun tilPersonDto() =
        PersonInnDto(
            ...
            infotrygdhistorikk =
                InfotrygdhistorikkInnDto(this.infotrygdhistorikk.map { it.tilDto() }),
            vilkårsgrunnlagHistorikk =
                VilkårsgrunnlaghistorikkInnDto(vilkårsgrunnlagHistorikk.map { it.tilDto() }),
            minimumSykdomsgradVurdering =
                MinimumSykdomsgradVurderingInnDto(
                    perioder =
                        minimumSykdomsgradVurdering?.map { PeriodeDto(it.fom, it.tom) }
                            ?: emptyList()
                ),
        )
    ...
    data class InfotrygdhistorikkElementData(
        ...
        fun tilDto() =
            InfotrygdhistorikkelementInnDto(
                ...
                arbeidsgiverutbetalingsperioder =
                    this.arbeidsgiverutbetalingsperioder.map { it.tilDto() },
        ...
        data class FerieperiodeData(val fom: LocalDate, val tom: LocalDate) {
            fun tilDto() =
                InfotrygdFerieperiodeDto(periode = PeriodeDto(fom = this.fom, tom = this.tom))
        }
    ...
    data class VilkårsgrunnlagElementData(
        ...
        data class InntektsgrunnlagData(
            ...
            val tilkommendeInntekter:
                List<NyInntektUnderveisData>?, // todo: migrere inn tom liste for å unngå null
            val vurdertInfotrygd: Boolean,
        ) {
            fun tilSpleisDto() =
                InntektsgrunnlagInnDto(
                    arbeidsgiverInntektsopplysninger =
                        this.arbeidsgiverInntektsopplysninger.map { it.tilDto() },
                    deaktiverteArbeidsforhold = this.deaktiverteArbeidsforhold.map { it.tilDto() },
                    tilkommendeInntekter =
                        this.tilkommendeInntekter?.map { it.tilDto() } ?: emptyList(),
            ...
            fun tilDto() =
                ArbeidsgiverInntektsopplysningInnDto(
                    ...
                    refusjonsopplysninger =
                        RefusjonsopplysningerInnDto(
                            opplysninger = this.refusjonsopplysninger.map { it.tilDto() }
                        ),
                )
                    ...
                    SkatteopplysningDto(
                        ...
                        type =
                            when (type) {
                                ...
                                InntekttypeData.PENSJON_ELLER_TRYGD ->
                                    InntekttypeDto.PENSJON_ELLER_TRYGD
                                InntekttypeData.YTELSE_FRA_OFFENTLIGE ->
                            ...
                            InntektsopplysningInnDto.InntektsmeldingDto(
                                ...
                                kilde =
                                    this.inntektsmeldingkilde?.let {
                                        when (it) {
                                            InntektsmeldingKildeDto.Arbeidsgiver ->
                                                InntektsopplysningInnDto.InntektsmeldingDto.KildeDto
                                                    .Arbeidsgiver
                                        ...
                                        ?: InntektsopplysningInnDto.InntektsmeldingDto.KildeDto
                                            .Arbeidsgiver, // todo: denne trenger ikke være nullable
                                                           // etter 20. oktober 2024..
                                tidsstempel = this.tidsstempel,
                            )
                        ...
                        Inntektsopplysningskilde.SKATT_SYKEPENGEGRUNNLAG ->
                            InntektsopplysningInnDto.SkattSykepengegrunnlagDto(
                                ...
                                inntektsopplysninger =
                                    this.skatteopplysninger!!.map { it.tilDto() },
        ...
        data class OpptjeningData(
            ...
            fun tilDto() =
                OpptjeningInnDto(
                    ...
                    opptjeningsperiode =
                        PeriodeDto(fom = this.opptjeningFom, tom = this.opptjeningTom),
                )
    ...
    data class ArbeidsgiverData(
        ...
        fun tilDto() =
            ArbeidsgiverInnDto(
                ...
                inntektshistorikk =
                    InntektshistorikkInnDto(this.inntektshistorikk.map { it.tilDto() }),
                sykdomshistorikk = SykdomshistorikkDto(this.sykdomshistorikk.map { it.tilDto() }),
                sykmeldingsperioder =
                    SykmeldingsperioderDto(this.sykmeldingsperioder.map { it.tilDto() }),
                ...
                refusjonshistorikk =
                    RefusjonshistorikkInnDto(this.refusjonshistorikk.map { it.tilDto() }),
                ubrukteRefusjonsopplysninger =
                    RefusjonsservitørDto(
                        this.ubrukteRefusjonsopplysninger.mapValues { (_, beløpstidslinje) ->
                            beløpstidslinje.tilDto()
                        }
                    ),
            )
        ...
        data class InntektsmeldingData(
            ...
            val beløp: Double,
            val kilde:
                InntektsmeldingKildeDto?, // todo: denne trenger ikke være nullable etter 20.
                                          // oktober 2024..
            val tidsstempel: LocalDateTime,
        ) {
            fun tilDto() =
                            ...
                            when (it) {
                                InntektsmeldingKildeDto.Arbeidsgiver ->
                                    InntektsopplysningInnDto.InntektsmeldingDto.KildeDto
                                        .Arbeidsgiver
                                InntektsmeldingKildeDto.AOrdningen ->
                                    InntektsopplysningInnDto.InntektsmeldingDto.KildeDto.AOrdningen
                            }
        ...
        data class SykdomstidslinjeData(
            ...
            data class DagData(
                ...
                private fun tilDto(dagen: LocalDate, kilde: HendelseskildeDto) =
                    when (type) {
                        JsonDagType.ARBEIDSDAG ->
                            SykdomstidslinjeDagDto.ArbeidsdagDto(dato = dagen, kilde = kilde)
                        JsonDagType.ARBEIDSGIVERDAG ->
                            if (dagen.erHelg())
                                SykdomstidslinjeDagDto.ArbeidsgiverHelgedagDto(
                                    dato = dagen,
                                    kilde = kilde,
                                    grad = ProsentdelDto(grad),
                                )
                            else
                                SykdomstidslinjeDagDto.ArbeidsgiverdagDto(
                                    dato = dagen,
                                    kilde = kilde,
                                    grad = ProsentdelDto(grad),
                                )
                        JsonDagType.FERIEDAG ->
                            SykdomstidslinjeDagDto.FeriedagDto(dato = dagen, kilde = kilde)
                        JsonDagType.ARBEID_IKKE_GJENOPPTATT_DAG ->
                            SykdomstidslinjeDagDto.ArbeidIkkeGjenopptattDagDto(
                                dato = dagen,
                                kilde = kilde,
                            )
                        JsonDagType.FRISK_HELGEDAG ->
                            SykdomstidslinjeDagDto.FriskHelgedagDto(dato = dagen, kilde = kilde)
                        JsonDagType.FORELDET_SYKEDAG ->
                            SykdomstidslinjeDagDto.ForeldetSykedagDto(
                                dato = dagen,
                                kilde = kilde,
                                grad = ProsentdelDto(grad),
                            )
                        JsonDagType.PERMISJONSDAG ->
                            SykdomstidslinjeDagDto.PermisjonsdagDto(dato = dagen, kilde = kilde)
                        JsonDagType.PROBLEMDAG ->
                            SykdomstidslinjeDagDto.ProblemDagDto(
                                dato = dagen,
                                kilde = kilde,
                                other = this.other!!.tilDto(),
                                melding = this.melding!!,
                            )
                        JsonDagType.SYKEDAG ->
                            if (dagen.erHelg())
                                SykdomstidslinjeDagDto.SykHelgedagDto(
                                    dato = dagen,
                                    kilde = kilde,
                                    grad = ProsentdelDto(grad),
                                )
                            else
                                SykdomstidslinjeDagDto.SykedagDto(
                                    dato = dagen,
                                    kilde = kilde,
                                    grad = ProsentdelDto(grad),
                                )
                        JsonDagType.SYKEDAG_NAV ->
                            SykdomstidslinjeDagDto.SykedagNavDto(
                                dato = dagen,
                                kilde = kilde,
                                grad = ProsentdelDto(grad),
                            )
                        JsonDagType.ANDRE_YTELSER_FORELDREPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                SykdomstidslinjeDagDto.AndreYtelserDto.YtelseDto.Foreldrepenger,
                            )
                        JsonDagType.ANDRE_YTELSER_AAP ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse = SykdomstidslinjeDagDto.AndreYtelserDto.YtelseDto.AAP,
                            )
                        JsonDagType.ANDRE_YTELSER_OMSORGSPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse =
                        ...
                        JsonDagType.ANDRE_YTELSER_PLEIEPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse =
                        ...
                        JsonDagType.ANDRE_YTELSER_SVANGERSKAPSPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse =
                                    SykdomstidslinjeDagDto.AndreYtelserDto.YtelseDto
                                        .Svangerskapspenger,
                            )
                        JsonDagType.ANDRE_YTELSER_OPPLÆRINGSPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse =
                        ...
                        JsonDagType.ANDRE_YTELSER_DAGPENGER ->
                            SykdomstidslinjeDagDto.AndreYtelserDto(
                                dato = dagen,
                                kilde = kilde,
                                ytelse = SykdomstidslinjeDagDto.AndreYtelserDto.YtelseDto.Dagpenger,
                            )
                        JsonDagType.UKJENT_DAG ->
                            SykdomstidslinjeDagDto.UkjentDagDto(dato = dagen, kilde = kilde)
                    }
            }
        ...
        data class ForkastetVedtaksperiodeData(val vedtaksperiode: VedtaksperiodeData) {
            fun tilDto() =
                ForkastetVedtaksperiodeInnDto(vedtaksperiode = this.vedtaksperiode.tilDto())
        }
            ...
            fun tilDto() =
                FeriepengeInnDto(
                    ...
                    infotrygdFeriepengebeløpArbeidsgiver =
                        this.infotrygdFeriepengebeløpArbeidsgiver,
        ...
        data class VedtaksperiodeData(
            ...
            fun tilDto() =
                VedtaksperiodeInnDto(
                    ...
                    tilstand =
                        when (tilstand) {
                            TilstandTypeData.AVVENTER_HISTORIKK ->
                            ...
                            TilstandTypeData.TIL_UTBETALING ->
                                VedtaksperiodetilstandDto.TIL_UTBETALING
                            TilstandTypeData.TIL_INFOTRYGD ->
                            ...
                            TilstandTypeData.REVURDERING_FEILET ->
            ...
            data class MaksdatoresultatData(
                ...
                fun tilDto() =
                    MaksdatoresultatInnDto(
                        ...
                        bestemmelse =
                            when (bestemmelse) {
                                MaksdatobestemmelseData.IKKE_VURDERT ->
                                    MaksdatobestemmelseDto.IKKE_VURDERT
                                MaksdatobestemmelseData.ORDINÆR_RETT ->
                        ...
                        forbrukteDager =
                            forbrukteDager.map { PeriodeDto(fom = it.fom, tom = it.tom) },
                        oppholdsdager =
                            oppholdsdager.map { PeriodeDto(fom = it.fom, tom = it.tom) },
                        avslåtteDager =
                            avslåtteDager.map { PeriodeDto(fom = it.fom, tom = it.tom) },
                        maksdato = maksdato,
        ...
        data class RefusjonData(
            ...
            data class EndringIRefusjonData(val beløp: Double, val endringsdato: LocalDate) {
                fun tilDto() =
                    EndringIRefusjonDto(
                        beløp = InntektbeløpDto.MånedligDouble(this.beløp),
            ...
            UtbetalingInnDto(
                ...
                tilstand =
                    when (status) {
                        ...
                        UtbetalingstatusData.GODKJENT_UTEN_UTBETALING ->
            ...
            private val økonomiDto by lazy {
                ØkonomiInnDto(
                    ...
                    arbeidsgiverRefusjonsbeløp =
                        InntektbeløpDto.DagligDouble(this.arbeidsgiverRefusjonsbeløp),
                    ...
                    arbeidsgiverbeløp =
                        this.arbeidsgiverbeløp?.let { InntektbeløpDto.DagligDouble(it) },
            ...
            private fun tilDto(dato: LocalDate): UtbetalingsdagInnDto {
                return when (type) {
                    TypeData.ArbeidsgiverperiodeDag ->
                        UtbetalingsdagInnDto.ArbeidsgiverperiodeDagDto(
                            dato = dato,
                            økonomi = økonomiDto,
                        )
                    TypeData.NavDag ->
                        UtbetalingsdagInnDto.NavDagDto(dato = dato, økonomi = økonomiDto)
                    TypeData.NavHelgDag ->
                        UtbetalingsdagInnDto.NavHelgDagDto(dato = dato, økonomi = økonomiDto)
                    TypeData.Arbeidsdag ->
                        UtbetalingsdagInnDto.ArbeidsdagDto(dato = dato, økonomi = økonomiDto)
                    TypeData.Fridag ->
                        UtbetalingsdagInnDto.FridagDto(dato = dato, økonomi = økonomiDto)
                    TypeData.AvvistDag ->
                        UtbetalingsdagInnDto.AvvistDagDto(
                            dato = dato,
                            økonomi = økonomiDto,
                            begrunnelser = begrunnelser!!.map { it.tilDto() },
                        )
                    TypeData.UkjentDag ->
                        UtbetalingsdagInnDto.UkjentDagDto(dato = dato, økonomi = økonomiDto)
                    TypeData.ForeldetDag ->
                        UtbetalingsdagInnDto.ForeldetDagDto(dato = dato, økonomi = økonomiDto)
