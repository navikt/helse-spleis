...
internal class V303KopiereMaksdatoFraUtbetalingTilBehandling : JsonMigration(version = 303) {
    ...
    override fun doMigration(jsonNode: ObjectNode, meldingerSupplier: MeldingerSupplier) {
        ...
        val infotrygdbetalteDager =
            jsonNode.path("infotrygdhistorikk").map { element ->
                ...
                val arbeidsgiverutbetalinger =
                    element.path("arbeidsgiverutbetalingsperioder").map { periode ->
                        LocalDate.parse(periode.path("fom").asText())..LocalDate.parse(
                                periode.path("tom").asText()
                            )
                    }
        ...
        val utbetalingerPerArbeidsgiver =
            jsonNode.path("arbeidsgivere").associate { arbeidsgiver ->
                val annullerteUtbetalinger =
                    arbeidsgiver
                        .path("utbetalinger")
                        ...
                        .map {
                            ...
                            val avslåtteDager =
                                it.path("utbetalingstidslinje")
                                    .path("dager")
                                    .filter {
                                        it.path("type").asText() == "AvvistDag" &&
                                            it.path("begrunnelser").any {
                                                it.asText() in maksdatobegrunnelser
                                            }
                                    }
                                    .map { dag ->
                            ...
                            Utbetalingmetadata(
                                ...
                                korrelasjonsId =
                                    UUID.fromString(it.path("korrelasjonsId").asText()),
                                beregningstidspunkt =
        ...
        val maksdatoresultater =
            utbetalingerPerArbeidsgiver
                .flatMap { (arbeidsgiver, utbetalinger) ->
                    utbetalinger.map { utbetaling ->
                        // kanskje er det dager utbetalt i Infotrygd?
                        // eller på andre arbeidsgivere?
                        // Vi må danne oss et totalbilde av alt som er forbrukt før utbetalingen ble
                        // beregnet
                        val sisteInfotrygdelementFørBeregning =
                            infotrygdbetalteDager
                                .firstOrNull { it.tidspunkt < utbetaling.beregningstidspunkt }
                                ?.betalteDager
                                ?.flatMap { it.datoer() }
                                ?.toSet() ?: emptySet()

                        val forbrukteDagerAndreArbeidsgivere =
                            utbetalingerPerArbeidsgiver
                                .flatMap { (_, andreUtbetalinger) ->
                                    andreUtbetalinger
                                        .filterNot {
                                            it.korrelasjonsId == utbetaling.korrelasjonsId
                                        }
                                        .groupBy { it.korrelasjonsId }
                                        .flatMap { (_, utbetalinger) ->
                                            utbetalinger
                                                .filter {
                                                    it.beregningstidspunkt <
                                                        utbetaling.beregningstidspunkt
                                                }
                                                .sortedByDescending { it.beregningstidspunkt }
                                                .takeLast(1)
                                                .flatMap {
                                                    it.forbrukteDager.flatMap { it.datoer() }
                                                }
                                        }
                                }
                                .toSet()
                        ...
                        val forbrukteDager =
                            utbetaling.forbrukteDager.flatMap { it.datoer() }.toSet()
                        val totalbildeForbrukteDager =
                            sisteInfotrygdelementFørBeregning +
                                forbrukteDagerAndreArbeidsgivere +
                                forbrukteDager

                        val faktiskForbrukte =
                            totalbildeForbrukteDager
                        ...
                        val bestemmelse =
                            when {
                                // bruker først evt. avslagsbegrunnelser som utgangspunkt for å
                                // bestemme rettighetsbestemmelsen
                                Maksdatobegrunnelse.Over70 in begrunnelerForUtbetalingen ->
                                    Maksdatobestemmelse.SYTTI_ÅR
                                Maksdatobegrunnelse.SykepengedagerOppbruktOver67 in
                                    begrunnelerForUtbetalingen -> Maksdatobestemmelse.BEGRENSET_RETT
                                Maksdatobegrunnelse.SykepengedagerOppbrukt in
                                    begrunnelerForUtbetalingen -> Maksdatobestemmelse.ORDINÆR_RETT
                                // hvis maksdato er syttiårsdagen, da er det avgjort!
                                utbetaling.maksdato == maksdatoSyttiåring ->
                                    Maksdatobestemmelse.SYTTI_ÅR
                                // hvis bruker ikke har gått til maks, men totalt antall
                                // sykepengedager er under 248, da må det være snakk om begrenset
                                (utbetaling.antallGjenståendeDager +
                                    utbetaling.antallForbrukteDager) < 248 ->
                                    Maksdatobestemmelse.BEGRENSET_RETT
                                else -> Maksdatobestemmelse.ORDINÆR_RETT
                            }

                        utbetaling.utbetalingId to
                            Maksdatoresultat(
                                vurdertTilOgMed = utbetaling.utbetalingTom,
                                bestemmelse = bestemmelse,
                                startdatoTreårsvindu =
                                    forbrukteDager.firstOrNull()?.minusYears(3) ?: LocalDate.MIN,
                                startdatoSykepengerettighet = forbrukteDager.firstOrNull(),
                                antallForbrukteDager = utbetaling.antallForbrukteDager,
                                forbrukteDager =
                                    faktiskForbrukte.merge().map {
                                        Datoperiode(it.start, it.endInclusive)
                                    },
                                oppholdsdager = emptyList(),
                                avslåtteDager =
                                    utbetaling.avslåtteDager
    ...
    private fun migrerEndring(
        ...
        val resultat =
            maksdatoresultater[utbetalingId]
    ...
    private data class Maksdatoresultat(
        ...
        // alle NavDag-er på utbetalingstidslinjene fra utbetalingene (eller en
        // Infotrygd-utbetalt-dag)
        // kun ukedager
        val antallForbrukteDager: Int,
    ...
    */
    ...
    private companion object {
        ...
        private fun JsonNode.somPeriode() =
            if (this.hasNonNull("dato")) LocalDate.parse(this.path("dato").asText()).let { it..it }
            else
        ...
        private fun Iterable<LocalDate>.merge(): List<ClosedRange<LocalDate>> {
            ...
            return sortert.fold(emptyList<ClosedRange<LocalDate>>()) { resultat, dagen ->
                ...
                when {
                    ...
                    last.endInclusive.plusDays(1) == dagen ->
                        resultat.dropLast(1).plusElement(last.start..dagen)
        ...
        private fun LocalDate.forrigeVirkedagFør() =
            minusDays(
                when (dayOfWeek) {
                    SUNDAY -> 2
                    MONDAY -> 3
