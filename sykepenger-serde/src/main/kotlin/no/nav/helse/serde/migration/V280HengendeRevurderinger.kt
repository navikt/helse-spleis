...
internal class V280HengendeRevurderinger : JsonMigration(280) {
    override val description =
        "fjerner hengende uberegnede revurderinger på eldre perioder som er avsluttet"
    ...
    private fun migrerVedtaksperiode(aktørId: String, periode: JsonNode) {
        ...
        val sisteIverksettingIndex =
            generasjoner.indexOfLast { it.path("tilstand").asText() == "VEDTAK_IVERKSATT" }
        ...
        if (sisteIverksetting !is ObjectNode || sisteIverksettingIndex < 0) {
            sikkerlogg.error(
                "[280] $aktørId Skulle flytte uberegnet revurdering til VEDTAK_IVERKSATT for ${
                    periode.path(
                        "fom"
                    ).asText()
                } - ${periode.path("tom").asText()} (${
                    periode.path("id").asText()
                }) fordi vedtaksperioden er i $tilstandForVedtaksperiode og siste generasjon har tilstand $tilstandForSisteGenerasjon, men vedtaksperioden har ikke en generasjon i VEDTAK_IVERKSATT"
            )
            return
        }
        ...
        val dokumentsporing = uberegnede
            .flatMap {
                it.path("endringer").map {
                    LocalDateTime.parse(
                        it.path("tidsstempel").asText()
                    ) to it.path("dokumentsporing").dokumentsporing
                }
            }.toSet()

        sikkerlogg.info(
            "[280] $aktørId Flytter ${uberegnede.size} generasjoner (${
                uberegnede.joinToString {
                    it.path(
                        "tilstand"
                    ).asText()
                }
            }) til VEDTAK_IVERKSATT (${sisteIverksetting.path("id").asText()}) for ${
                periode.path("fom").asText()
            } - ${periode.path("tom").asText()} (${
                periode.path("id").asText()
            }) fordi vedtaksperioden er i $tilstandForVedtaksperiode"
        )
