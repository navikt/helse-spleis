            ...
            uberegnede
                .flatMap {
                    it.path("endringer").map {
                        LocalDateTime.parse(it.path("tidsstempel").asText()) to
                            it.path("dokumentsporing").dokumentsporing
                    }
                }
                .toSet()

        sikkerlogg.info(
            "[280] $aktørId Flytter ${uberegnede.size} generasjoner (${uberegnede.joinToString { it.path("tilstand").asText() }}) til VEDTAK_IVERKSATT (${sisteIverksetting.path("id").asText()}) for ${periode.path("fom").asText()} - ${periode.path("tom").asText()} (${periode.path("id").asText()}) fordi vedtaksperioden er i $tilstandForVedtaksperiode"
        )
