...
internal class V292AnnullertPeriode : JsonMigration(version = 292) {
        ...
        MDC.putCloseable("aktørId", aktørId).use {
            jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
                ...
                MDC.putCloseable("orgnr", orgnr).use {
                    val opphørtAvAndreUtbetalinger =
                        arbeidsgiver
                            .path("utbetalinger")
                            .flatMap { utbetaling ->
                            ...
                            .toSet()
                    val utbetalinger =
                        arbeidsgiver.path("utbetalinger").associateBy { it.path("id").asText() }

                    val annulleringer =
                        arbeidsgiver
                            .path("utbetalinger")
                            .filter { utbetaling ->
                                utbetaling.path("type").asText() == "ANNULLERING" &&
                                    utbetaling.path("status").asText() == "ANNULLERT"
                            }
                            .filterNot { annullering ->
                                annullering.path("id").asText() in opphørtAvAndreUtbetalinger
                            }

                    arbeidsgiver.path("forkastede").forEach { forkastet ->
                        val vedtaksperiode = forkastet.path("vedtaksperiode")
                        MDC.putCloseable("vedtaksperiodeId", vedtaksperiode.path("id").asText())
                            .use {
    ...
    private fun migrerVedtaksperiode(
        utbetalinger: Map<String, JsonNode>,
        annulleringer: List<JsonNode>,
        vedtaksperiode: JsonNode,
    ) {
        val generasjoner = vedtaksperiode.path("generasjoner") as ArrayNode
        val siste = generasjoner.last()

        if (siste.path("tilstand").asText() != "TIL_INFOTRYGD")
            return sikkerLogg.info(
                "[V292] siste generasjon ${siste.path("id").asText()} som står i ${siste.path("tilstand").asText()} og det er forventet at den skal være TIL_INFOTRYGD"
            )
        ...
        val utbetalingId =
            sisteEndring.path("utbetalingId")?.takeIf(JsonNode::isTextual)?.asText()
                ?: return sikkerLogg.info(
                    "[V292] nest siste generasjon ${siste.path("id").asText()} har ingen utbetaling ?!"
                )
        val utbetalingen = utbetalinger.getValue(utbetalingId)

        val tilhørendeAnnullering =
            annulleringer.firstOrNull { annullering ->
                annullering.path("korrelasjonsId").asText() ==
                    utbetalingen.path("korrelasjonsId").asText()
            }
                ?: return sikkerLogg.info(
                    "[V292] fant ikke en tilhørende annullering for utbetaling $utbetalingId ?!"
                )

        sikkerLogg.info(
            "[V292] endrer tilstand til ANNULLERT_PERIODE for generasjon ${siste.path("id").asText()}"
        )
        siste as ObjectNode
        ...
        kopi.path("vilkårsgrunnlagId").takeIf(JsonNode::isTextual)?.asText()?.also {
            vilkårsgrunnlagId ->
