...
internal class V292AnnullertPeriode : JsonMigration(version = 292) {
            ...
            jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
                ...
                MDC.putCloseable("orgnr", orgnr).use {
                    val opphørtAvAndreUtbetalinger =
                        arbeidsgiver.path("utbetalinger").flatMap { utbetaling ->
                    ...
                    val annulleringer = arbeidsgiver.path("utbetalinger")
                        .filter { utbetaling ->
                            utbetaling.path("type")
                                .asText() == "ANNULLERING" && utbetaling.path("status")
                                .asText() == "ANNULLERT"
                        }
    ...
    private fun migrerVedtaksperiode(
        ...
        if (siste.path("tilstand").asText() != "TIL_INFOTRYGD") return sikkerLogg.info(
            "[V292] siste generasjon ${
                siste.path(
                    "id"
                ).asText()
            } som står i ${
                siste.path("tilstand").asText()
            } og det er forventet at den skal være TIL_INFOTRYGD"
        )
        ...
        kopi.path("vilkårsgrunnlagId").takeIf(JsonNode::isTextual)?.asText()
            ?.also { vilkårsgrunnlagId ->
