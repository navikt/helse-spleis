    ...
    override fun doMigration(jsonNode: ObjectNode, meldingerSupplier: MeldingerSupplier) {
        ...
        jsonNode.path("vilkårsgrunnlagHistorikk").forEach { historikkinnslag ->
            historikkinnslag.path("vilkårsgrunnlag").forEach { vilkårsgrunnlag ->
                vilkårsgrunnlag.path("inntektsgrunnlag").path("deaktiverteArbeidsforhold")
                    .forEach { arbeidsgiverInntektsopplysning ->
                        val fallbackTidsstempel = LocalDateTime.parse(
                            arbeidsgiverInntektsopplysning.path("inntektsopplysning")
                                .path("tidsstempel").asText()
                        )
                        arbeidsgiverInntektsopplysning.path("refusjonsopplysninger")
                            .filter { refusjonsopplysning ->
                                refusjonsopplysning.path("tidsstempel").isMissingNode || refusjonsopplysning.path(
                                    "tidsstempel"
                                ).isNull
                            }
                            .forEach { refusjonsopplysning ->
                                refusjonsopplysning as ObjectNode
                                val meldingsreferanseId = UUID.fromString(
