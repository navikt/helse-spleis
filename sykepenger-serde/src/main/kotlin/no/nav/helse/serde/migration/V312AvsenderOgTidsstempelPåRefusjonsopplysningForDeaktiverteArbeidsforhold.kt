...
internal class V312AvsenderOgTidsstempelPåRefusjonsopplysningForDeaktiverteArbeidsforhold :
    JsonMigration(version = 312) {
    override val description =
        "legger til avsender og tidsstempel på refusjonsopplysning i deaktiverte arbedisforhold"
            ...
            historikkinnslag.path("vilkårsgrunnlag").forEach { vilkårsgrunnlag ->
                vilkårsgrunnlag
                    ...
                    .forEach { arbeidsgiverInntektsopplysning ->
                        val fallbackTidsstempel =
                            LocalDateTime.parse(
                                arbeidsgiverInntektsopplysning
                                    .path("inntektsopplysning")
                                    .path("tidsstempel")
                                    .asText()
                            )
                        arbeidsgiverInntektsopplysning
                            .path("refusjonsopplysninger")
                            .filter { refusjonsopplysning ->
                                refusjonsopplysning.path("tidsstempel").isMissingNode ||
                                    refusjonsopplysning.path("tidsstempel").isNull
                            }
                            .forEach { refusjonsopplysning ->
                                refusjonsopplysning as ObjectNode
                                val meldingsreferanseId =
                                    UUID.fromString(
                                        refusjonsopplysning.path("meldingsreferanseId").asText()
                                    )
                                ...
                                if (hendelse == null) {
                                    ...
                                    sikkerlogg.info(
                                        "Fant ikke melding med meldingsreferanse $meldingsreferanseId for person $fnr. Defaulter til ARBEIDSGIVER og tidsstempel for inntektsopplysning"
                                    )
                                } else {
                                    refusjonsopplysning.put(
                                        "avsender",
                                        hendelse.meldingstype.avsender,
                                    )
                ...
                when (this) {
                    ...
                    "GJENOPPLIV_VILKÅRSGRUNNLAG" -> "SAKSBEHANDLER"
                        ...
                        "ARBEIDSGIVER"
                            .also {
                                sikkerlogg.info(
                                    "Mappet meldingstype $this til avsender ARBEIDSGIVER"
                                )
                            }
                }
