...
internal class V285LoggeRareAnnulleringer(
    ...
    override fun doMigration(jsonNode: ObjectNode, meldingerSupplier: MeldingerSupplier) {
        ...
        jsonNode.path("arbeidsgivere").forEach { arbeidsgiver ->
            ...
            val utbetalteKorrelasjonsIder =
                arbeidsgiver
                    .path("utbetalinger")
                    .groupBy { it.path("korrelasjonsId").asText() }
                    .mapValues { (_, utbetalinger) ->
                        utbetalinger.filter {
                            it.path("type").asText() == "UTBETALING" &&
                                it.path("status").asText() == "UTBETALT"
                        }
                    }
                    .filterValues { it.isNotEmpty() }
                    .keys

            val annullerteKorrelasjonsIder =
                arbeidsgiver
                    .path("utbetalinger")
                    .groupBy { it.path("korrelasjonsId").asText() }
                    .mapValues { (_, utbetalinger) ->
            ...
            arbeidsgiver
                ...
                .filter {
                    it.path("arbeidsgiverOppdrag").oppdragManglerOverføring ||
                        it.path("personOppdrag").oppdragManglerOverføring
                }
                ...
                .forEach { manglerOverføring ->
                    ...
                    val oppdatert =
                        LocalDateTime.parse(manglerOverføring.path("oppdatert").asText())
                    sikkerLogg.warn(
                        "Annullering $id i Status $nåværendeStatus ser ikke ut til å være overført til Oppdrag (sist oppdatert $oppdatert). AktørId $aktørId, Organisasjonsnummer $organisasjonsnummer (Versjon4)"
                    )
                    if (id in forkast) {
                        sikkerLogg.info(
                            "Setter status på Utbetaling $id til FORKASTET og setter nytt oppdatert-tidspunkt"
                        )
