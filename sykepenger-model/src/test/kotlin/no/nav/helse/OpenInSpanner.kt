...
class SpannerEtterTestInterceptor : TestWatcher {
    ...
    private fun lastOppTilSpannerish(json: String, testnavn: String, errorMsg: String?): URI {
        ...
        HttpClient.newHttpClient().use { client ->
            // Må laste opp mot intern.dev pga. en Microsoft-redirect på ansatt.dev. Så må ha
            // naisdevicen på
            val request =
                HttpRequest.newBuilder(URI("https://spannerish.intern.dev.nav.no/api/person/$uuid"))
                    .header("Content-Type", "application/json")
                    .POST(HttpRequest.BodyPublishers.ofString(json))
                    .build()
            val response = client.send(request, HttpResponse.BodyHandlers.ofString())
            check(response.statusCode() == 201) {
                "Det var sprøtt, fikk http status ${response.statusCode()} fra Spannerish. Kanskje du ikke er nais device?"
            }
        }
    ...
    private val Map<String, String?>.queryString
        get() =
            this.filterNot { (_, value) -> value.isNullOrBlank() }
                .entries
    ...
    private fun SugUtAlleAktivitetene(aktivitetslogg: Aktivitetslogg): String {
        ...
        val liste =
            aktivitetslogg.aktiviteter.map {
                when (it) {
                    is Aktivitet.Behov ->
                        AktivitetDto(
                            id = 0,
                            ...
                            kontekster =
                                it.kontekster.associateBy({ it.kontekstType }, { it.kontekstMap }),
                        )
