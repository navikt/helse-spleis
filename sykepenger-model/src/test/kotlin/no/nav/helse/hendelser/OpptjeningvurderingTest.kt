...
internal class OpptjeningvurderingTest {
    ...
    fun `27 dager opptjening gir ikke rett til opptjening`() {
        assertFalse(undersøke(listOf(Arbeidsforhold(ORGNUMMER.toString(), 5.desember(2017)))) {
    ...
    fun `tom eldre enn fom`() {
        assertTrue(undersøke(listOf(
            Arbeidsforhold(ORGNUMMER.toString(), 1.november(2017)),
            Arbeidsforhold(ORGNUMMER.toString(), 10.desember(2017), 1.desember(2017))
        )) {
    ...
    fun `arbeidsforhold nyere enn skjæringstidspunkt`() {
        assertFalse(undersøke(listOf(Arbeidsforhold(ORGNUMMER.toString(), SKJÆRINGSTIDSPUNKT.plusDays(1)))) {
    ...
    fun `arbeidsforhold avsluttet før skjæringstidspunkt`() {
        assertFalse(undersøke(listOf(Arbeidsforhold(ORGNUMMER.toString(), LocalDate.EPOCH, SKJÆRINGSTIDSPUNKT.minusDays(1)))) {
            assertEquals(0, it.antallOpptjeningsdager)
            assertFalse(it.harOpptjening())
        })
    }
    ...
    fun `28 dager opptjening fører til OK opptjening`() {
        assertTrue(undersøke(listOf(Arbeidsforhold(ORGNUMMER.toString(), 4.desember(2017)))) {
    ...
    fun `flere arbeidsforhold i samme bedrift`() {
        assertTrue(undersøke(listOf(
            Arbeidsforhold(ORGNUMMER.toString(), 4.desember(2017)),
            Arbeidsforhold(ORGNUMMER.toString(), 4.desember(2017), 1.januar(2018)),
            Arbeidsforhold(ORGNUMMER.toString(), 1.januar(2018), 1.september(2018))
        )) {
            assertEquals(28, it.antallOpptjeningsdager)
            assertTrue(it.harOpptjening())
        })
