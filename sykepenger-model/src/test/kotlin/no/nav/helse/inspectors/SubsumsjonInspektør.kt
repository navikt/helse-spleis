...
internal class SubsumsjonInspektør(jurist: SubsumsjonsListLog) {
    ...
    private data class Subsumsjon(
        ...
        fun vedtaksperiodeIdFraSporing(): UUID =
            UUID.fromString(sporing.first { it.type == KontekstType.Vedtaksperiode }.verdi)
    }
    ...
    internal fun assertBeregnet(
        ...
        assertEquals(
            forventetAntall,
            resultat.size,
            "Forventer kun en subsumsjon. Subsumsjoner funnet: $resultat",
        )
        ...
        assertEquals(VILKAR_BEREGNET, subsumsjon.utfall) {
            "Forventet oppfylt $paragraf $ledd $punktum"
        }
    ...
    internal fun assertOppfylt(
        ...
        assertEquals(VILKAR_OPPFYLT, subsumsjon.utfall) {
            "Forventet oppfylt $paragraf $ledd $punktum"
        }
    ...
    internal fun assertPaaIndeks(
        ...
        assertEquals(
            forventetAntall,
            resultat.size,
            "Forventer $forventetAntall subsumsjoner for vilkåret. Subsumsjoner funnet: ${resultat.size}",
        )
    ...
    internal fun assertIkkeOppfylt(
        ...
        val subsumsjon = resultat.first()
        assertEquals(VILKAR_IKKE_OPPFYLT, subsumsjon.utfall) {
            "Forventet ikke oppfylt $paragraf $ledd $punktum"
        }
        if (input == null && output == null) return
        assertResultat(input, output, subsumsjon)
    }
            ...
            finnSubsumsjoner(
                ...
                .also {
                    assertEquals(
                        antall,
                        it.size,
                        "Forventer $antall subsumsjoner for vilkåret. Subsumsjoner funnet: $it",
                    )
                }
        resultat.forEach { assertResultat(input, output, it) }
    }
    ...
    internal fun assertIkkeVurdert(
        ...
        assertEquals(
            0,
            resultat.size,
            "Forventer ingen subsumsjoner. Subsumsjoner funnet: $resultat",
        )
    }
