...
internal class TestArbeidsgiverInspektør(
    ...
    internal fun ikkeUtbetalteUtbetalingerForVedtaksperiode(vedtaksperiodeId: UUID) =
        vedtaksperioder(vedtaksperiodeId).inspektør.utbetalinger.filter {
            it.inspektør.erUbetalt
        }
    ...
    private fun <R> Collection<R>.singleOrNullOrThrow() =
        if (size <
            ...
            throw IllegalStateException("Listen inneholder $size elementer: $this")
        }

    internal fun refusjonsopplysningerFraVilkårsgrunnlag(
        skjæringstidspunkt: LocalDate =
            person.vilkårsgrunnlagHistorikk.inspektør.aktiveSpleisSkjæringstidspunkt
                .max(),
    ) = personInspektør.vilkårsgrunnlagHistorikk
        .grunnlagsdata(
            skjæringstidspunkt,
        ).inspektør.inntektsgrunnlag.inspektør.arbeidsgiverInntektsopplysningerPerArbeidsgiver[orgnummer]
        ?.inspektør
