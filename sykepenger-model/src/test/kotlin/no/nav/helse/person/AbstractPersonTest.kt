...
internal abstract class AbstractPersonTest {
    ...
    internal companion object {
        ...
        val a1Hendelsefabrikk = ArbeidsgiverHendelsefabrikk(organisasjonsnummer = ORGNUMMER)
        val a2: String = "654321987"
        val a3: String = "321987654"
        val a4: String = "456789123"

        private fun overgangFraInfotrygdPerson(jurist: Subsumsjonslogg) =
            gjenopprettFraJSON("/personer/infotrygdforlengelse.json", jurist).also { person ->
                person.håndter(
                    Utbetalingshistorikk(
                        ...
                        InfotrygdhistorikkElement.opprett(
                            ...
                            listOf(
                                Inntektsopplysning(ORGNUMMER, 1.januar, TestPerson.INNTEKT, true)
                            ),
                            emptyMap(),
                        ),
        ...
        private fun pingPongPerson(jurist: Subsumsjonslogg) =
            gjenopprettFraJSON("/personer/pingpong.json", jurist).also { person ->
                person.håndter(
                    Utbetalingshistorikk(
                        UUID.randomUUID(),
                        ORGNUMMER,
                        ...
                        InfotrygdhistorikkElement.opprett(
                            ...
                            listOf(
                                Inntektsopplysning(ORGNUMMER, 1.februar, TestPerson.INNTEKT, true)
                            ),
                            emptyMap(),
                        ),
                        besvart = LocalDateTime.now(),
    ...
    val inspektør
        get() = inspektør(ORGNUMMER)

    val Int.vedtaksperiode: IdInnhenter
        get() = IdInnhenter { orgnummer -> this@vedtaksperiode.vedtaksperiodeId(orgnummer) }

    fun Int.vedtaksperiode(orgnummer: String): IdInnhenter = IdInnhenter {
        this@vedtaksperiode.vedtaksperiodeId(orgnummer)
    }

    fun IdInnhenter.filter(orgnummer: String = ORGNUMMER) =
        AktivitetsloggFilter.vedtaksperiode(this, orgnummer)
    ...
    private fun regler(maksSykedager: Int): ArbeidsgiverRegler =
        object : ArbeidsgiverRegler {
            override fun burdeStarteNyArbeidsgiverperiode(oppholdsdagerBrukt: Int) =
                oppholdsdagerBrukt >= 16
    ...
    protected fun createOvergangFraInfotrygdPerson() = createTestPerson { jurist ->
        overgangFraInfotrygdPerson(jurist)
    }
    ...
    private fun Int.vedtaksperiodeId(orgnummer: String) =
    ...
    fun inspektør(orgnummer: String, block: TestArbeidsgiverInspektør.() -> Unit) =
        inspektør(orgnummer).run(block)
}

internal fun AbstractPersonTest.nullstillTilstandsendringer() =
    observatør.nullstillTilstandsendringer()
