...
internal class VilkårsgrunnlagHistorikkTest {
    ...
    fun `kan overskrive vilkårsgrunnlag`() {
        ...
        val nyMeldingsreferanseId = UUID.randomUUID()
        val nyttVilkårsgrunnlag = Vilkårsgrunnlag(
            meldingsreferanseId = nyMeldingsreferanseId,
            vedtaksperiodeId = UUID.randomUUID().toString(),
            aktørId = "AKTØR_ID",
            personidentifikator = "20043769969".somPersonidentifikator(),
            orgnummer = "ORGNUMMER",
        ...
        vilkårsgrunnlagHistorikk.lagre(nyttVilkårsgrunnlag.grunnlagsdata())
        assertEquals(nyMeldingsreferanseId, vilkårsgrunnlagHistorikk.vilkårsgrunnlagFor(1.januar)?.inspektør?.meldingsreferanseId)
    }
