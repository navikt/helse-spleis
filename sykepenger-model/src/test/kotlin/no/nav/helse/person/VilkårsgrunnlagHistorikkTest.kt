...
internal class VilkårsgrunnlagHistorikkTest {
    ...
    private val inspektør
        get() = Vilkårgrunnlagsinspektør(historikk.view())
    ...
    fun `lager perioder for vilkårsgrunnlagene`() {
        val grunnlagMedSkjæringstidspunkt = { skjæringstidspunkt: LocalDate ->
            ...
            VilkårsgrunnlagHistorikk.Grunnlagsdata(
                ...
                opptjening =
                    Opptjening.nyOpptjening(arbeidsforholdFraHistorikk, skjæringstidspunkt),
    ...
    fun `fjerner vilkårsgrunnlag som ikke gjelder lengre`() {
        ...
        historikk.lagre(
            VilkårsgrunnlagHistorikk.Grunnlagsdata(
                ...
                inntektsgrunnlag = inntekt.inntektsgrunnlag(ORGNR),
                opptjening =
                    Opptjening.nyOpptjening(arbeidsforholdFraHistorikk, gammeltSkjæringstidspunkt),
                medlemskapstatus = Medlemskapsvurdering.Medlemskapstatus.Ja,
                vurdertOk = true,
                meldingsreferanseId = UUID.randomUUID(),
                vilkårsgrunnlagId = UUID.randomUUID(),
        ...
        assertEquals(0, historikk.inspektør.vilkårsgrunnlagTeller[0]) {
            "det siste innslaget skal være tomt"
        }
        assertEquals(1, historikk.inspektør.vilkårsgrunnlagTeller[1])
        assertNull(historikk.vilkårsgrunnlagFor(gammeltSkjæringstidspunkt)) {
            "skal ikke beholde vilkårsgrunnlag for skjæringstidspunkter som ikke finnes"
        }
        assertNull(historikk.vilkårsgrunnlagFor(nyttSkjæringstidspunkt)) {
            "skal ikke ha vilkårsgrunnlag for skjæringstidspunkt som ikke er vilkårsprøvd"
        }
    }
        ...
        val vilkårsgrunnlag =
            Vilkårsgrunnlag(
                ...
                inntekterForOpptjeningsvurdering =
                    lagStandardInntekterForOpptjeningsvurdering("ORGNUMMER", INGEN, 1.januar),
    ...
    fun `Registrerer subsumsjoner ved validering av vilkårsgrunnlag`() {
        ...
        SubsumsjonInspektør(subsumsjonslogg)
            .assertVurdert(paragraf = PARAGRAF_8_2, ledd = 1.ledd, versjon = 12.juni(2020))
    }
    ...
    fun `Avslår vilkår for minimum inntekt med riktig begrunnelse for personer under 67 år`() {
        ...
        vilkårsgrunnlag.valider(Aktivitetslogg(), 10.månedlig.sykepengegrunnlag, jurist)
        vilkårsgrunnlagHistorikk.lagre(vilkårsgrunnlag.grunnlagsdata())
        assertNotNull(vilkårsgrunnlagHistorikk.vilkårsgrunnlagFor(1.januar))
        val grunnlagsdataInspektør =
            GrunnlagsdataInspektør(vilkårsgrunnlagHistorikk.vilkårsgrunnlagFor(1.januar)!!.view())
