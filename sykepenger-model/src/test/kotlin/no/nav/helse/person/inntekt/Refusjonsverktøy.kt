...
internal fun assertLikeRefusjonsopplysninger(
    ...
    assertEquals(
        expected.map { it.copy(tidsstempel = LocalDate.EPOCH.atStartOfDay()) },
        actual.map { it.copy(tidsstempel = LocalDate.EPOCH.atStartOfDay()) },
    )
}
...
internal fun assertLikRefusjonsopplysning(
    ...
    assertEquals(
        expected.copy(tidsstempel = LocalDate.EPOCH.atStartOfDay()),
        actual.copy(tidsstempel = LocalDate.EPOCH.atStartOfDay()),
    )
}
