...
internal class SpesifikkKontekstTest {
    ...
    fun `to string`() {
        assertEquals(
            "Person fødselsnummer: 1 aktørId: 2",
            SpesifikkKontekst("Person", mapOf("fødselsnummer" to "1", "aktørId" to "2")).melding(),
        )
    }
    ...
    fun `samme type og like kontekster er like`() {
        val ytelserKontekst =
            SpesifikkKontekst(
                ...
                mapOf("fødselsnummer" to "1", "aktørId" to "2"),
            )
        val utbetalingshistorikkKontekst =
            SpesifikkKontekst(
                "Utbetalingshistorikk",
                mapOf("fødselsnummer" to "1", "aktørId" to "2"),
            )
    ...
    fun `forskjellige typer, men med samme kontekster, er ikke like`() {
        val ytelserKontekst =
            SpesifikkKontekst("Ytelser", mapOf("fødselsnummer" to "1", "aktørId" to "2"))
        val utbetalingshistorikkKontekst =
            SpesifikkKontekst(
                "Utbetalingshistorikk",
                mapOf("fødselsnummer" to "1", "aktørId" to "2"),
    ...
    fun `forskjellige typer gir forskjellige hashCodes`() {
        val kontekst1 = SpesifikkKontekst("a", mapOf("b" to "c", "c" to "e"))
        val kontekst2 = SpesifikkKontekst("b", mapOf("b" to "c", "c" to "e"))
    ...
    fun `forskjellige kontekster gir forskjellige hashCodes`() {
        val kontekst1 = SpesifikkKontekst("a", mapOf("b" to "c", "c" to "e"))
        val kontekst2 = SpesifikkKontekst("a", mapOf("b" to "c", "c" to "f"))
        assertNotEquals(kontekst1, kontekst2)
        assertNotEquals(kontekst1.hashCode(), kontekst2.hashCode())
    }
}
