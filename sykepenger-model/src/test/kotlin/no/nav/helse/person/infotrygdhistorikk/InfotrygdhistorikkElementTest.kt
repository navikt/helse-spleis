...
internal class InfotrygdhistorikkElementTest {
    ...
    fun `lik historikk`() {
        ...
        val inntekter = listOf(Inntektsopplysning("orgnr", 1.januar, 25000.månedlig, true))
        ...
        assertNotEquals(
            ...
            nyttHistorikkelement(inntekter = inntekter),
        )
        assertNotEquals(
            nyttHistorikkelement(perioder, inntekter),
    ...
    fun `sortering på arbeidsgiverkategorikode har ikke betydning`() {
        val arbeidskategorikoder1 = mapOf("01" to 1.januar, "02" to 2.januar)
        val arbeidskategorikoder2 = mapOf("02" to 2.januar, "01" to 1.januar)
    ...
    fun `samlet utbetalingstidslinje`() {
        ...
        element.utbetalingstidslinje().also { assertEquals(1.januar til 20.januar, it.periode()) }
    }
    ...
    fun `hver første utbetalingsdag har en tilhørende inntekt`() {
        ...
        aktivitetslogg.barn().also { assertTrue(element.valider(it, februar, "ag1")) }
    }
    ...
    fun `RefusjonTilArbeidsgiver mappes til utbetalingstidslinje`() {
        ...
        val tidslinje =
            utbetalinger.map { it.utbetalingstidslinje() }.reduce(Utbetalingstidslinje::plus)
    ...
    fun `validering gir ikke warning hvis vi har to inntekter for samme arbeidsgiver på samme dato, men dato er 12 måneder før perioden`() {
        ...
        assertTrue(
            element.valider(aktivitetslogg, Periode(1.februar(2019), 28.februar(2019)), "ag1")
        )
        aktivitetslogg.assertIngenVarsler()
    }
