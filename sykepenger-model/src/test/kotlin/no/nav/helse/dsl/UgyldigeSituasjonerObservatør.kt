...
internal class UgyldigeSituasjonerObservatør(private val person: Person) : PersonObserver {
    ...
    private val arbeidsgivere
        get() = arbeidsgivereMap.values
    ...
    private fun loggBehandlingstatus(vedtaksperiodeId: UUID, status: Behandlingstatus) {
        gjeldendeBehandlingstatus
            .getOrPut(vedtaksperiodeId) { mutableListOf() }
    ...
    override fun vedtaksperiodeAnnullert(
    ...
    private fun bekreftAtBehandlingFinnes(behandlingId: UUID) {
        ...
        check(
            behandlingVarsletOmFør(behandlingId) || behandlingFinnesHosArbeidsgiver(behandlingId)
        ) {
    ...
    override fun vedtaksperiodeEndret(event: PersonObserver.VedtaksperiodeEndretEvent) {
        arbeidsgivereMap.getOrPut(event.organisasjonsnummer) {
            person.arbeidsgiver(event.organisasjonsnummer)
        }
    ...
    private fun sjekkUgyldigeVentesituasjoner(event: PersonObserver.VedtaksperiodeVenterEvent) {
        if (event.venterPå.venteårsak.hva != "HJELP")
            return // Om vi venter på noe annet enn hjelp er det OK 👍
        """
    ...
    private fun sjekkSøknadIdEierskap(vedtaksperiodeId: UUID, hendelseIder: Set<UUID>) {
        ...
        søknadIder.forEach { søknadId ->
            ...
            if (eier == null) søknader[søknadId] = vedtaksperiodeId
                ...
                check(eier == vedtaksperiodeId) {
                    "Både vedtaksperiode $eier og $vedtaksperiodeId peker på søknaden $søknadId"
                }
        }
    }
    ...
    override fun overstyringIgangsatt(event: PersonObserver.OverstyringIgangsatt) {
        check(event.berørtePerioder.isNotEmpty()) {
            "Forventet ikke en igangsatt overstyring uten berørte perioder."
        }
        if (event.årsak == "KORRIGERT_INNTEKTSMELDING")
            IM.korrigertInntekt(event.meldingsreferanseId)
    }

    private fun PersonObserver.VedtaksperiodeVenterEvent.tilstander() =
        when (vedtaksperiodeId == venterPå.vedtaksperiodeId) {
            true ->
                "En vedtaksperiode i ${gjeldendeTilstander[vedtaksperiodeId]} trenger hjelp${venterPå.venteårsak.hvorfor?.let { " fordi $it" } ?: ""}! 😱"
            false ->
                "En vedtaksperiode i ${gjeldendeTilstander[vedtaksperiodeId]} venter på en annen vedtaksperiode i ${gjeldendeTilstander[venterPå.vedtaksperiodeId]} som trenger${venterPå.venteårsak.hvorfor?.let { " fordi $it" } ?: ""}! 😱"
        }
        ...
        aktivitetslogg.aktiviteter.forEach {
            when (it) {
                ...
                is Aktivitet.Varsel -> {
                    ...
                    // 💀
                    if (it.kode in setOf(RV_SV_1, Varselkode.RV_RV_7, Varselkode.RV_UT_2))
                        return@forEach

                    val vedtaksperiodekontekst =
                        checkNotNull(
                            it.kontekster.firstOrNull { it.kontekstType == "Vedtaksperiode" }
                        ) {
                            "Det er opprettet et varsel utenom Vedtaksperiode:\n${it}"
                        }
                    val vedtaksperiodeId =
                        UUID.fromString(
                            vedtaksperiodekontekst.kontekstMap.getValue("vedtaksperiodeId")
                        )
                    val behandlingstatusPåTidspunkt =
                        gjeldendeBehandlingstatus
                            .getValue(vedtaksperiodeId)
                            .first { (tidspunkt, _) -> tidspunkt < it.tidsstempel }
                            .second
            ...
            it.view().aktiveVedtaksperioder.forEach {
                it.behandlinger.behandlinger.forEach {
                    it.endringer.forEach {
                        ...
                        // Inntektsmeldingen driver selvfølgelig å lager noen ukjente dager i snuten
                        // når første fraværsdag blir SykedagNav 🫠
                        val førsteIkkeUkjenteDagErSykedagNav =
                            it.sykdomstidslinje.inspektør.dager[
                                    it.sykdomstidslinje.inspektør.førsteIkkeUkjenteDag] is
                                Dag.SykedagNav
                    ...
                    vedtaksperiode.behandlinger.behandlinger.forEach behandling@{ behandling ->
                        behandling.endringer.last().let { endring ->
                            if (endring.refusjonstidslinje.isEmpty()) {
                                if (behandling.tilstand == AVSLUTTET_UTEN_VEDTAK)
                                    return@behandling // Ikke noe refusjonsopplysning på AUU er OK
                                if (
                                    vedtaksperiode.tilstand == AVVENTER_BLOKKERENDE_PERIODE &&
                                        behandling.tilstand == UBEREGNET_OMGJØRING
                                )
                                    return@behandling // Dette kan være AUU'er som skal tilbake til
                                                      // AUU, de må ikke ha refusjonsopplysninger.
                                if (
                                    vedtaksperiode.tilstand in
                                        setOf(AVVENTER_INFOTRYGDHISTORIKK, AVVENTER_INNTEKTSMELDING)
                                )
                                    return@behandling // Ikke fått refusjonsopplysninger enda da
                                error(
                                    "Burde ikke ha tom refusjonstidslinje i tilstand ${vedtaksperiode.tilstand}"
                                )
                            }
                            val perioder = endring.refusjonstidslinje.perioderMedBeløp
                            check(perioder.size == 1) {
                                "Burde ikke være noen hull i refusjonstidslinjen."
                            }
                            check(perioder.single() == endring.periode) {
                                "Refusjonstidslinjen skal dekke hele perioden. Perioden er ${endring.periode}, refusjonsopplysninger for $perioder"
                            }
                        }
                    }
                }
    ...
    private fun BehandlingView.gyldigTilInfotrygd() =
        tilstand == BehandlingView.TilstandView.TIL_INFOTRYGD &&
            avsluttet != null &&
            vedtakFattet == null
    ...
    private fun BehandlingView.gyldigAvsluttet() =
        tilstand == BehandlingView.TilstandView.VEDTAK_IVERKSATT &&
            avsluttet != null &&
            vedtakFattet != null

    private val BehandlingView.nøkkelinfo
        get() = "tilstand=$tilstand, avsluttet=$avsluttet, vedtakFattet=$vedtakFattet"
    ...
    private fun validerTilstandPåSisteBehandlingForFerdigbehandledePerioder() {
        arbeidsgivere.forEach { arbeidsgiver ->
            ...
            view.aktiveVedtaksperioder
                ...
                .groupBy(keySelector = { it.tilstand }) { it.behandlinger.behandlinger.last() }
                .forEach { (tilstand, sisteBehandlinger) ->
                    when (tilstand) {
                        TilstandType.TIL_INFOTRYGD ->
                            sisteBehandlinger
                                .filterNot { it.gyldigTilInfotrygd() }
                                .let {
                                    check(it.isEmpty()) {
                        ...
                        TilstandType.AVSLUTTET_UTEN_UTBETALING ->
                            sisteBehandlinger
                                .filterNot { it.gyldigAvsluttetUtenUtbetaling() }
                                .let {
                                    check(it.isEmpty()) {
                        ...
                        TilstandType.AVSLUTTET ->
                            ...
                            error(
                                "Svært snedig at perioder i ${tilstand::class.simpleName} er ferdig behandlet"
                            )
                    }
                }
        }
        ...
        arbeidsgivere.forEach { arbeidsgiver ->
            ...
            view.aktiveVedtaksperioder.forEach { current ->
                if (forrigePeriode?.periode?.overlapperMed(current.periode) == true) {
                    error(
                        "For Arbeidsgiver ${view.organisasjonsnummer} overlapper Vedtaksperiode ${current.id} (${current.periode}) og Vedtaksperiode ${forrigePeriode.id} (${forrigePeriode.periode}) med hverandre!"
                    )
                }
    ...
    internal companion object {
        internal class UgyldigSituasjonException(cause: Throwable) :
