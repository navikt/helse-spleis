...
internal abstract class AbstractDslTest {
    ...
    @Suppress("unused")
    private val person get() = testperson.person // Hen brukes av @OpenInSpanner
    ...
    protected fun TestPerson.TestArbeidsgiver.assertIngenFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) = testArbeidsgiverAsserter.assertIngenFunksjonelleFeil(*filtre)

    protected fun TestPerson.TestArbeidsgiver.assertFunksjonelleFeil() = testArbeidsgiverAsserter.assertFunksjonelleFeil()
    ...
    protected fun TestPerson.TestArbeidsgiver.assertVarsler(vararg filtre: AktivitetsloggFilter) = testArbeidsgiverAsserter.assertVarsler(*filtre)
    ...
    protected fun TestPerson.TestArbeidsgiver.assertIngenVarsler(vararg filtre: AktivitetsloggFilter) = testArbeidsgiverAsserter.assertIngenVarsler(*filtre)
    ...
    protected fun String.håndterSimulering(vedtaksperiodeId: UUID) = this { håndterSimulering(vedtaksperiodeId) }
    ...
    protected fun String.håndterUtbetalingshistorikkEtterInfotrygdendring(vararg utbetalinger: Infotrygdperiode) = this { håndterUtbetalingshistorikkEtterInfotrygdendring(*utbetalinger) }

    protected fun String.håndterUtbetalt(status: Oppdragstatus) = this { håndterUtbetalt(status) }

    protected fun String.håndterAnnullering(utbetalingId: UUID) = this { håndterAnnullering(utbetalingId) }

    protected fun String.håndterIdentOpphørt(
        nyttFnr: Personidentifikator,
        nyAktørId: String
    ...
    protected fun String.håndterGrunnbeløpsregulering(skjæringstidspunkt: LocalDate) = this { håndterGrunnbeløpsregulering(skjæringstidspunkt) }

    protected fun nullstillTilstandsendringer() = observatør.nullstillTilstandsendringer()

    protected fun String.assertTilstander(
        id: UUID,
        vararg tilstander: TilstandType
    ) = this { assertTilstander(id, *tilstander) }

    protected fun String.assertSisteTilstand(
        id: UUID,
        tilstand: TilstandType
    ) = this { assertSisteTilstand(id, tilstand) }

    protected fun String.assertIngenFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) = this { assertIngenFunksjonelleFeil(*filtre) }

    protected fun String.assertVarsler(vararg filtre: AktivitetsloggFilter) = this { assertVarsler(*filtre) }

    protected fun String.assertVarsel(
        warning: String,
        vararg filtre: AktivitetsloggFilter
    ...
    protected fun String.assertIngenVarsel(
        kode: Varselkode,
        vararg filtre: AktivitetsloggFilter
    ...
    protected fun String.assertIngenVarsler(vararg filtre: AktivitetsloggFilter) = this { assertIngenVarsler(*filtre) }
    ...
    protected fun håndterAnnullering(utbetalingId: UUID) = bareÈnArbeidsgiver(a1).håndterAnnullering(utbetalingId)

    protected fun håndterIdentOpphørt(
        nyttFnr: Personidentifikator,
        nyAktørId: String
    ...
    protected fun håndterGrunnbeløpsregulering(skjæringstidspunkt: LocalDate) = bareÈnArbeidsgiver(a1).håndterGrunnbeløpsregulering(skjæringstidspunkt)
    ...
    protected fun assertSisteTilstand(
        id: UUID,
        tilstand: TilstandType,
        orgnummer: String = a1
    ) = bareÈnArbeidsgiver(orgnummer).assertSisteTilstand(id, tilstand)

    protected fun assertIngenFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) = bareÈnArbeidsgiver(a1).assertIngenFunksjonelleFeil(*filtre)

    protected fun assertVarsler(vararg filtre: AktivitetsloggFilter) = bareÈnArbeidsgiver(a1).assertVarsler(*filtre)
    ...
    protected fun assertIngenVarsel(
        kode: Varselkode,
        vararg filtre: AktivitetsloggFilter
    ) = bareÈnArbeidsgiver(a1).assertIngenVarsel(kode, *filtre)

    protected fun assertFunksjonellFeil(
        kode: Varselkode,
        vararg filtre: AktivitetsloggFilter
    ) = bareÈnArbeidsgiver(a1).assertFunksjonellFeil(kode, *filtre)

    protected fun assertIngenVarsler(vararg filtre: AktivitetsloggFilter) = bareÈnArbeidsgiver(a1).assertIngenVarsler(*filtre)
