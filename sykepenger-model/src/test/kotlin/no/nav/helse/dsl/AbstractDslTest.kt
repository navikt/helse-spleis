...
internal abstract class AbstractDslTest {
    internal companion object {
        @JvmStatic protected val a1 = "a1"
        @JvmStatic protected val a2 = "a2"
        @JvmStatic protected val a3 = "a3"
        @JvmStatic protected val frilans = Frilans
        @JvmStatic protected val selvstendig = Selvstendig
        @JvmStatic protected val arbeidsledig = Arbeidsledig
        @JvmStatic @Deprecated("må bruke a1") protected val ORGNUMMER = a1
        @JvmStatic protected val personInspektør = { person: Person -> PersonInspektør(person) }
        @JvmStatic protected val personView = { person: Person -> person.view() }
        @JvmStatic
        protected val agInspektør = { orgnummer: String ->
            { person: Person -> TestArbeidsgiverInspektør(person, orgnummer) }
        }

        @JvmStatic protected infix fun String.og(annen: String) = listOf(this, annen)

        @JvmStatic protected infix fun List<String>.og(annen: String) = this.plus(annen)
    }
    ...
    @Suppress("unused")
    private val person
        get() = testperson.person // Hen brukes av @OpenInSpanner

    private lateinit var deferredLog: DeferredLog

    protected fun Int.vedtaksperiode(orgnummer: String) = orgnummer { vedtaksperiode }

    protected val Int.vedtaksperiode
        get() = vedtaksperiode(bareÈnArbeidsgiver(a1))

    protected val String.inspektør
        get() = inspektør(this)

    protected val inspektør: TestArbeidsgiverInspektør
        get() = bareÈnArbeidsgiver(a1).inspektør

    private val TestPerson.TestArbeidsgiver.testArbeidsgiverAsserter
        get() =
            TestArbeidsgiverAssertions(observatør, inspektør, testperson.inspiser(personInspektør))

    private val testPersonAsserter
        get() = TestPersonAssertions(testperson.inspiser(personInspektør), jurist)
    ...
    protected operator fun <R> List<String>.invoke(testblokk: TestPerson.TestArbeidsgiver.() -> R) =
        forEach { organisasjonsnummer ->
            organisasjonsnummer { testblokk() }
        }
    ...
    protected fun List<String>.nyeVedtak(
        ...
        sykepengegrunnlagSkatt: InntektForSykepengegrunnlag =
            lagStandardSykepengegrunnlag(map { it to inntekt }, periode.start),
    ...
    protected fun TestPerson.TestArbeidsgiver.assertIngenFunksjonelleFeil(
    ...
    protected fun TestPerson.TestArbeidsgiver.assertIngenVarsler(
        vararg filtre: AktivitetsloggFilter
    ...
    protected fun String.håndterSimulering(vedtaksperiodeId: UUID) = this {
    ...
    protected fun String.håndterUtbetalingshistorikkEtterInfotrygdendring(
        vararg utbetalinger: Infotrygdperiode
    ) = this { håndterUtbetalingshistorikkEtterInfotrygdendring(*utbetalinger) }

    protected fun String.håndterUtbetalt(status: Oppdragstatus) = this { håndterUtbetalt(status) }

    protected fun String.håndterAnnullering(utbetalingId: UUID) = this {
    ...
    protected fun String.håndterGrunnbeløpsregulering(skjæringstidspunkt: LocalDate) = this {
        håndterGrunnbeløpsregulering(skjæringstidspunkt)
    }

    protected fun nullstillTilstandsendringer() = observatør.nullstillTilstandsendringer()

    protected fun String.assertTilstander(id: UUID, vararg tilstander: TilstandType) = this {
        assertTilstander(id, *tilstander)
    }

    protected fun String.assertSisteTilstand(id: UUID, tilstand: TilstandType) = this {
        assertSisteTilstand(id, tilstand)
    }

    protected fun String.assertIngenFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) = this {
        assertIngenFunksjonelleFeil(*filtre)
    }

    protected fun String.assertVarsler(vararg filtre: AktivitetsloggFilter) = this {
        assertVarsler(*filtre)
    }

    protected fun String.assertVarsel(warning: String, vararg filtre: AktivitetsloggFilter) = this {
        assertVarsel(warning, *filtre)
    }

    protected fun String.assertVarsel(kode: Varselkode, vararg filtre: AktivitetsloggFilter) =
        this {
            assertVarsel(kode, *filtre)
        }

    protected fun String.assertIngenVarsel(kode: Varselkode, vararg filtre: AktivitetsloggFilter) =
        this {
            assertIngenVarsel(kode, *filtre)
        }
    ...
    protected fun String.assertIngenVarsler(vararg filtre: AktivitetsloggFilter) = this {
        assertIngenVarsler(*filtre)
    }
    ...
    protected fun String.nyttVedtak(
        ...
        sykepengegrunnlagSkatt: InntektForSykepengegrunnlag =
            lagStandardSykepengegrunnlag(this, beregnetInntekt, førsteFraværsdag),
    ) = this {
        nyttVedtak(
            periode,
            grad,
    ...
    protected fun håndterPåminnelse(
        ...
        bareÈnArbeidsgiver(a1)
            .håndterPåminnelse(vedtaksperiodeId, tilstand, tilstandsendringstidspunkt)
    ...
    protected fun assertActivities() {
        ...
        assertTrue(inspektør.aktivitetslogg.aktiviteter.isNotEmpty()) {
            inspektør.aktivitetslogg.toString()
        }
    }
    ...
    protected fun nyttVedtak(
        ...
        status: Oppdragstatus = Oppdragstatus.AKSEPTERT,
        sykepengegrunnlagSkatt: InntektForSykepengegrunnlag =
            lagStandardSykepengegrunnlag(a1, beregnetInntekt, førsteFraværsdag),
    ) =
        bareÈnArbeidsgiver(a1)
            .nyttVedtak(
                vedtaksperiode,
