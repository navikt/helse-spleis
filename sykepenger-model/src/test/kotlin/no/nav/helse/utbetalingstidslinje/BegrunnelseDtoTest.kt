...
internal class BegrunnelseDtoTest {
    ...
    fun `MinimumSykdomsgrad avviser ikke helg`() {
        ...
        assertTrue(
            Begrunnelse.MinimumSykdomsgrad.skalAvvises(ArbeidsgiverperiodedagNav(1.januar, økonomi))
        )
    ...
    fun `avviser med flere begrunnelser`() {
        ...
        assertEquals(
            2,
            dag.avvis(listOf(Begrunnelse.MinimumSykdomsgrad, Begrunnelse.MinimumInntekt))
                ?.begrunnelser
                ?.size ?: 0,
        )
    }
