...
internal class AssertForventetFeilTest {
                ...
                assertForventetFeil(
                    nå = {
                        throw IllegalStateException(
                            "Irrelevant om det feiler med assertion eller exception"
                        )
                    },
    ...
    fun `nå-oppførsel er endret uten at det er ønsket oppførsel`() {
        val assertionError =
            assertThrows<AssertionError> {
                assertForventetFeil(nå = { assertTrue(false) }, ønsket = { assertTrue(false) })
            }
        assertEquals(
    ...
    fun `både nå- og ønsket oppførsel er oppfylt`() {
        val assertionError =
            assertThrows<AssertionError> {
                assertForventetFeil(nå = { assertTrue(true) }, ønsket = { assertTrue(true) })
            }
        assertEquals(
            "✅ Koden oppfører seg nå som ønsket! Fjern bruken av 'assertForventetFeil', og behold kun assertions for ønsket oppførsel ✅ ==> Expected java.lang.Throwable to be thrown, but nothing was thrown.",
            assertionError.message,
        )
    }
    ...
    fun `nå-oppførsel er oppfylt uten at det er ønsket oppførsel`() {
        assertDoesNotThrow {
            assertForventetFeil(nå = { assertTrue(true) }, ønsket = { assertTrue(false) })
        }
    }
