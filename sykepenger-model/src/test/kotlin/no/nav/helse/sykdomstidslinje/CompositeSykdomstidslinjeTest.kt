...
internal class CompositeSykdomstidslinjeTest {
    ...
    internal fun `kan bestemme hvilken type dager mellom to perioder skal ha`() {
        val arbeidsgiverperiode1 = ConcreteSykdomstidslinje.sykedager(Uke(1).mandag, Uke(1).onsdag, Dag.Kildehendelse.Søknad)
        val arbeidsgiverperiode2 = ConcreteSykdomstidslinje.sykedager(Uke(2).onsdag, Uke(2).fredag, Dag.Kildehendelse.Søknad)
    ...
    internal fun `to sykeperioder med mellomrom får riktig slutt og start dato`() {
        val førsteInterval = ConcreteSykdomstidslinje.sykedager(Uke(1).mandag, Uke(1).tirsdag, Dag.Kildehendelse.Søknad)
        val andreInterval = ConcreteSykdomstidslinje.sykedager(Uke(1).fredag, Uke(2).mandag, Dag.Kildehendelse.Søknad)
    ...
    internal fun `tidslinje med ubestemt dag er utenfor omfang`() {
        val studiedag = ConcreteSykdomstidslinje.studiedag(Uke(1).mandag, Dag.Kildehendelse.Søknad)
        val sykedag = ConcreteSykdomstidslinje.sykedag(Uke(1).mandag, Dag.Kildehendelse.Søknad)
    ...
    internal fun `tidslinje med permisjonsdag er utenfor omfang`() {
        val permisjonsdag = ConcreteSykdomstidslinje.permisjonsdag(Uke(1).mandag, Dag.Kildehendelse.Søknad)
