...
internal class LåstePerioderTest {
    ...
    fun `låse hele sykdomtidslinjen`() {
        val actual =
            (1.januar jobbTil 5.januar)
                .lås(Periode(1.januar, 5.januar))
    ...
    fun `kan bare låse perioder som dekkes av tidslinjen`() {
        assertThrows<IllegalArgumentException> {
            (2.januar jobbTil 4.januar).lås(Periode(1.januar, 4.januar))
        }
        assertThrows<IllegalArgumentException> {
            (2.januar jobbTil 4.januar).lås(Periode(2.januar, 5.januar))
        }
        assertThrows<IllegalArgumentException> {
            (2.januar jobbTil 4.januar).lås(Periode(10.januar, 15.januar))
        }
    }
    ...
    fun `kan bare låse opp perioder som allerede er låst`() {
        assertThrows<IllegalArgumentException> {
            (1.januar jobbTil 5.januar).låsOpp(Periode(2.januar, 4.januar))
        }
    }
