...
internal class LåstePerioderTest {
    ...
    fun `låse hele sykdomtidslinjen`() {
        val actual = (1.januar jobbTil 5.januar).lås(Periode(1.januar, 5.januar))
            .merge(1.januar ferieTil 5.januar)
        assertEquals(5, actual.filterIsInstance<Arbeidsdag>().size)
    }

    @Test
    fun `låse deler av sykdomtidslinje`() {
        val actual = (1.januar jobbTil 5.januar).lås(Periode(2.januar, 4.januar))
            .merge(1.januar ferieTil 5.januar)
    ...
    fun `låse opp sykdomtidslinje`() {
        val initial = (1.januar jobbTil 5.januar).lås(Periode(2.januar, 4.januar))
            .merge(1.januar ferieTil 5.januar)
    ...
    fun `kan bare låse opp perioder som allerede er låst`() {
        assertThrows<IllegalArgumentException> {
            (1.januar jobbTil 5.januar).låsOpp(
                Periode(
                    2.januar,
                    4.januar
                )
    ...
    fun `låse opp overlappende låste dagperioder`() {
        val initial = (1.januar jobbTil 25.januar).lås(Periode(2.januar, 11.januar))
