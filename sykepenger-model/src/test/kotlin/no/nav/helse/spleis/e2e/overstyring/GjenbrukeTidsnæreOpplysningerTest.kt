...
internal class GjenbrukeTidsnæreOpplysningerTest : AbstractDslTest() {
    ...
    fun `Stuckiness med helg involvert - da gjenbruker vi tidsnære opplysninger`() {
        ...
        /**
         * Kjære leser av testen: Alt før dette er litt kjedelig, men nå blir det gøy Ting ser slik
         * ut:
         *
         *      A1 |---------------januar--------------|
         *      A2 |-----------|  |--------------------|
         * - Det lille hullet vi ikke har søknad for på A2 er helg (lørdag-søndag)
         * - Nå kommer det en korrigerende inntektsmelding fra A1 som sier at AGP startet mandagen
         *   A2 sin andre periode starter
         */
        a1 {
            assertEquals(
            ...
            assertEquals(
                "AAAAARR AAAAARR AAAAARR SSSSSHH SSS",
                inspektør.sykdomstidslinje.toShortString(),
            )
        }
        a2 {
            assertEquals(
                "SSSSSHH SSSSSHH SSSSS?? SSSSSHH SSS",
                inspektør.sykdomstidslinje.toShortString(),
            )
    ...
    fun `revurdere seg inn i en situasjon hvor man ikke har noen første fraværsdag, men gjenbrukbare opplysninger biter læll`() {
        a1 {
            ...
            håndterOverstyrTidslinje(
                (15.mars til 31.mars).map { ManuellOverskrivingDag(it, Pleiepengerdag) }
            )
            håndterYtelser(3.vedtaksperiode)
            håndterSimulering(3.vedtaksperiode)
            håndterUtbetalingsgodkjenning(3.vedtaksperiode)
            ...
            håndterOverstyrTidslinje(
                (15.februar til 28.februar).map { ManuellOverskrivingDag(it, Pleiepengerdag) }
            )
            håndterYtelser(2.vedtaksperiode)
            håndterUtbetalingsgodkjenning(2.vedtaksperiode)
    ...
    fun `Først litt gjenbruk av tidsnære opplysninger etterfulgt av overstyring til andre ytelser som gjør at vi ikke trengte dem allikevel`() {
        ...
        a1 {
            håndterOverstyrTidslinje(
                listOf(ManuellOverskrivingDag(31.januar, Dagtype.Svangerskapspengerdag))
            )
            håndterYtelser(1.vedtaksperiode)
            håndterSimulering(1.vedtaksperiode)
        }
        ...
        a1 {
            håndterOverstyrTidslinje(
                februar.map { ManuellOverskrivingDag(it, Dagtype.Svangerskapspengerdag) }
            )
            assertEquals(1.februar, inspektør.skjæringstidspunkt(2.vedtaksperiode))
        }
        a2 {
        ...
        a1 { assertSisteTilstand(2.vedtaksperiode, AVVENTER_HISTORIKK_REVURDERING) }
    }
    ...
    fun `endrer skjæringstidspunkt på en førstegangsbehandling ved å omgjøre en arbeidsdag til sykedag`() {
        a1 {
            ...
            val sykepengegrunnlagFør =
                inspektør.vilkårsgrunnlag(2.vedtaksperiode)?.inspektør?.inntektsgrunnlag
    ...
    fun `endrer skjæringstidspunkt på sykefraværstilfelle etter - endrer ikke dato for inntekter`() {
        ...
        val sykepengegrunnlagFør = a2 {
            inspektør.vilkårsgrunnlag(1.vedtaksperiode)?.inspektør?.inntektsgrunnlag
                ?: fail { "finner ikke vilkårsgrunnlag" }
        }
    ...
    fun `vedtaksperiode flytter skjæringstidspunktet frem`() {
        a1 {
            ...
            assertUtbetalingsdag(
                18.januar,
                Utbetalingsdag.ArbeidsgiverperiodeDag::class,
                Inntekt.INGEN,
                Inntekt.INGEN,
            )
            assertUtbetalingsdag(
                19.januar,
                Utbetalingsdag.NavDag::class,
                1431.daglig,
                Inntekt.INGEN,
            )
    ...
    fun `innfører ny arbeidsgiverperiode på en førstegangsbehandling etter tidligere utbetalt`() {
        a1 {
            ...
            assertEquals(
                Endringskode.NY,
                revurderingFebruar.arbeidsgiverOppdrag.inspektør.endringskode,
            )
    ...
    fun `gjenbruker saksbehandlerinntekt`() {
        a1 {
            ...
            val sykepengegrunnlagFør =
                inspektør.vilkårsgrunnlag(1.vedtaksperiode)!!.inspektør.inntektsgrunnlag.inspektør
            ...
            håndterOverstyrArbeidsgiveropplysninger(
                ...
                listOf(
                    OverstyrtArbeidsgiveropplysning(
                        ...
                        "overstyring",
                        null,
                        listOf(Triple(1.januar, null, INNTEKT)),
                    )
                ),
            )
            håndterYtelser(1.vedtaksperiode)
            ...
            val sykepengegrunnlag =
                inspektør.vilkårsgrunnlag(2.vedtaksperiode)!!.inspektør.inntektsgrunnlag.inspektør
    ...
    fun `gjenbruker saksbehandlerinntekt som overstyrer annen saksbehandler`() {
        a1 {
            ...
            assertEquals(
                andreOverstyring,
                inspektør.skjønnsmessigFastsattOverstyrtHendelseId(1.januar),
            )
            assertEquals(
                inntektsmelding,
                inspektør.skjønnsmessigFastsattOverstyrtHendelseId(1.februar),
            )
        }
    }
    ...
    fun `gjenbruker grunnlaget for skjønnsmessig fastsettelse`() {
        a1 {
            ...
            håndterSkjønnsmessigFastsettelse(
                1.februar,
                listOf(OverstyrtArbeidsgiveropplysning(a1, beregnetInntektIM)),
            )
    ...
    fun `ikke varsel når det er hull i agp`() {
        a1 {
            ...
            håndterInntektsmelding(listOf(17.januar til 20.januar, 2.februar til 13.februar))
    ...
    fun `gjenbruker den underliggende inntektsmeldingen ved skjønnsmessig fastsatt`() {
        a1 {
            ...
            val inntektsmeldingId =
                håndterInntektsmelding(listOf(8.januar til 23.januar), beregnetInntekt = INNTEKT)
            håndterVilkårsgrunnlag(1.vedtaksperiode)
            håndterYtelser(1.vedtaksperiode)
            håndterSimulering(1.vedtaksperiode)
            håndterUtbetalingsgodkjenning(1.vedtaksperiode)
    ...
    fun `gjenbruker den siste saksbehandlerinntekten om det er overstyrt mange ganger`() {
        a1 {
            ...
            håndterOverstyrArbeidsgiveropplysninger(
                8.januar,
                listOf(
                    OverstyrtArbeidsgiveropplysning(a1, overstyring1Inntekt, forklaring = "ja takk")
                ),
            ...
            håndterOverstyrArbeidsgiveropplysninger(
                8.januar,
                listOf(
                    OverstyrtArbeidsgiveropplysning(a1, overstyring2Inntekt, forklaring = "ja takk")
                ),
    ...
    private fun assertTidsnærInntektsopplysning(
        ...
        val inntektsopplysningerEtterEndring =
            inntektsgrunnlagEtter.inspektør.arbeidsgiverInntektsopplysningerPerArbeidsgiver
                .getValue(orgnummer)
    ...
    private fun TestPerson.TestArbeidsgiver.inntektsopplysning(skjæringstidspunkt: LocalDate) =
        inspektør
            ...
            ?.singleOrNull { it.inspektør.orgnummer == this.orgnummer }
            ?.inspektør
            ?.inntektsopplysning
            ?: error(
                "Forventet å finne inntektsopplysning for ${this.orgnummer} på $skjæringstidspunkt"
            )
    ...
    private fun TestArbeidsgiverInspektør.skjønnsmessigFastsattOverstyrtHendelseId(
        ...
        vilkårsgrunnlag(skjæringstidspunkt)!!
            ...
            .inspektør
            .arbeidsgiverInntektsopplysninger
            .single { it.gjelder(organisasjonsnummer) }
            .inspektør
            .inntektsopplysning
            .let {
                assertTrue(it is SkjønnsmessigFastsatt) {
                    "Forventet SkjønnmessigFastsatt inntekt, var ${it::class.simpleName}"
                }
