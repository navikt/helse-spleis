...
internal class TrengerArbeidsgiveropplysningerTest : AbstractEndToEndTest() {
    ...
    fun `Skal høre på arbeidsgiver når hen sier at egenmeldinger ikke gjelder`() {
        ...
        assertEquals(
            emptyList<Periode>(),
            inspektør.vedtaksperioder(1.vedtaksperiode).egenmeldingsperioder,
        )
    }
    ...
    fun `Skal ikke sende forespørsel for korte perioder etter at arbeidsgiver har sendt riktig AGP`() {
        håndterSøknad(
            Sykdom(6.januar, 17.januar, 100.prosent),
            egenmeldinger = listOf(1.januar til 5.januar),
        )
        assertEquals(1, observatør.trengerArbeidsgiveropplysningerVedtaksperioder.size)
        håndterSøknad(
            Sykdom(22.januar, 25.januar, 100.prosent),
            egenmeldinger = listOf(21.januar til 21.januar),
        )
        ...
        assertEquals(
            listOf(21.januar til 21.januar),
            inspektør.vedtaksperioder(2.vedtaksperiode).egenmeldingsperioder,
        )
    ...
    fun `første fraværsdag vi mottar i IM blir feil når det er ferie første dag i sykmeldingsperioden etter kort gap`() {
        ...
        observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last().let { event ->
            ...
            assertEquals(
                listOf(PersonObserver.FørsteFraværsdag(ORGNUMMER, 13.februar)),
                event.førsteFraværsdager,
            )
            assertFalse(
                event.forespurteOpplysninger.any { it is PersonObserver.Arbeidsgiverperiode }
            )
        }
    ...
    fun `ber ikke om arbeidsgiveropplysninger på ghost når riktig inntektsmelding kommer`() {
        ...
        assertEquals(
            1,
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.count {
                it.organisasjonsnummer == a1
            },
        )
        assertEquals(
            2,
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.count {
                it.organisasjonsnummer == a2
            },
        )
    ...
    fun `ber ikke om arbeidsgiveropplysninger på forlengelse med forskjøvet agp`() {
        ...
        assertEquals(1, observatør.trengerArbeidsgiveropplysningerVedtaksperioder.size)
        assertEquals(
            2.vedtaksperiode.id(ORGNUMMER),
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.single().vedtaksperiodeId,
        )
    }
        ...
        val expectedForespurteOpplysninger =
            listOf(
                ...
                PersonObserver.Refusjon(
                    forslag = listOf(Refusjonsforslag(1.januar, null, INNTEKT.månedlig))
                ),
            )
    ...
    fun `sender med begge sykmeldingsperiodene når vi har en kort periode som forlenges av en lang`() {
        ...
        val trengerArbeidsgiveropplysningerEvent =
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last()
        val expectedSykmeldingsperioder = listOf(1.januar til 16.januar, 17.januar til 31.januar)
        assertEquals(
            expectedSykmeldingsperioder,
            trengerArbeidsgiveropplysningerEvent.sykmeldingsperioder,
        )
    ...
    fun `sender med riktig sykmeldingsperioder når arbeidsgiverperioden er stykket opp i flere korte perioder`() {
        ...
        val expectedSykmeldingsperioder =
            listOf(1.januar til 7.januar, 9.januar til 14.januar, 16.januar til 21.januar)
        assertEquals(
            expectedSykmeldingsperioder,
            trengerArbeidsgiveropplysningerEvent.sykmeldingsperioder,
        )
    ...
    fun `ber ikke om inntekt når vi allerede har inntekt på skjæringstidspunktet -- med arbeidsgiverperiode`() {
        ...
        val expectedForespurteOpplysninger =
            listOf(
                ...
                PersonObserver.Refusjon(
                    listOf(Refusjonsforslag(1.januar, null, INNTEKT_FLERE_AG.månedlig))
                ),
                PersonObserver.Arbeidsgiverperiode,
            )
        assertEquals(expectedForespurteOpplysninger, actualForespurteOpplysninger)
    ...
    fun `sender med første fraværsdag på alle arbeidsgivere for skjæringstidspunktet`() {
        nyeVedtakMedUlikFom(mapOf(a1 to (januar), a2 to (2.januar til 31.januar)))
    ...
    fun `sender med FastsattInntekt når vi allerede har inntekt fra skatt lagt til grunn på skjæringstidspunktet`() {
        nyeVedtakMedUlikFom(mapOf(a1 to (31.desember(2017) til 31.januar), a2 to (januar)))
        ...
        assertEquals(
            1,
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.count {
                it.vedtaksperiodeId == 2.vedtaksperiode.id(a2)
            },
        )
    ...
    fun `Skal ikke be om arbeidsgiverperiode når vi allerede har motatt inntektsmelding`() {
        ...
        val trengerArbeidsgiveropplysningerEvent =
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last()
        assertFalse(
            trengerArbeidsgiveropplysningerEvent.forespurteOpplysninger.contains(
                PersonObserver.Arbeidsgiverperiode
            )
        )
    }
    ...
    fun `blir syk fra ghost`() {
        ...
        val arbeidsgiveropplysningerEvents =
    ...
    fun `skal kun sende med refusjonsopplysninger som overlapper med, eller er etter, vedtaksperioden som ikke trenger inntektsopplysninger fra arbeidsgiver`() {
        ...
        assertUgyldigSituasjon(
            "Burde ikke ha tom refusjonstidslinje i tilstand AVVENTER_HISTORIKK"
        ) {
            nyPeriode(mars, a2)
        }
        ...
        //  Denne testen klager på at den har gått videre uten refusjonsopplysninge
        // (UgyldigeSituasjonerObservatør).
        //  Så denne vil løses (stå å vente på IM) når refusjonsopplysninger på behandlingen er
        // førende for om perioden har refusjonsopplysninger hen trenger.
        assertForventetFeil(
            forklaring =
                "perioden i mars (2.vedtaksperiode for a2) har en ny arbeidsgiverperiode og skal vente på opplysninger fra AG",
            nå = {
                assertTilstander(
                    2.vedtaksperiode,
                    START,
    ...
    fun `skal sende med riktig refusjonsopplysninger ved ingen refusjon`() {
        ...
        håndterInntektsmelding(
            arbeidsgiverperioder = listOf(1.januar til 16.januar),
            beregnetInntekt = INNTEKT_FLERE_AG,
            refusjon = Inntektsmelding.Refusjon(beløp = INNTEKT_FLERE_AG, opphørsdato = 15.mars),
            orgnummer = a2,
        )
        fraVilkårsprøvingTilGodkjent(INNTEKT_FLERE_AG)
        forlengVedtak(februar, orgnummer = a1)
        ...
        assertForventetFeil(
            ...
            ønsket = {
                ...
                val trengerArbeidsgiveropplysningerEvents =
                    observatør.trengerArbeidsgiveropplysningerVedtaksperioder
                assertEquals(3, trengerArbeidsgiveropplysningerEvents.size)

                val trengerArbeidsgiveropplysningerEvent =
                    trengerArbeidsgiveropplysningerEvents.last()
                val inntektsmeldingId = inspektør(a2).hendelseIder(1.vedtaksperiode.id(a2)).last()
    ...
    fun `Skal sende egenmeldingsdager fra søknad i forespørsel`() {
        ...
        håndterSøknad(
            Sykdom(5.januar, 31.januar, 100.prosent),
            egenmeldinger = listOf(1.januar til 4.januar),
        )
        assertEquals(
            5.januar til 31.januar,
            inspektør.vedtaksperioder(1.vedtaksperiode).inspektør.periode,
        )

        val expectedEgenmeldinger = listOf(1.januar til 4.januar)
        assertEquals(
            expectedEgenmeldinger,
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last().egenmeldingsperioder,
        )
    }
    ...
    fun `Sender med egenmeldingsdager fra kort søknad`() {
        ...
        håndterSøknad(
            Sykdom(5.januar, 10.januar, 100.prosent),
            egenmeldinger = listOf(4.januar til 4.januar),
        )
    ...
    fun `Skal sende med saksbehandlerinntekt som inntektForrigeSkjæringstidspunkt`() {
        ...
        val expectedForespurteOpplysninger =
            listOf(
                ...
                PersonObserver.Refusjon(
                    forslag = listOf(Refusjonsforslag(1.januar, null, 32000.månedlig.månedlig))
                ),
            )
        val actualForespurteOpplysninger =
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last().forespurteOpplysninger
    ...
    fun `Sender med riktige refusjonsopplysninger ved opphør av refusjon før perioden`() {
        ...
        val id =
            håndterInntektsmelding(
                arbeidsgiverperioder = listOf(1.januar til 16.januar),
                refusjon =
                    Inntektsmelding.Refusjon(INNTEKT, 10.februar, endringerIRefusjon = emptyList()),
            )
        håndterVilkårsgrunnlag(1.vedtaksperiode)
        ...
        val expectedForespurteOpplysninger =
            listOf(
                ...
                PersonObserver.Refusjon(
                    forslag = listOf(Refusjonsforslag(11.februar, null, INGEN.månedlig))
                ),
            )
        val actualForespurteOpplysninger =
            observatør.trengerArbeidsgiveropplysningerVedtaksperioder.last().forespurteOpplysninger
    ...
    fun `Skal ikke sende med egenmeldingsperioder når vi ikke ber om AGP`() {
        ...
        assertEquals(
            ...
            inspektør.vedtaksperioder(2.vedtaksperiode).inspektør.periode,
        )
        ...
        assertForventetFeil(
            forklaring =
                "Trenger ikke sende med egenmeldingsperioder når vi ikke ber om AGP, burde være sikker på at egenmeldinger ikke kommer fra søknad på perioden uten ny AGP",
            nå = {
                assertEquals(
                    listOf(19.januar til 19.januar),
                    actualForespørsel.egenmeldingsperioder,
                )
            },
            ønsket = { assertEquals(emptyList<Periode>(), actualForespørsel.egenmeldingsperioder) },
        )
    }
    ...
    fun `Skal ikke be om arbeidsgiverperiode når det er kort gap pga av arbeid gjenopptatt i slutten av perioden før en forlengelse`() {
        ...
        håndterSøknad(
            Sykdom(3.januar, 20.januar, 100.prosent),
            Søknad.Søknadsperiode.Arbeid(13.januar, 20.januar),
            orgnummer = a2,
        )
        håndterInntektsmelding(listOf(1.januar til 16.januar), orgnummer = a1)
    ...
    fun `egenmeldinger som hadde strukket perioden utover AGP skal føre til forespørsel om arbeidsgiveropplysninger`() {
        ...
        håndterSøknad(
            Sykdom(25.januar, 25.januar, 100.prosent),
            egenmeldinger = listOf(24.januar til 24.januar),
        )
