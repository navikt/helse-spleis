...
internal class FlereArbeidsgivereGhostTest : AbstractEndToEndTest() {
    ...
    fun `blir syk fra ghost man har vært syk fra tidligere, og inntektsmeldingen kommer først`() {
        ...
        assertInfoSomFølgeAv("Inntektsmelding oppdaterer ikke vilkårsgrunnlag") {
            håndterInntektsmelding(listOf(mandag den 29.januar til 13.februar), orgnummer = a2, avsendersystem = ALTINN)
        }
    ...
    fun `blir syk fra ghost og inntektsmeldingen kommer først`() {
        ...
        håndterInntektsmelding(listOf(1.februar til 16.februar), orgnummer = a2, avsendersystem = ALTINN)
        håndterSøknad(Sykdom(1.februar, 28.februar, 100.prosent), orgnummer = a2)
    ...
    fun `blir syk fra ghost annen måned enn skjæringstidspunkt etter at saksbehandler har overstyrt inntekten etter 8-28, 3 ledd bokstav b -- Ghost sender klassisk inntektsmelding`() {
        ...
        håndterOverstyrArbeidsgiveropplysninger(
            1.januar,
            listOf(OverstyrtArbeidsgiveropplysning(a2, INNTEKT * 1.1, veldigViktigSubsumsjonSomMåVæreSattForAtDetteSkalFeile)),
        )
        håndterYtelser(1.vedtaksperiode, orgnummer = a1)
        håndterSimulering(1.vedtaksperiode, orgnummer = a1)
        håndterUtbetalingsgodkjenning(1.vedtaksperiode, orgnummer = a1)
    ...
    fun `Korrigerende refusjonsopplysninger på arbeidsgiver med skatteinntekt i sykepengegrunnlaget`() {
        ...
        assertLikeRefusjonsopplysninger(
            listOf(
                Refusjonsopplysning(inntektsmelding, 1.januar, 31.januar, beløp = 32000.månedlig, ARBEIDSGIVER),
                Refusjonsopplysning(inntektsmelding, 1.februar, null, beløp = 32000.månedlig, ARBEIDSGIVER),
            ),
    ...
    fun `deaktivert arbeidsforhold blir med i vilkårsgrunnlag`() {
        ...
        val relevanteOrgnumre1: Iterable<String> =
            hendelselogg.etterspurtBehov(1.vedtaksperiode.id(ORGNUMMER), Behovtype.Godkjenning, "orgnummereMedRelevanteArbeidsforhold")
    ...
    fun `deaktivere arbeidsgiver med kort søknad`() {
        ...
        assertEquals(
            74,
            utbetaling1.utbetalingstidslinje[1.januar]
    ...
    private fun assertInfoSomFølgeAv(
        forventetInfo: String,
        block: () -> Unit,
    ) {
