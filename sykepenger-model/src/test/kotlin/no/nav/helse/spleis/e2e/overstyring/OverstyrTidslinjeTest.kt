...
internal class OverstyrTidslinjeTest : AbstractEndToEndTest() {
    ...
    fun `Sendes ut overstyring i gangsatt når det er en periode som står i avventer inntektsmelding ved endring fra saksbehandler`() {
        ...
        // Når saksbehandler overstyrer tidslinjen forventer Speilvendt å få overstyring igangsatt
        // for å håndhenve totrinns
        håndterOverstyrTidslinje(
            (1.februar til 14.februar).map { ManuellOverskrivingDag(it, Dagtype.Sykedag, 100) }
        )
    ...
    fun `Senere perioder inngår ikke i overstyring igangsatt selv om det er en endring fra saksbehandler`() {
        ...
        val overstyringIgangsatt = observatør.overstyringIgangsatt.single()
        assertEquals(
            listOf(1.vedtaksperiode.id(ORGNUMMER)),
    ...
    fun `overstyring av tidslinje i avventer inntektsmelding`() {
        ...
        assertEquals(
            "UGG UUUUUGG UUUUUGR AAAAARR AAAAARR ASSSSHH SSSSSHH SSSSSHH SSSSSH",
            inspektør.sykdomstidslinje.toShortString(),
        )
        håndterOverstyrTidslinje(
            (20.februar til 24.februar).map { ManuellOverskrivingDag(it, Dagtype.Sykedag, 100) }
        )
        assertEquals(
            "UGG UUUUUGG UUUUUGR ASSSSHR AAAAARR ASSSSHH SSSSSHH SSSSSHH SSSSSH",
            inspektør.sykdomstidslinje.toShortString(),
        )
        håndterOverstyrTidslinje(
            (18.februar til 19.februar).map { ManuellOverskrivingDag(it, Dagtype.Sykedag, 100) }
        )
        assertEquals(
            "UGG UUUUUGG UUUUUGH SSSSSHR AAAAARR ASSSSHH SSSSSHH SSSSSHH SSSSSH",
            inspektør.sykdomstidslinje.toShortString(),
        )
    ...
    fun `arbeidsgiver endrer arbeidsgiverperioden tilbake - må overstyre tidslinje for å fikse`() {
        ...
        assertUgyldigSituasjon(
            "En vedtaksperiode i AVVENTER_INNTEKTSMELDING trenger hjelp fordi FLERE_SKJÆRINGSTIDSPUNKT!"
        ) {
    ...
    fun `overstyre ferie til sykdom`() {
        ...
        håndterInntektsmelding(listOf(1.januar til 16.januar))
        håndterOverstyrTidslinje(
            (17.januar til 31.januar).map { dagen ->
                ManuellOverskrivingDag(dagen, Dagtype.Sykedag, 100)
            }
        )
        håndterVilkårsgrunnlag(1.vedtaksperiode)
        håndterYtelser(1.vedtaksperiode)
        ...
        val vilkårsgrunnlaget =
            inspektør.vilkårsgrunnlag(1.vedtaksperiode) ?: fail { "fant ikke vilkårsgrunnlag" }
        ...
        assertEquals(
            ...
            arbeidsgiverInntektsopplysning.inntektsopplysning::class,
        )
    }
    ...
    fun `ferie uten sykmelding mellom to perioder`() {
        ...
        assertEquals(
            "SHH SSSSSHH SSSSSHH SSSSSHH SSSSSHJ JJJJJJJ JJJJJJJ JJJJJJJ JJJJJJJ JJNSSHH SSSSSHH SSSSSHH SSSSSHH SSSSS",
            inspektør.sykdomstidslinje.toShortString(),
        )
        ...
        augustutbetalingEtter.arbeidsgiverOppdrag.also { oppdrag ->
            ...
            oppdrag[0].also { linje -> assertEquals(17.juni til 29.juni, linje.fom til linje.tom) }
            oppdrag[1].also { linje -> assertEquals(august, linje.fom til linje.tom) }
        }
    }
    ...
    fun `vedtaksperiode strekker seg tilbake og endrer ikke skjæringstidspunktet`() {
        ...
        nullstillTilstandsendringer()
        håndterOverstyrTidslinje(
            listOf(ManuellOverskrivingDag(9.januar, Dagtype.Arbeidsdag)),
            orgnummer = a1,
        )
    ...
    fun `vedtaksperiode strekker seg ikke tilbake hvis det er en periode foran`() {
        ...
        nullstillTilstandsendringer()
        håndterOverstyrTidslinje(
            listOf(ManuellOverskrivingDag(9.januar, Dagtype.Sykedag, 100)),
            orgnummer = a1,
        )
        håndterYtelser(2.vedtaksperiode, orgnummer = a1)
    ...
    fun `kan ikke utbetale overstyrt utbetaling`() {
        ...
        håndterOverstyrTidslinje(
            listOf(
                manuellSykedag(2.januar),
                manuellArbeidsgiverdag(24.januar),
                manuellFeriedag(25.januar),
            )
        )
    ...
    fun `overstyrt til fridager i midten av en periode blir ikke utbetalt`() {
        ...
        håndterSimulering(1.vedtaksperiode)
        håndterOverstyrTidslinje(
            listOf(manuellFeriedag(22.januar), manuellPermisjonsdag(23.januar))
        )
    ...
    fun `Overstyring oppdaterer sykdomstidlinjene`() {
        ...
        assertEquals(
            "PPPPP PPPPPPP PPPPNHH NNNNF",
            inspektør.utbetalingstidslinjer(1.vedtaksperiode).toString(),
        )
    }
    ...
    fun `skal kunne overstyre dagtype i utkast til revurdering ved revurdering av inntekt`() {
        ...
        assertEquals(
            "PPPPPPP PPPPPPP PPNNNFF FFFFFFF FNN",
    ...
    fun `overstyrer fra SykedagNav til Sykedag`() {
        ...
        agp.forEach { assertSykdomstidslinjedag(it, Dag.SykedagNav::class, "Inntektsmelding") }
        ...
        håndterYtelser(1.vedtaksperiode)
        agp.forEach { assertSykdomstidslinjedag(it, Dag.Sykedag::class, OverstyrTidslinje::class) }
        val førsteUtbetalingsdagEtterOverstyring =
            inspektør.utbetaling(1).arbeidsgiverOppdrag[0].fom
    ...
    fun `overstyring av andre ytelser i halen`() {
        ...
        assertEquals(
            "SSSSSHH SSSSSHH SSSSSYY YYYYYYY YYY",
            inspektør.sykdomshistorikk.sykdomstidslinje().toShortString(),
        )
        håndterYtelser(1.vedtaksperiode)
    ...
    fun `overstyring av egenmeldingsdager til arbeidsdager`() {
        ...
        håndterSimulering(2.vedtaksperiode)
        assertEquals(
            "HH SSSSSHH SUUUUGG UAA",
            inspektør.vedtaksperiodeSykdomstidslinje(2.vedtaksperiode).toShortString(),
        )
        håndterOverstyrTidslinje(
            (20..26).map { ManuellOverskrivingDag(it.februar, Dagtype.Arbeidsdag) }
        )
        håndterYtelser(2.vedtaksperiode)
        håndterSimulering(2.vedtaksperiode)
        assertEquals(
            "HH SSSSSHH SAAAARR AAA",
            inspektør.vedtaksperiodeSykdomstidslinje(2.vedtaksperiode).toShortString(),
        )
        håndterUtbetalingsgodkjenning(2.vedtaksperiode)
        håndterUtbetalt()
    ...
    private val PersonObserver.UtbetalingUtbetaltEvent.dager
        get() = utbetalingsdager.associateBy { it.dato }.mapValues { it.value.type }
}
