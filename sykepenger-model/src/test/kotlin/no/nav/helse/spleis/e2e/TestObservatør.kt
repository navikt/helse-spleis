...
internal class TestObservatør(person: Person? = null) : PersonObserver {
    ...
    val trengerArbeidsgiveropplysningerVedtaksperioder =
        mutableListOf<PersonObserver.TrengerArbeidsgiveropplysningerEvent>()
    val trengerIkkeArbeidsgiveropplysningerVedtaksperioder =
        mutableListOf<PersonObserver.TrengerIkkeArbeidsgiveropplysningerEvent>()
    val arbeidsgiveropplysningerKorrigert =
        mutableListOf<PersonObserver.ArbeidsgiveropplysningerKorrigertEvent>()
    ...
    val avsluttetMedVedtakEventer =
        mutableMapOf<UUID, MutableList<PersonObserver.AvsluttetMedVedtakEvent>>()
    ...
    val avsluttetUtenVedtakEventer =
        mutableMapOf<UUID, MutableList<PersonObserver.AvsluttetUtenVedtakEvent>>()
    ...
    val skatteinntekterLagtTilGrunnEventer =
        mutableListOf<PersonObserver.SkatteinntekterLagtTilGrunnEvent>()
    val søknadHåndtert = mutableListOf<Pair<UUID, UUID>>()
    val vedtaksperiodeAnnullertEventer =
        mutableListOf<PersonObserver.VedtaksperiodeAnnullertEvent>()
    val vedtaksperiodeOpprettetEventer = mutableListOf<PersonObserver.VedtaksperiodeOpprettet>()
    val overlappendeInfotrygdperioder =
        mutableListOf<PersonObserver.OverlappendeInfotrygdperioder>()
    val utkastTilVedtakEventer = mutableListOf<PersonObserver.UtkastTilVedtakEvent>()
    val sykefraværstilfelleIkkeFunnet =
        mutableListOf<PersonObserver.SykefraværstilfelleIkkeFunnet>()
    ...
    private val vedtaksperioder =
        person
            ?.inspektør
            ?.vedtaksperioder()
            ?.mapValues { (_, perioder) -> perioder.map { it.inspektør.id }.toMutableSet() }
            ?.toMutableMap() ?: mutableMapOf()

    private val vedtaksperiodeendringer =
        mutableMapOf<UUID, MutableList<VedtaksperiodeEndretEvent>>()

    private val forkastedeEventer =
        mutableMapOf<UUID, PersonObserver.VedtaksperiodeForkastetEvent>()
    ...
    fun sisteVedtaksperiode() = IdInnhenter { orgnummer ->
        vedtaksperioder.getValue(orgnummer).last()
    }
    ...
    fun vedtaksperiode(orgnummer: String, indeks: Int) =
    ...
    override fun vedtaksperiodeEndret(event: VedtaksperiodeEndretEvent) {
        ...
        vedtaksperioder
            ...
            .add(sisteVedtaksperiode)
        tilstandsendringer
            .getOrPut(event.vedtaksperiodeId) { mutableListOf(event.forrigeTilstand) }
            .add(event.gjeldendeTilstand)
        if (event.gjeldendeTilstand == TilstandType.AVSLUTTET)
            ...
            Forespørsel(
                ...
                sykmeldingsperioder =
                    sykmeldingsperioder.map {
                        no.nav.helse.spill_av_im.Periode(it.start, it.endInclusive)
                    },
                egenmeldinger =
    ...
    override fun overlappendeInfotrygdperioder(
    ...
    override fun sykefraværstilfelleIkkeFunnet(
        event: PersonObserver.SykefraværstilfelleIkkeFunnet
    ) {
