...
internal class TestObservatør(person: Person? = null) : PersonObserver {
    ...
    internal val tilstandsendringer =
        person?.inspektør?.sisteVedtaksperiodeTilstander()?.mapValues { mutableListOf(it.value) }
    ...
    fun sisteVedtaksperiode() =
        IdInnhenter { orgnummer -> vedtaksperioder.getValue(orgnummer).last() }
    ...
    override fun vedtaksperiodeEndret(event: VedtaksperiodeEndretEvent) {
        ...
        vedtaksperioder.getOrPut(event.organisasjonsnummer) { mutableSetOf() }
            .add(sisteVedtaksperiode)
        tilstandsendringer.getOrPut(event.vedtaksperiodeId) { mutableListOf(event.forrigeTilstand) }
    ...
    override fun inntektsmeldingReplay(
        ...
        inntektsmeldingReplayEventer.add(
            Forespørsel(
                ...
                førsteFraværsdager = førsteFraværsdager.map {
                    no.nav.helse.spill_av_im.FørsteFraværsdag(
                        it.organisasjonsnummer,
                        it.førsteFraværsdag
                    )
                },
                sykmeldingsperioder = sykmeldingsperioder.map {
                    no.nav.helse.spill_av_im.Periode(
                        it.start,
                        it.endInclusive
                    )
                },
                egenmeldinger = egenmeldingsperioder.map {
                    no.nav.helse.spill_av_im.Periode(
                        it.start,
                        it.endInclusive
                    )
