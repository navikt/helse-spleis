...
internal class InntektsmeldingE2ETest : AbstractEndToEndTest() {
    ...
    fun `Når vedtaksperioden er forkastet skal vi ikke bruke portal-inntektsmeldingen som peker på den`() {
        ...
        assertEquals(
            INNTEKT,
            inspektør.vedtaksperioder(1.vedtaksperiode).refusjonstidslinje[20.januar].beløp,
        )
    ...
    fun `Manglende sporing av IM ved langt gap mellom AGP og FF når IM kommer før søknad`() {
        ...
        assertForventetFeil(
            forklaring =
                "Manglende sporing av IM ved langt gap mellom AGP og FF når IM kommer før søknad",
            nå = {
                assertFalse(inspektør.hendelseIder(1.vedtaksperiode).contains(inntektsmeldingId))
            },
            ønsket = {
                assertTrue(inspektør.hendelseIder(1.vedtaksperiode).contains(inntektsmeldingId))
            },
        )
    }
    ...
    fun `ingen søknad for halen av arbeidsgiverperiode`() {
        ...
        assertForventetFeil(
            forklaring =
                "vi har ikke søknad for halen av AGP. Burde vi strukket 2.vedtaksperiode tilbake?",
            nå = { assertSisteTilstand(2.vedtaksperiode, AVSLUTTET_UTEN_UTBETALING) },
            ønsket = {
                // om vi strekker perioden tilbake så vil det likevel foreligge nytt
                // skjæringstidspunkt
    ...
    fun `tom arbeidsgiverperiode og første fraværsdag dagen efter`() {
        ...
        assertEquals(
            ...
            inspektør.vedtaksperioder(1.vedtaksperiode).inspektør.behandlinger.first().tilstand,
        )
        assertEquals(
            AVSLUTTET_UTEN_VEDTAK,
            inspektør.vedtaksperioder(1.vedtaksperiode).inspektør.behandlinger.last().tilstand,
        )
    }
    ...
    fun `korrigerer arbeidsgiverperiode etter utbetalt`() {
        ...
        assertEquals(
            "AAAAARR AAAAARR AAAAARR AAAASHH SSSSSHH SSSSSHH SSSSSHH SSSSSHH SSS",
        ...
        assertEquals(
            setOf(1.januar, 26.januar),
            inspektør.vilkårsgrunnlaghistorikk().aktiveSpleisSkjæringstidspunkt,
        )
    }
    ...
    fun `uenighet i agp kan delvis bli utbetalt automatisk`() {
        ...
        ) // En periode arbeidsgiver har glemt/ikke fått med seg
        ...
        assertEquals(
            listOf(1.januar til 15.januar, 22.januar til 22.januar),
            inspektør.arbeidsgiverperioder(2.vedtaksperiode),
        )
        assertEquals(
            "PNNNNHH NNNNNHH N",
            inspektør.utbetalingstidslinjer(2.vedtaksperiode).toString(),
        )
        assertVarsel(
            RV_IM_3,
            3.vedtaksperiode.filter(),
        ) // Siste periode som håndterer dager fra IM får varsel (6.feb)
    ...
    fun `arbeidsgiverperiode slutter på fredag, søknad starter mandag`() {
        ...
        håndterYtelser(1.vedtaksperiode)
        assertEquals(
            "UUGG UUUUUGG UUUUU?? SSSSSHH SSS",
    ...
    fun `To inntektsmeldinger krangler om arbeidsgiverperioden`() {
        ...
        håndterInntektsmelding(
            ...
            // Før replay har vi nå gått til avventer inntektsmelding på bakgrunn av denne
            // inntektsmeldingen
            // MEN, ettersom første fraværsdag er satt til 21.mars "treffer" ikke inntekt & refusjon
            // Så vi trenger en annen inntektsmelding som kan gi inntekt og refusjon for 20.mars som
            // nå skal utbetales
            assertTilstander(
                1.vedtaksperiode,
                AVSLUTTET_UTEN_UTBETALING,
    ...
    fun `mange korte perioder som ikke er sykdom`() {
        ...
        assertForventetFeil(
            ...
            nå = {
                ...
                assertEquals(
                    13.februar,
                    inspektør.utbetaling(0).arbeidsgiverOppdrag.first().inspektør.fom,
                )
            },
            ønsket = {
                assertEquals(
    ...
    fun `ulik arbeidsgiverperiode - flere arbeidsgivere`() {
        ...
        assertEquals(
            "UUGR AAAAAGG SSSSSHH SSSSSHH SSSSSHH SSSS",
            inspektør(a1).sykdomshistorikk.sykdomstidslinje().toShortString(),
        )
        assertEquals(
            "AAAAARR AAAAARR AAAAARR AAAA",
    ...
    fun `Periode uten inntekt går ikke videre ved mottatt inntektsmelding`() {
        ...
        assertEquals(
            listOf(1.januar til 6.januar),
            inspektør.arbeidsgiverperioder(1.vedtaksperiode),
        )
        assertEquals(
            listOf(1.januar til 6.januar, 9.januar til 18.januar),
            inspektør.arbeidsgiverperioder(2.vedtaksperiode),
        )
    ...
    fun `arbeidsgiverperiode fra inntektsmelding trumfer ferieopplysninger`() {
        ...
        assertTrue(
            (1.januar til 16.januar).all {
                vedtaksperiode.utbetalingstidslinje[it] is Utbetalingsdag.ArbeidsgiverperiodeDag
            }
        )
    }
    ...
    fun `Inntektsmelding med error som treffer flere perioder`() {
        ...
        håndterInntektsmelding(
            arbeidsgiverperioder =
                listOf(29.mars(2021) til 31.mars(2021), 6.april(2021) til 18.april(2021)),
            beregnetInntekt = INNTEKT,
    ...
    fun `Inntektsmelding med error som treffer flere perioder uten gap`() {
        ...
        håndterInntektsmelding(
            arbeidsgiverperioder =
                listOf(29.mars(2021) til 31.mars(2021), 1.april(2021) til 12.april(2021)),
    ...
    fun `Unngår aggresiv håndtering av arbeidsdager før opplyst AGP ved tidligere revurdering uten endring`() {
        ...
        håndterOverstyrTidslinje(
            (22.januar til 23.januar).map { ManuellOverskrivingDag(it, Permisjonsdag) }
        )
        håndterYtelser(2.vedtaksperiode)
        håndterUtbetalingsgodkjenning(2.vedtaksperiode)
        håndterUtbetalt()
    ...
    fun `inntektsmelding oppgir arbeidsgiverperiode senere`() {
        ...
        assertTrue(
            (1.januar til 7.januar).all {
                tidslinje[it] is Dag.Arbeidsdag || tidslinje[it] is Dag.FriskHelgedag
            }
        )
        assertTrue(
    ...
    fun `Håndterer ikke inntektsmelding to ganger ved replay - hvor vi har en tidligere periode og gap`() {
        // Ved en tidligere periode resettes trimming av inntektsmelding og vi ender med å håndtere
        // samme inntektsmelding flere ganger i en vedtaksperiode
    ...
    fun `Håndterer ikke inntektsmelding to ganger ved replay`() {
        // Happy case av testen med navn: Håndterer ikke inntektsmelding to ganger ved replay - hvor
        // vi har en tidligere periode og gap
        håndterInntektsmelding(
            listOf(10.januar til 25.januar),
            førsteFraværsdag = 10.januar,
            avsendersystem = ALTINN,
    ...
    fun `vilkårsvurdering med flere arbeidsgivere skal ikke medføre at vi går til avventer historikk fra mottatt sykmelding ferdig forlengelse uten IM`() {
                ...
                inntekter =
                    listOf(a1, a2).map { arbeidsgiver ->
                        ArbeidsgiverInntekt(
                            ...
                            (0..2).map {
                                val yearMonth =
                                    YearMonth.from(skjæringstidspunkt).minusMonths(3L - it)
                                ArbeidsgiverInntekt.MånedligInntekt(
                                    yearMonth = yearMonth,
                                    type =
                                        ArbeidsgiverInntekt.MånedligInntekt.Inntekttype
                                            .LØNNSINNTEKT,
                                    inntekt = INNTEKT,
                                    fordel = "fordel",
        ...
        assertForventetFeil(
            ...
            nå = { assertNotNull(inspektør(a1).vilkårsgrunnlag(1.vedtaksperiode)) },
            ønsket = { assertNull(inspektør(a1).vilkårsgrunnlag(1.vedtaksperiode)) },
        )
    }
    ...
    fun `Ny inntektsmelding som treffer AvventerGodkjenning - hensyntar nye refusjonsopplysninger`() {
        ...
        assertEquals(
            17.januar til 31.januar,
            Oppdrag.periode(inspektør.utbetaling(0).arbeidsgiverOppdrag),
        )
    ...
    fun `inntektsmelding oppgir første fraværsdag i en periode med ferie etter sykdom`() {
        ...
        val tidslinje = inspektør.sykdomstidslinje
        assertTrue(
            (17.januar til 31.januar).all {
                tidslinje[it] is Dag.Sykedag || tidslinje[it] is Dag.SykHelgedag
            }
        )
        assertFalse(
            (1.februar til 11.februar).all {
                tidslinje[it] is Dag.Arbeidsdag || tidslinje[it] is Dag.FriskHelgedag
            }
        )
        assertTrue(
            (12.februar til 28.februar).all {
                tidslinje[it] is Dag.Sykedag || tidslinje[it] is Dag.SykHelgedag
            }
        )
    ...
    fun `To tilstøtende perioder, inntektsmelding 2 med arbeidsdager i starten`() {
        ...
        håndterInntektsmelding(
            listOf(
                Periode(3.januar, 7.januar),
                Periode(15.januar, 20.januar),
                Periode(23.januar, 28.januar),
            ),
            vedtaksperiodeIdInnhenter = 2.vedtaksperiode,
        )
    ...
    fun `inntektsmelding oppgir ny arbeidsgiverperiode i en sammenhengende periode`() {
        ...
        // Siden vi tidligere fylte ut 2. vedtaksperiode med arbeidsdager ville vi regne ut et
        // ekstra skjæringstidspunkt i den sammenhengende perioden
    ...
    fun `kaste ut vedtaksperiode hvis arbeidsgiver ikke utbetaler arbeidsgiverperiode med begrunnelse FiskerMedHyre`() {
        ...
        assertInfo(
            "Arbeidsgiver har redusert utbetaling av arbeidsgiverperioden på grunn av: FiskerMedHyre",
            1.vedtaksperiode.filter(),
        )
        assertFunksjonellFeil(
            "Arbeidsgiver har redusert utbetaling av arbeidsgiverperioden",
            1.vedtaksperiode.filter(),
        )
        assertSisteTilstand(1.vedtaksperiode, TIL_INFOTRYGD)
    }
    ...
    fun `Korrigerende inntektsmelding før søknad`() {
        ...
        håndterInntektsmelding(listOf(1.januar til 16.januar))
        // På dette tidspunktet har AUU'en lagret dagene i historikken og innsett at
        // skjæringstidspunktet er 1.januar
        nyPeriode(17.januar til 31.januar)
        // IM 1 replayes først og blir lagret på 2.januar av forlengelsen -> kan ikke beregne
        // sykepengegrunnlag
        // IM 2 replayes deretter og blir lagret på 1.januar av forlengelsen -> kan beregne
        // sykepengegrunnlag og går videre
    ...
    fun `padding med arbeidsdager før arbeidsgiverperioden`() {
        ...
        assertEquals(
            28.februar,
            inspektør.vedtaksperioder(3.vedtaksperiode).inspektør.skjæringstidspunkt,
        )
        ...
        assertTrue(
            beregnetSykdomstidslinjeDager
                .filterKeys { it in 28.januar til førsteDagIArbeidsgiverperioden.minusDays(1) }
                .values
                .all {
                    (it is Dag.Arbeidsdag || it is Dag.FriskHelgedag) &&
                        it.kommerFra("Inntektsmelding")
                }
        ) {
        ...
        assertTrue(
            beregnetSykdomstidslinjeDager
                .filterKeys { it in førsteDagIArbeidsgiverperioden til 31.mars }
                .values
                .all { (it is Dag.Sykedag || it is Dag.SykHelgedag) && it.kommerFra(Søknad::class) }
        ) {
            beregnetSykdomstidslinje.toShortString()
        }
    }
    ...
    fun `arbeidsgiveperiode i forkant av vedtaksperiode med en dags gap`() {
        ...
        assertIngenVarsel(RV_IM_3, 1.vedtaksperiode.filter())
        assertEquals(
            "GG UUUUUGG UUUUUGG ?SSSSHH SSSSSHH SSSSSHH SSS",
            inspektør.sykdomshistorikk.sykdomstidslinje().toShortString(),
        )
        assertEquals(1, inspektør.inntektInspektør.size)
    ...
    fun `inntektsmelding korrigerer periode til godkjenning`() {
        ...
        nullstillTilstandsendringer()
        håndterInntektsmelding(
            listOf(1.januar til 10.januar, 14.januar til 19.januar),
            førsteFraværsdag = 1.mars,
            avsendersystem = ALTINN,
        )
    ...
    fun `Spleis bruker feilaktig en ugyldig egenmeldingsdag i gap-beregning`() {
        ...
        assertForventetFeil(
            ...
            ønsket = {
                assertTrue(
                    (20.februar til 7.mars).all {
                        vedtaksperiode.utbetalingstidslinje[it] is
    ...
    fun `Skal lagre inntekt fra inntektsmelding på datoen som er oppgitt av inntektsdato-feltet`() {
        ...
        val inntektsopplysningVilkårsgrunnlagA2 =
            inspektør(a2)
                ...
                ?.firstOrNull { it.inspektør.orgnummer == a2 }
                ?.inspektør
                ?.inntektsopplysning
        assertTrue(
            inntektsopplysningVilkårsgrunnlagA2 is no.nav.helse.person.inntekt.Inntektsmelding
        )
