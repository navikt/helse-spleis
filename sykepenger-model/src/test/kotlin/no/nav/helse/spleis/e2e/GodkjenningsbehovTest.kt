...
internal class GodkjenningsbehovTest : AbstractEndToEndTest() {
    ...
    private fun omregnedeÅrsinntedekter(vedtaksperiode: IdInnhenter, orgnummer: String = a1) =
        hendelselogg.etterspurtBehov<List<Map<String, Any>>>(
    ...
    private fun sykepengegrunnlag(vedtaksperiode: IdInnhenter, orgnummer: String = a1) =
        hendelselogg.etterspurtBehov<Map<String, Any>>(
            vedtaksperiodeId = vedtaksperiode.id(orgnummer),
            behov = Aktivitet.Behov.Behovtype.Godkjenning,
    ...
    fun `sender med feil vilkårsgrunnlagId i påminnet godkjenningsbehov om det har kommet nytt vilkårsgrunnlag med endring _senere_ enn perioden`() {
        ...
        assertEquals(
            vilkårsgrunnlagId1,
            vilkårsgrunnlagIdFraSisteGodkjenningsbehov(1.vedtaksperiode)
        )
        ...
        ) // Reberegnes ikke ettersom endringen gjelder fra og med 1.mars
        val vilkårsgrunnlagId2 = vilkårsgrunnlagIdFraVilkårsgrunnlaghistorikken(1.januar)
        assertNotEquals(
            vilkårsgrunnlagId1,
            vilkårsgrunnlagId2
        ) // IM lager nytt innslag i vilkårsgrunnlaghistorikken pga nye refusjonsopplysninger
    ...
    fun `godkjenningsbehov som ikke kan avvises blir forsøkt avvist`() {
        ...
        val utbetalingId = UUID.fromString(
            person.personLogg.sisteBehov(Aktivitet.Behov.Behovtype.Godkjenning)
    ...
    private fun kanAvvises(vedtaksperiode: IdInnhenter, orgnummer: String = a1) =
        hendelselogg.etterspurtBehov<Boolean>(
            vedtaksperiodeId = vedtaksperiode.id(orgnummer),
            behov = Aktivitet.Behov.Behovtype.Godkjenning,
            felt = "kanAvvises"
        )!!

    private fun omregnedeÅrsinntekter(vedtaksperiode: IdInnhenter, orgnummer: String = a1) =
        hendelselogg.etterspurtBehov<List<Map<String, Any>>>(
            vedtaksperiodeId = vedtaksperiode.id(orgnummer),
            behov = Aktivitet.Behov.Behovtype.Godkjenning,
            felt = "omregnedeÅrsinntekter"
    ...
    private data class OmregnetÅrsinntektFraGodkjenningsbehov(
        val orgnummer: String,
        val beløp: Inntekt
    )

    private fun vilkårsgrunnlagIdFraSisteGodkjenningsbehov(
        vedtaksperiode: IdInnhenter,
        orgnummer: String = a1
    ) = hendelselogg.etterspurtBehov<String>(
        vedtaksperiodeId = vedtaksperiode.id(orgnummer),
        behov = Aktivitet.Behov.Behovtype.Godkjenning,
        felt = "vilkårsgrunnlagId"
    )!!.let { UUID.fromString(it) }

    private fun vilkårsgrunnlagIdFraVilkårsgrunnlaghistorikken(
        skjæringstidspunkt: LocalDate,
        orgnummer: String = a1
    ) = inspektør(orgnummer).vilkårsgrunnlag(skjæringstidspunkt)!!
        .view().inspektør.vilkårsgrunnlagId
}
