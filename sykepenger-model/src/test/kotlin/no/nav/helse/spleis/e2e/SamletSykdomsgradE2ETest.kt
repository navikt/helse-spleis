...
internal class SamletSykdomsgradE2ETest: AbstractEndToEndTest() {
    ...
    private infix fun Periode.med(grad: Prosentdel) = this to grad
    private infix fun Pair<Periode, Prosentdel>.sykdom(act: ArrangeContext.() -> Unit) =
        arrange(Sykmeldingsperiode(first.start, first.endInclusive, second), act)

    @Test
    fun `avviser dager under 20 prosent`() {
        (1.januar til 20.januar med 19.prosent sykdom {
            håndterSøknad(Søknad.Søknadsperiode.Sykdom(1.januar, 20.januar, 19.prosent))
            håndterInntektsmeldingMedValidering(listOf(Periode(1.januar, 16.januar)))
            håndterYtelser()
            håndterVilkårsgrunnlag()
            håndterYtelser()
            håndterUtbetalingsgodkjenning(true)
        }).assert { inspektør, _ ->
            assertTilstander(
                START,
