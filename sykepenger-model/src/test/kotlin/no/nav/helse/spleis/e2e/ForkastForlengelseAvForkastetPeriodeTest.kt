...
internal class ForkastForlengelseAvForkastetPeriodeTest : AbstractDslTest() {
    ...
    fun `forlenger med forkastet periode hos annen arbeidsgiver`() {
        ...
        a2 {
            håndterSykmelding(Sykmeldingsperiode(18.januar, 16.februar))
            håndterSøknad(18.januar til 16.februar)
    ...
    fun `forlenger tidligere overlappende sykmelding`() {
        a1 {
            forkast(1.januar til 10.januar)
            ...
            assertTrue(observatør.hendelseider(1.vedtaksperiode).isNotEmpty())
            assertTrue(observatør.hendelseider(2.vedtaksperiode).isNotEmpty())
            assertTrue(observatør.hendelseider(3.vedtaksperiode).isNotEmpty())
            assertSisteForkastetTilstand(1.vedtaksperiode, TIL_INFOTRYGD)
    ...
    fun `søknader som overlapper`() {
        a1 {
            ...
            assertTrue(observatør.hendelseider(2.vedtaksperiode).isNotEmpty())
            assertTrue(observatør.hendelseider(3.vedtaksperiode).isNotEmpty())
            assertTrue(observatør.hendelseider(4.vedtaksperiode).isNotEmpty())
            assertTrue(observatør.hendelseider(5.vedtaksperiode).isNotEmpty())
    ...
    fun `forkaster periode som er forlengelse av forkastet periode`() {
        a1 {
            forkast(1.januar til 17.januar)
            assertSisteForkastetTilstand(1.vedtaksperiode, TIL_INFOTRYGD)
    ...
    fun `forkaster periode som er forlengelse av forkastet periode over helg`() {
        a1 {
            forkast(1.januar til 19.januar)
            assertSisteForkastetTilstand(1.vedtaksperiode, TIL_INFOTRYGD)
    ...
    fun `fortsetter å forkaste gjentatte forlengelser av kastede perioder`() {
        a1 {
            forkast(1.januar til 18.januar)
            assertSisteForkastetTilstand(1.vedtaksperiode, TIL_INFOTRYGD)
    ...
    fun `forkaster forlengede vedtak på tvers av arbeidsgivere - a2 overlapper med alle perioder til a1`() {
        ...
        a2 { nyPeriode(15.januar til 15.mars) }
        a1 {
            nyPeriode(mars)
            nyPeriode(februar)
        ...
        a2 { assertSisteForkastetTilstand(1.vedtaksperiode, TIL_INFOTRYGD) }
    }
    ...
    fun `forkaster forlengede og overlappende vedtak på tvers av arbeidsgivere - a2 overlapper med en periode til a1`() {
        ...
        a2 { nyPeriode(10.februar til 20.februar) }
        a1 { nyPeriode(februar) }
        ...
        a2 {
            assertForkastetPeriodeTilstander(1.vedtaksperiode, START, AVVENTER_INNTEKTSMELDING, AVVENTER_AVSLUTTET_UTEN_UTBETALING, AVSLUTTET_UTEN_UTBETALING, TIL_INFOTRYGD)
        }
    }
    ...
    @Test
    fun `forkaster periode dersom forlenger forkastet periode med friskmelding`() {
        a1 {
    ...
    fun `person som kun har helg mellom to sykefraværstilfeller skal ikke få to funksjonelle feil når den kastes ut pga for lite gap`() {
        a1 {
            forkast(5.januar til (fredag den 19.januar))
            nyPeriode(mandag den 22.januar til 31.januar)
            assertForkastetPeriodeTilstander(2.vedtaksperiode, START, TIL_INFOTRYGD)
            assertFunksjonellFeil(RV_SØ_37, AktivitetsloggFilter.Alle)
            assertIngenFunksjonellFeil(RV_SØ_28, AktivitetsloggFilter.Alle)
        }
    }
    ...
    fun `søknad som har mindre enn 20 dagers gap til en forkastet periode og overlapper med en annen forkastet periode skal kun få én funksjonell feil`() {
        a1 {
            ...
            assertForkastetPeriodeTilstander(3.vedtaksperiode, START, TIL_INFOTRYGD)
            assertFunksjonellFeil(RV_SØ_33, AktivitetsloggFilter.Alle)
            assertIngenFunksjonellFeil(RV_SØ_28, AktivitetsloggFilter.Alle)
        }
    }
    ...
    fun `kun én error dersom søknad forlenger forkastet periode og har en forkastet periode som er senere i tid`() {
        a1 {
            ...
            assertFunksjonellFeil(RV_SØ_37, AktivitetsloggFilter.Alle)
            assertIngenFunksjonellFeil(RV_SØ_31, AktivitetsloggFilter.Alle)
        }
    }

    private fun TestPerson.TestArbeidsgiver.forkast(periode: Periode) {
        håndterSøknad(periode)
        håndterAnmodningOmForkasting(sisteVedtaksperiode)
    }
}
