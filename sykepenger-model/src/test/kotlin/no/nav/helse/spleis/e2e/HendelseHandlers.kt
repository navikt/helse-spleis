...
internal fun AbstractEndToEndTest.håndterVilkårsgrunnlag(
            ...
            inntekter =
                listOf(
                    ArbeidsgiverInntekt(
                        ...
                        (0..2).map {
                            val yearMonth =
                                YearMonth.from(inspektør(orgnummer).skjæringstidspunkt(vedtaksperiodeIdInnhenter)).minusMonths(
                                    3L - it,
                                )
                            ArbeidsgiverInntekt.MånedligInntekt(
                                yearMonth = yearMonth,
    ...
    inntekterForOpptjeningsvurdering: InntekterForOpptjeningsvurdering =
        lagStandardInntekterForOpptjeningsvurdering(ORGNUMMER, INNTEKT, inspektør(orgnummer).skjæringstidspunkt(vedtaksperiodeIdInnhenter)),
    ...
    fun assertEtterspurt(behovtype: Behovtype) = assertEtterspurt(Vilkårsgrunnlag::class, behovtype, vedtaksperiodeIdInnhenter, orgnummer)
...
internal fun AbstractEndToEndTest.håndterSimulering(
    ...
    simulering(vedtaksperiodeIdInnhenter, simuleringOK, fnr, orgnummer, simuleringsresultat).forEach { simulering ->
...
internal fun AbstractEndToEndTest.håndterYtelser(
    ...
    fun assertEtterspurt(behovtype: Behovtype) = assertEtterspurt(Ytelser::class, behovtype, vedtaksperiodeIdInnhenter, orgnummer)
...
internal fun AbstractEndToEndTest.håndterUtbetalt(
    ...
    val faktiskUtbetalingId =
        utbetalingId?.toString() ?: person.personLogg
...
private fun AbstractEndToEndTest.førsteUhåndterteUtbetalingsbehov(orgnummer: String): Pair<UUID, List<String>>? {
    ...
    return inspektør(orgnummer)
        ...
        .also {
            require(it.size < 2) {
                "For mange utbetalinger i spill! Er sendt ut godkjenningsbehov for periodene ${it.map { utbetaling ->
                    utbetaling.periode
                }}"
            }
                ...
                RefusjonsopplysningerBuilder()
                    .apply {
                        it.refusjonsopplysninger.forEach { (fom, tom, refusjonsbeløp) ->
                            leggTil(
                                Refusjonsopplysning(meldingsreferanseId, fom, tom, refusjonsbeløp, SAKSBEHANDLER, LocalDateTime.now()),
                                LocalDateTime.now(),
                            )
                        }
                    }.build(),
