...
internal class FlereArbeidsgivereTest : AbstractDslTest() {
    ...
    fun `en gjenskapning fra virkeligheten med foreldrepenger, ferie og en spenstig IM blant annet`() {
        ...
        a1 {
            håndterOverstyrTidslinje(
                (1..28).map { ManuellOverskrivingDag(it.februar, Dagtype.Foreldrepengerdag) }
            )
        }
        a2 {
            håndterOverstyrTidslinje(
        ...
        a1 {
            assertForventetFeil(
                ...
                nå = { assertThrows<IllegalStateException> { håndterSøknad(april) } },
                ønsket = { fail("""\_(ツ)_/¯""") },
            )
        }
    }
    ...
    fun `Forsøker å beregne seg forbi det punktet vi venter på inntektsmelding hos annen arbeidsgiver`() {
        ...
        a1 {
            ...
            håndterVilkårsgrunnlag(
                ...
                inntektsvurderingForSykepengegrunnlag =
                    lagStandardSykepengegrunnlag(
                        listOf(a1 to INNTEKT, a2 to INNTEKT, a3 to INNTEKT),
                        1.januar,
                    ),
                arbeidsforhold =
                    listOf(
            ...
            assertTrue(
                inntekter.single { it.gjelder(a1) }.inspektør.inntektsopplysning
                    is no.nav.helse.person.inntekt.Inntektsmelding
            )
            assertTrue(
                inntekter.single { it.gjelder(a2) }.inspektør.inntektsopplysning
                    is no.nav.helse.person.inntekt.Inntektsmelding
            )
            assertTrue(
                inntekter.single { it.gjelder(a3) }.inspektør.inntektsopplysning
        ...
        a1 {
            ...
            val venterPå =
                observatør.vedtaksperiodeVenter
                    .last { it.vedtaksperiodeId == 1.vedtaksperiode }
                    .venterPå
    ...
    fun `tre arbeidsgivere med flere perioder som overlapper - første periode hos a3 skal i auu`() {
        ...
        a2 { håndterInntektsmelding(listOf(1.januar til 16.januar)) }
        a3 { håndterInntektsmelding(listOf(2.januar til 17.januar)) }
        a3 { håndterSøknad(Sykdom(2.januar, 18.januar, 100.prosent), Ferie(18.januar, 18.januar)) }
        a1 {
            håndterVilkårsgrunnlag(1.vedtaksperiode)
            håndterYtelser(1.vedtaksperiode)
            håndterSimulering(1.vedtaksperiode)
    ...
    fun `mangler refusjonsopplysninger etter overstyring av saksbehander - ikke fravær`() {
        ...
        a1 { håndterInntektsmelding(listOf(7.januar til 22.januar)) }
        a2 {
            håndterInntektsmelding(listOf(7.januar til 22.januar), førsteFraværsdag = 1.februar)
            // denne inntektsmeldingen lagrer refusjonsopplysninger uten første fraværsdag. Uten
            // denne IMen så er testen useless
            håndterInntektsmelding(
        ...
        a2 {
            ...
            håndterSimulering(1.vedtaksperiode)
            håndterOverstyrTidslinje(
                (7.januar til 31.januar).map { ManuellOverskrivingDag(it, Dagtype.Arbeidsdag) }
            )
        }
        a1 {
            håndterYtelser(1.vedtaksperiode)
    ...
    fun `mangler refusjonsopplysninger etter at skjæringstidspunktet flyttes - da gjenbruker vi tidsnære opplysninger`() {
        a1 { nyPeriode(3.januar til onsdag den 17.januar) }
        a2 {
        ...
        a1 {
            ...
            håndterVilkårsgrunnlag(
                2.vedtaksperiode,
                inntektsvurderingForSykepengegrunnlag =
                    lagStandardSykepengegrunnlag(listOf(a1 to INNTEKT, a2 to INNTEKT), 19.januar),
                arbeidsforhold =
                    listOf(
                        Arbeidsforhold(a1, LocalDate.EPOCH, type = Arbeidsforholdtype.ORDINÆRT),
                        Arbeidsforhold(a2, LocalDate.EPOCH, type = Arbeidsforholdtype.ORDINÆRT),
        ...
        a1 { nyPeriode(24.januar til 31.januar) }
        a2 {
    ...
    fun `replay av inntektsmelding medfører at to perioder går til godkjenning samtidig`() {
        ...
        a2 { assertSisteTilstand(1.vedtaksperiode, AVVENTER_HISTORIKK) }
    }
    ...
    fun `Beholder skatt i sykepengegrunnlag ved inntektsmelding i annen måned enn skjæringstidspunktet`() {
        ...
        a1 {
            ...
            assertSisteTilstand(1.vedtaksperiode, AVSLUTTET)
            assertTrue(
                inspektør.inntektsopplysning(1.vedtaksperiode, a1) is InntektFraInntektsmelding
            )
    ...
    fun `kort sykdom hos ag2`() {
        ...
        a1 { håndterSykmelding(15.januar til 31.januar) }
        a2 { assertSisteTilstand(1.vedtaksperiode, AVSLUTTET_UTEN_UTBETALING) }
        a1 {
    ...
    fun `bruddstykker hos arbeidsgivere skal ikke medføre at alle perioder må ha inntekt`() {
        ...
        a2 {
            assertSisteTilstand(
                1.vedtaksperiode,
                AVVENTER_BLOKKERENDE_PERIODE,
            ) // sitter fast her fordi overlappende periode hos arbeidsgiver 1 mangler
        ...
        a2 {
            ...
            håndterInntektsmelding(
                arbeidsgiverperioder =
                    listOf(1.januar(2021) til 3.januar(2021), 6.januar(2021) til 18.januar(2021)),
    ...
    fun `medlemskap ikke oppfyllt i vilkårsgrunnlag, avviser perioden riktig for begge arbeidsgivere`() {
        val periode = januar
        nyPeriode(periode, a1, a2)
        a1 { håndterInntektsmelding(listOf(Periode(periode.start, periode.start.plusDays(15)))) }
        a2 { håndterInntektsmelding(listOf(Periode(periode.start, periode.start.plusDays(15)))) }
        a1 {
            håndterVilkårsgrunnlag(
                1.vedtaksperiode,
    ...
    fun `Sykmelding og søknad kommer for to perioder før inntektsmelding kommer - skal fortsatt vilkårsprøve kun én gang`() {
        ...
        // Sender med en annen inntekt enn i forrige IM for å kunne asserte på at det er denne vi
        // bruker
        a2 {
            håndterInntektsmelding(
                listOf(1.januar til 16.januar),
    ...
    fun `kastes ikke ut pga manglende inntekt etter inntektsmelding`() {
        ...
        a2 { håndterSøknad(3.januar til 18.januar) }
        a1 {
    ...
    fun `inntektsmelding skal kun treffe sammenhengende vedtaksperioder, ikke alle med samme skjæringstidspunkt`() {
        // Vedtaksperiode for AG 1 skal bare koble sammen to vedtaksperioder for AG 2 så de får
        // samme skjæringstidspunkt
        a1 { håndterSykmelding(januar) }
        a2 {
            håndterSykmelding(1.januar til 20.januar)
    ...
    fun `skal ikke ha to vedtaksperioder til godkjenning samtidig`() {
        ...
        a1 {
            inspektør.utbetalinger(2.vedtaksperiode).last().inspektør.utbetalingstidslinje.also {
