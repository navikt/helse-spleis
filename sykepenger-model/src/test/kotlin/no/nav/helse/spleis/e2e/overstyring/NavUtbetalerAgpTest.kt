...
internal class NavUtbetalerAgpTest : AbstractEndToEndTest() {
    ...
    fun `skal aldri foreslå sykedag NAV ved en hullete arbedisgiverperiode og begrunnelse for reduksjon satt`() {
        ...
        assertEquals(
            "GR AASSSHH SSSSSHH SSSSSHH SSSSSHH S?????? ?SSSSH",
    ...
    fun `begrunnelse satt med første fraværsdag og hullete arbeidsgiverperiode kan behandles`() {
        ...
        assertEquals(
            "GR AASSSHH SSSSSHH SSSSSHH SSSSSHH S?????? ?NSSSH",
            inspektør.sykdomstidslinje.toShortString(),
        )
        assertSisteTilstand(1.vedtaksperiode, AVSLUTTET_UTEN_UTBETALING)
        assertTilstander(
    ...
    fun `kort periode så gap til neste - korrigert inntektsmelding opplyser om ikke-utbetalt AGP`() {
        ...
        assertEquals(
            "SSSSSHH SSSSSHH SU???HH SSSSSHH SS",
            inspektør.sykdomstidslinje.toShortString(),
        )
    ...
    fun `hullete AGP sammen med begrunnelse for reduksjon blir kastet ut foreløpig`() {
        ...
        assertEquals(
            setOf(Dokumentsporing.søknad(søknad), Dokumentsporing.inntektsmeldingDager(im)),
    ...
    fun `Overstyrer agp til sykedagNav - ingen refusjon`() {
        ...
        håndterOverstyrTidslinje(
            (1.januar til 16.januar).map { dagen ->
                ManuellOverskrivingDag(dagen, Dagtype.SykedagNav, 100)
            }
        )
        håndterYtelser(1.vedtaksperiode)
        håndterSimulering(1.vedtaksperiode)
    ...
    fun `Overstyrer sykedagNav tilbake til vanlig agp`() {
        ...
        (1.januar til 16.januar).forEach {
            if (!it.erHelg())
                assertTrue(dagerFør.getValue(it).kommerFra("Inntektsmelding")) {
                    "$it kommer ikke fra Inntektsmelding"
                }
            assertTrue(dagerEtter.getValue(it).kommerFra(OverstyrTidslinje::class)) {
                "$it kommer ikke fra OverstyrTidslinje"
            }
        }
    ...
    fun `arbeidsgiver ikke utbetalt i arbeidsgiverperiode på grunn av manglende opptjening`() {
        ...
        assertEquals(
            SykedagNav::class,
            inspektør.sykdomshistorikk.sykdomstidslinje()[1.januar]::class,
        )
        assertTrue(
            inspektør.sykdomshistorikk.sykdomstidslinje()[1.januar].kommerFra("Inntektsmelding")
        )
        håndterVilkårsgrunnlag(1.vedtaksperiode)
        håndterYtelser(1.vedtaksperiode)
        håndterSimulering(1.vedtaksperiode)
    ...
    fun `kort periode etter ferie uten sykdom med arbeidsgiverperioden spredt litt utover`() {
        nyttVedtak(juni, arbeidsgiverperiode = listOf(1.juni til 5.juni, 8.juni til 18.juni))
        håndterSøknad(Sykdom(1.august, 10.august, 100.prosent))
        nullstillTilstandsendringer()
        håndterInntektsmelding(
    ...
    fun `Inntektsmelding med begrunnelseForReduksjonEllerIkkeUtbetalt må forkaste alle perioder med samme arbeidsgiverperiode`() {
        ...
        assertSisteTilstand(3.vedtaksperiode, AVVENTER_INNTEKTSMELDING)
        assertEquals(
            "UUUARR AAAAARH SSSSSHH SSSSS?? ??????? ???SSHH",
    ...
    fun `avviser dager nav skal utbetale i arbeidsgiverperioden om sykdomsgrad er for lav`() {
        ...
        assertUtbetalingsdag(
            inspektør.sisteUtbetalingUtbetalingstidslinje()[2.januar],
            expectedDagtype = Utbetalingsdag.AvvistDag::class,
            11,
        )
    }
    ...
    fun `eget varsel ved oppgitt begrunnelse FerieEllerAvspasering`() {
        ...
        håndterOverstyrTidslinje(
            (1..31).map { ManuellOverskrivingDag(it.juli, Dagtype.ArbeidIkkeGjenopptattDag) } +
                listOf(ManuellOverskrivingDag(1.august, Dagtype.Sykedag, 50))
        )
        håndterYtelser(2.vedtaksperiode)
        håndterSimulering(2.vedtaksperiode)
        assertEquals(
            "SHH SSSSSHH SSSSSHH SSSSSHH SSSSSHJ JJJJJJJ JJJJJJJ JJJJJJJ JJJJJJJ JJSSSHH SSSSSHH SSSSSHH SSSSSHH SSSSS",
            inspektør.sykdomshistorikk.sykdomstidslinje().toShortString(),
        )
    }
    ...
    fun `avviser dager nav skal utbetale i arbeidsgiverperioden om kravet til minsteinntekt ikke er innfridd`() {
        ...
        (1.januar til 16.januar)
            .filterNot { it.erHelg() }
            .forEach { assertTrue(inspektør.sykdomstidslinje[it] is SykedagNav) }
        (17.januar til 31.januar)
            .filterNot { it.erHelg() }
            .forEach { assertTrue(inspektør.sykdomstidslinje[it] is Sykedag) }
        (januar)
            .filterNot { it.erHelg() }
            .forEach {
                assertNotNull(
                    inspektør
                        .utbetalingstidslinjer(1.vedtaksperiode)[it]
                        .erAvvistMed(Begrunnelse.MinimumInntekt)
                )
            }
    }
}
