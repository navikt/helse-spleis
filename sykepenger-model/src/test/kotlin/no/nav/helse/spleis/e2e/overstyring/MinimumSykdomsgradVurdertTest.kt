...
internal class MinimumSykdomsgradVurdertTest : AbstractEndToEndTest() {
    ...
    fun `Saksbehandler overstyrer avslag pga minimum sykdomsgrad`() {
        ...
        assertEquals(
            11,
            inspektør.utbetalinger(1.vedtaksperiode)
                .last().inspektør.utbetalingstidslinje.inspektør.navdager.size
        )
        assertTrue(
            inspektør.utbetalinger(1.vedtaksperiode)
                .last().inspektør.utbetalingstidslinje.inspektør.navdager.all { it.økonomi.inspektør.totalGrad == 10 })
    }
    ...
    fun `Får søknad fra ghost etter at minimum sykdomsgrad er vurdert`() {
        ...
        assertEquals(
            10,
            inspektør.sisteUtbetaling().utbetalingstidslinje[17.januar].økonomi.inspektør.totalGrad
        )
    ...
    fun `Vurderer samme måned til å være både ok og ikke ok`() {
        ...
        assertThrows<IllegalStateException> {
            håndterMinimumSykdomsgradVurdert(
                perioderMedMinimumSykdomsgradVurdertOK = listOf(januar),
