...
internal class InntektsmeldingPåKjølTest : AbstractEndToEndTest() {
    ...
    fun `publiserer event når vi mottar inntektsmelding uten at det fins perioder`() {
        val hendelseIdInntektsmelding = UUID.randomUUID()
        håndterInntektsmelding(listOf(Periode(fom = 1.januar, tom = 16.januar)), id = hendelseIdInntektsmelding)

        assertEquals(hendelseIdInntektsmelding, observatør.inntektsmeldingerLagtPåKjøl[0])
    }
    ...
    fun `publiserer event når inntektsmelding ikke treffer noen periode`() {
        ...
        håndterInntektsmelding(listOf(Periode(fom = 1.mars, tom = 16.mars)), id = hendelseIdInntektsmelding)
