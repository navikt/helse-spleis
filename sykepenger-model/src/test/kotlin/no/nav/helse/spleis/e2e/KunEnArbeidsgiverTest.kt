...
import no.nav.helse.sykdomstidslinje.NyDag.NySykHelgedag
import no.nav.helse.sykdomstidslinje.NyDag.NySykedag
...
internal class KunEnArbeidsgiverTest : AbstractEndToEndTest() {
    ...
    fun `ingen historie med inntektsmelding først`() {
        ...
        inspektør.also {
            ...
            assertEquals(18, it.dagtelling[NySykedag::class])
    ...
    fun `ingen historie med søknad til arbeidsgiver først`() {
        ...
        inspektør.also {
            ...
            assertEquals(2, it.dagtelling[NySykHelgedag::class])
        }
        assertTilstander(
            0,
            START,
    ...
    fun `ingen historie med to søknader til arbeidsgiver før inntektsmelding`() {
        ...
        inspektør.also {
            ...
            assertEquals(INNTEKT.toBigDecimal(), it.inntektshistorikk.inntekt(2.januar))
            assertEquals(3, it.sykdomshistorikk.size)
            assertEquals(4, it.dagtelling[NySykHelgedag::class])
            assertEquals(14, it.dagtelling[NySykedag::class])
        }
        assertTilstander(
            0,
            START,
    ...
    fun `ingen historie med to søknader (med gap mellom) til arbeidsgiver først`() {
        ...
        inspektør.also {
            ...
            assertEquals(3, it.sykdomshistorikk.size)
            assertEquals(4, it.dagtelling[NySykHelgedag::class])
            assertEquals(13, it.dagtelling[NySykedag::class])
        }
        assertTilstander(
            0,
            START,
    ...
    fun `søknad sendt etter 3 mnd`() {
        ...
        inspektør.also {
            ...
            assertEquals(INNTEKT.toBigDecimal(), it.inntektshistorikk.inntekt(2.januar))
            assertEquals(3, it.sykdomshistorikk.size)
            assertNull(it.dagtelling[NySykedag::class])
            assertEquals(6, it.dagtelling[NySykHelgedag::class])
    ...
    fun `søknad til arbeidsgiver etter inntektsmelding`() {
        ...
        inspektør.also {
            assertNoErrors(it)
            assertMessages(it)
            assertEquals(38, it.dagtelling[NySykedag::class])
            assertEquals(14, it.dagtelling[NySykHelgedag::class])
        }
        assertTilstander(
            0,
            START, MOTTATT_SYKMELDING_FERDIG_GAP, AVVENTER_SØKNAD_FERDIG_GAP
