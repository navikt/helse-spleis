...
internal class NødnummerE2ETest : AbstractEndToEndTest() {
    ...
    fun `slår ikke ut på tidligere utbetaling på nødnummer`() {
        håndterUtbetalingshistorikkEtterInfotrygdendring(PersonUtbetalingsperiode(nødnummer, 1.januar, 14.januar, 100.prosent, 500.daglig), inntektshistorikk = listOf(Inntektsopplysning(nødnummer, 1.januar, 500.daglig, false)))
        håndterSykmelding(Sykmeldingsperiode(1.februar, 28.februar))
        håndterSøknadMedValidering(1.vedtaksperiode, Sykdom(1.februar, 28.februar, 100.prosent))
        håndterInntektsmeldingMedValidering(1.vedtaksperiode, listOf(1.februar til 16.februar))
        håndterVilkårsgrunnlag(1.vedtaksperiode)
    ...
    fun `slår ikke ut på inntektsopplysning med nødnummer`() {
        håndterUtbetalingshistorikkEtterInfotrygdendring(ArbeidsgiverUtbetalingsperiode(ORGNUMMER, 17.januar, 18.januar, 100.prosent, 500.daglig), inntektshistorikk = listOf(
    ...
    fun `slår ikke ut på utbetaling med nødnummer innenfor samme agp`() {
        håndterUtbetalingshistorikkEtterInfotrygdendring(PersonUtbetalingsperiode(nødnummer, 17.januar, 18.januar, 100.prosent, 500.daglig), inntektshistorikk = listOf(Inntektsopplysning(nødnummer, 17.januar, 500.daglig, false)))
        håndterSykmelding(Sykmeldingsperiode(1.februar, 28.februar))
        håndterSøknadMedValidering(1.vedtaksperiode, Sykdom(1.februar, 28.februar, 100.prosent))
        håndterInntektsmeldingMedValidering(1.vedtaksperiode, listOf(1.januar til 16.januar), førsteFraværsdag = 1.februar)
        håndterVilkårsgrunnlag(1.vedtaksperiode)
