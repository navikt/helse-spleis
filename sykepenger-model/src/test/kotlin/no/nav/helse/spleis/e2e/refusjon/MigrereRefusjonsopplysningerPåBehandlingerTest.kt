...
internal class MigrereRefusjonsopplysningerPåBehandlingerTest : AbstractDslTest() {
    ...
    fun `uberegnet forlengelse når det mottas informasjon om opphør - uten toggle`() =
        LagreRefusjonsopplysningerPåBehandling.disable {
            a1 {
                ...
                inspektør.vedtaksperioder(2.vedtaksperiode).refusjonstidslinje.assertBeløpstidslinje(
                    1.februar til 10.februar to INNTEKT,
                    11.februar til 28.februar to INGEN
                )
            }
    ...
    fun `flere arbeidsgivere med bare en inntektsmelding- med toggle`() =
        LagreRefusjonsopplysningerPåBehandling.enable {
            a1 {
                setup13og14()
                forrigeRefusjonstidslinje =
                    inspektør.vedtaksperioder(1.vedtaksperiode).refusjonstidslinje + inspektør.vedtaksperioder(
                        2.vedtaksperiode
                    ).refusjonstidslinje
    ...
    private fun Beløpstidslinje.assertBeløpstidslinje(vararg forventetBeløp: Pair<Periode, Inntekt>) {
        forventetBeløp.forEach { (periode, inntekt) ->
            ...
            assertEquals(
                periode,
                subset.perioderMedBeløp.singleOrNull()
