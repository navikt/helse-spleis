...
internal fun AbstractEndToEndTest.assertEtterspurt(
    ...
    val etterspurtBehov = EtterspurtBehov.finnEtterspurtBehov(
        ikkeBesvarteBehov,
        type,
        vedtaksperiodeIdInnhenter,
        orgnummer
...
internal fun AbstractEndToEndTest.assertUtbetalingsbeløp(
    ...
    val utbetalingstidslinje = inspektør(orgnummer).utbetalingstidslinjer(vedtaksperiodeIdInnhenter)
        .let { subset?.let(it::subset) ?: it }
...
internal fun AbstractEndToEndTest.assertHarHendelseIder(
    ...
    assertTrue(
        inspektør(orgnummer).hendelseIder(vedtaksperiodeIdInnhenter)
...
internal fun AbstractEndToEndTest.assertHarIkkeHendelseIder(
    ...
    assertTrue(
        inspektør(orgnummer).hendelseIder(vedtaksperiodeIdInnhenter)
            .none { it in hendelseIder.toList() }) { "Perioden inneholder ikke alle hendelseidene" }
}
...
internal fun AbstractEndToEndTest.assertTilstander(
    ...
    vararg tilstander: TilstandType,
    orgnummer: String = AbstractPersonTest.ORGNUMMER,
    inspektør: TestArbeidsgiverInspektør = inspektør(
        orgnummer
    ),
    message: String? = null
) {
...
internal fun interface AktivitetsloggFilter {
    companion object {
        ...
        internal fun vedtaksperiode(
            ...
            kontekst.kontekstType == "Vedtaksperiode" && kontekst.kontekstMap["vedtaksperiodeId"] == idInnhenter.id(
                orgnummer
            ).toString()
        }

        internal fun person(personidentifikator: Personidentifikator = AbstractPersonTest.UNG_PERSON_FNR_2018): AktivitetsloggFilter =
            AktivitetsloggFilter { kontekst ->
