...
): Boolean = EtterspurtBehov.finnEtterspurtBehov(ikkeBesvarteBehov, type, vedtaksperiodeIdInnhenter, orgnummer, tilstand) != null
...
internal fun AbstractEndToEndTest.assertUtbetalingsbeløp(
    ...
    utbetalingstidslinje.filterNot { it.dato.erHelg() }.forEach { utbetalingsdag ->
        assertEquals(forventetArbeidsgiverbeløp.daglig, utbetalingsdag.økonomi.inspektør.arbeidsgiverbeløp) {
            "feil arbeidsgiverbeløp for dag ${utbetalingsdag.dato} "
        }
        assertEquals(
            forventetArbeidsgiverRefusjonsbeløp.daglig,
            utbetalingsdag.økonomi.inspektør.arbeidsgiverRefusjonsbeløp
                .rundTilDaglig(),
        )
...
internal fun AbstractEndToEndTest.assertHarIkkeHendelseIder(
    ...
    assertTrue(
        inspektør(orgnummer).hendelseIder(vedtaksperiodeIdInnhenter).none { it in hendelseIder.toList() },
    ) { "Perioden inneholder ikke alle hendelseidene" }
}
...
internal fun AbstractPersonTest.assertInfo(
    forventet: String,
...
internal fun Aktivitetslogg.assertInfo(
    forventet: String,
    vararg filtre: AktivitetsloggFilter,
) {
...
internal fun Aktivitetslogg.assertVarsel(
    warning: String,
    vararg filtre: AktivitetsloggFilter,
) {
    val warnings = collectVarsler(*filtre)
...
internal fun Aktivitetslogg.assertLogiskFeil(
    severe: String,
    vararg filtre: AktivitetsloggFilter,
) {
