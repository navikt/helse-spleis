...
internal fun assertInntektshistorikkForDato(
    ...
    assertEquals(
        forventetInntekt,
        inspektør.inntektInspektør.omregnetÅrsinntekt(dato)?.sykepengegrunnlag,
    )
}

internal fun assertInntektForDato(
    forventetInntekt: Inntekt?,
    dato: LocalDate,
    inspektør: TestArbeidsgiverInspektør,
) {
    ...
    sykepengegrunnlagInspektør.arbeidsgiverInntektsopplysningerPerArbeidsgiver[inspektør.orgnummer]
        ?.inspektør
        .also { assertEquals(forventetInntekt, it?.inntektsopplysning?.inspektør?.beløp) }
}

internal fun AbstractEndToEndTest.erEtterspurt(
    type: Aktivitet.Behov.Behovtype,
    vedtaksperiodeIdInnhenter: IdInnhenter,
    orgnummer: String,
    tilstand: TilstandType,
): Boolean {
    return EtterspurtBehov.finnEtterspurtBehov(
        ikkeBesvarteBehov,
        type,
...
internal fun AbstractEndToEndTest.assertEtterspurt(
    løsning: KClass<out Hendelse>,
    type: Aktivitet.Behov.Behovtype,
    vedtaksperiodeIdInnhenter: IdInnhenter,
    orgnummer: String,
) {
    val etterspurtBehov =
        EtterspurtBehov.finnEtterspurtBehov(
            ikkeBesvarteBehov,
            type,
            vedtaksperiodeIdInnhenter,
            orgnummer,
...
internal fun AbstractEndToEndTest.assertIkkeEtterspurt(
    løsning: KClass<out Hendelse>,
    type: Aktivitet.Behov.Behovtype,
    vedtaksperiodeIdInnhenter: IdInnhenter,
    orgnummer: String,
...
internal inline fun <reified R : Utbetalingsdag> assertUtbetalingsdag(
    dag: Utbetalingsdag,
...
internal fun AbstractEndToEndTest.assertUtbetalingsbeløp(
    ...
    val utbetalingstidslinje =
        inspektør(orgnummer).utbetalingstidslinjer(vedtaksperiodeIdInnhenter).let {
            subset?.let(it::subset) ?: it
        }
...
internal fun AbstractEndToEndTest.assertHarHendelseIder(
    ...
    assertTrue(
        inspektør(orgnummer)
            .hendelseIder(vedtaksperiodeIdInnhenter)
            .containsAll(hendelseIder.toList())
    ) {
...
internal fun AbstractEndToEndTest.assertHarIkkeHendelseIder(
    ...
    assertTrue(
        inspektør(orgnummer).hendelseIder(vedtaksperiodeIdInnhenter).none {
            it in hendelseIder.toList()
        }
    ) {
        "Perioden inneholder ikke alle hendelseidene"
    }
...
internal fun AbstractEndToEndTest.assertSisteTilstand(
    vedtaksperiodeIdInnhenter: IdInnhenter,
    tilstand: TilstandType,
    orgnummer: String = AbstractPersonTest.ORGNUMMER,
...
internal fun AbstractEndToEndTest.assertTilstander(
    indeks: Int,
    vararg tilstander: TilstandType,
    orgnummer: String = AbstractPersonTest.ORGNUMMER,
) {
    assertTilstander(
        vedtaksperiodeIdInnhenter = (indeks + 1).vedtaksperiode,
...
internal fun AbstractEndToEndTest.assertTilstander(
    ...
    vararg tilstander: TilstandType,
    orgnummer: String = AbstractPersonTest.ORGNUMMER,
    inspektør: TestArbeidsgiverInspektør = inspektør(orgnummer),
    message: String? = null,
) {
    val id = vedtaksperiodeIdInnhenter.id(orgnummer)
    assertFalse(inspektør.periodeErForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er forkastet med tilstander: ${observatør.tilstandsendringer[id]}:\n${person.personLogg}"
    }
    assertTrue(inspektør.periodeErIkkeForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er forkastet med tilstander: ${observatør.tilstandsendringer[id]}\n${person.personLogg}"
    }
    assertEquals(tilstander.asList(), observatør.tilstandsendringer[id], message)
}

internal fun AbstractEndToEndTest.assertForkastetPeriodeTilstander(
    indeks: Int,
    vararg tilstander: TilstandType,
    orgnummer: String = AbstractPersonTest.ORGNUMMER,
) {
    assertForkastetPeriodeTilstander(
        vedtaksperiodeIdInnhenter = indeks.vedtaksperiode,
        tilstander = tilstander,
        orgnummer = orgnummer,
    )
}
...
internal fun AbstractEndToEndTest.assertForkastetPeriodeTilstander(
    ...
    assertTrue(inspektør.periodeErForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er ikke forkastet"
    }
    assertFalse(inspektør.periodeErIkkeForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er ikke forkastet"
    }
    assertEquals(
        tilstander.asList(),
        observatør.tilstandsendringer[vedtaksperiodeIdInnhenter.id(orgnummer)],
    )
...
internal fun AbstractEndToEndTest.assertForkastetPeriodeTilstander(
    ...
    assertTrue(inspektør(orgnummer).periodeErForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er ikke forkastet"
    }
    assertFalse(inspektør(orgnummer).periodeErIkkeForkastet(vedtaksperiodeIdInnhenter)) {
        "Perioden er ikke forkastet"
    }
    assertEquals(
        tilstander.asList(),
...
internal fun AbstractEndToEndTest.assertSisteForkastetPeriodeTilstand(
    orgnummer: String,
    vedtaksperiodeIdInnhenter: IdInnhenter,
    tilstand: TilstandType,
) {
...
internal fun AbstractPersonTest.assertInfo(forventet: String, vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertInfo(forventet, *filtre)

internal fun AbstractPersonTest.assertIngenInfo(
    forventet: String,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertIngenInfo(forventet, *filtre)

internal fun AbstractPersonTest.assertIngenVarsler(vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertIngenVarsler(*filtre)

internal fun AbstractPersonTest.assertVarsler(vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertVarsler(*filtre)

internal fun AbstractPersonTest.assertVarsler(
    warnings: List<String>,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertVarsler(warnings, *filtre)

internal fun AbstractPersonTest.assertVarsel(warning: String, vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertVarsel(warning, *filtre)

internal fun AbstractPersonTest.assertVarsel(
    varsel: Varselkode,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertVarsel(varsel, *filtre)

internal fun AbstractPersonTest.assertIngenVarsel(
    warning: String,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertIngenVarsel(warning, *filtre)

internal fun AbstractPersonTest.assertIngenVarsel(
    varselkode: Varselkode,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertIngenVarsel(varselkode, *filtre)

internal fun AbstractPersonTest.assertFunksjonellFeil(
    error: String,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertFunksjonellFeil(error, *filtre)

internal fun AbstractPersonTest.assertFunksjonellFeil(
    varselkode: Varselkode,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertFunksjonellFeil(varselkode.funksjonellFeilTekst, *filtre)

internal fun AbstractPersonTest.assertFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertFunksjonelleFeil(*filtre)

internal fun AbstractPersonTest.assertIngenFunksjonellFeil(
    varselkode: Varselkode,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertIngenFunksjonellFeil(varselkode, *filtre)

internal fun AbstractPersonTest.assertIngenFunksjonelleFeil(vararg filtre: AktivitetsloggFilter) =
    person.personLogg.assertIngenFunksjonelleFeil(*filtre)

internal fun AbstractPersonTest.assertLogiskFeil(
    severe: String,
    vararg filtre: AktivitetsloggFilter,
) = person.personLogg.assertLogiskFeil(severe, *filtre)

internal fun Aktivitetslogg.assertInfo(forventet: String, vararg filtre: AktivitetsloggFilter) {
    val info = collectInfo(*filtre)
    assertTrue(
        info.contains(forventet),
        "fant ikke ett tilfelle av infomelding <$forventet>. Info:\n${info.joinToString("\n")}",
    )
}

internal fun Aktivitetslogg.assertIngenInfo(
    forventet: String,
    vararg filtre: AktivitetsloggFilter,
) {
    val info = collectInfo(*filtre)
...
internal fun Aktivitetslogg.assertVarsler(
    warnings: List<String>,
    vararg filtre: AktivitetsloggFilter,
) {
    assertVarsler(*filtre)
    val ekteWarnings = collectVarsler(*filtre)
    assertTrue(warnings.containsAll(ekteWarnings)) {
        "Forventet ikke warnings: ${ekteWarnings.filterNot { it in warnings }}"
    }
}
...
internal fun Aktivitetslogg.assertVarsel(varsel: Varselkode, vararg filtre: AktivitetsloggFilter) {
    ...
    assertTrue(
        varselkoder.contains(varsel),
        "\nFant ikke forventet varselkode:\n\t$varsel\nVarselkoder funnet:\n\t${varselkoder.joinToString("\n\t")}\n",
    )
}

internal fun Aktivitetslogg.assertIngenVarsel(
    warning: String,
    vararg filtre: AktivitetsloggFilter,
) {
    val warnings = collectVarsler(*filtre)
...
internal fun Aktivitetslogg.assertIngenVarsel(
    ...
    val varselkoder = collectVarselkoder(*filtre)
    assertFalse(
        varselkoder.contains(varselkode),
        "\nFant en varselkode vi ikke forventet:\n\t$varselkode\nVarselkoder funnet:\n\t${varselkoder.joinToString("\n\t")}\n",
    )
}

internal fun Aktivitetslogg.assertHarTag(
    vedtaksperiode: IdInnhenter,
    orgnummer: String = ORGNUMMER,
    forventetTag: String,
) {
    val tags =
        this.etterspurtBehov<Set<String>>(
            vedtaksperiodeId = vedtaksperiode.id(orgnummer),
            behov = Aktivitet.Behov.Behovtype.Godkjenning,
            felt = "tags",
        )
    assertTrue(
        tags?.contains(forventetTag) ?: false,
        "Fant ikke forventet tag: $forventetTag. Faktiske tags: $tags ",
    )
}

internal fun Aktivitetslogg.assertFunksjonellFeil(
    error: String,
    vararg filtre: AktivitetsloggFilter,
) {
...
internal fun Aktivitetslogg.assertFunksjonellFeil(
    ...
    val errors = collectFunksjonelleFeil(*filtre)
    assertTrue(
        errors.contains(varselkode.funksjonellFeilTekst),
        "fant ikke forventet error. Errors:\n${errors.joinToString("\n")}",
    )
}
...
internal fun Aktivitetslogg.assertIngenFunksjonellFeil(
    varselkode: Varselkode,
    vararg filtre: AktivitetsloggFilter,
) {
    val errors = collectFunksjonelleFeil(*filtre)
    assertFalse(
        errors.contains(varselkode.varseltekst),
        "\nFant en varselkode vi ikke forventet:\n\t$varselkode\nVarselkoder funnet:\n\t${errors.joinToString("\n\t")}\n",
    )
}
...
internal fun Aktivitetslogg.collectVarselkoder(
    vararg filtre: AktivitetsloggFilter
): List<Varselkode> {
    return varsel
        .filter { filtre.all { filter -> it.kontekster.any { filter.filtrer(it) } } }
...
internal fun Aktivitetslogg.collectFunksjonelleFeil(
    vararg filtre: AktivitetsloggFilter
): List<String> {
...
internal fun interface AktivitetsloggFilter {
    companion object {
        ...
        internal fun vedtaksperiode(vedtaksperiodeId: UUID): AktivitetsloggFilter =
            AktivitetsloggFilter { kontekst ->
                kontekst.kontekstType == "Vedtaksperiode" &&
                    kontekst.kontekstMap["vedtaksperiodeId"] == vedtaksperiodeId.toString()
            }

        internal fun vedtaksperiode(
            idInnhenter: IdInnhenter,
            orgnummer: String,
        ): AktivitetsloggFilter = AktivitetsloggFilter { kontekst ->
            kontekst.kontekstType == "Vedtaksperiode" &&
                kontekst.kontekstMap["vedtaksperiodeId"] == idInnhenter.id(orgnummer).toString()
        }

        internal fun person(
            personidentifikator: Personidentifikator = AbstractPersonTest.UNG_PERSON_FNR_2018
        ): AktivitetsloggFilter = AktivitetsloggFilter { kontekst ->
            kontekst.kontekstMap["fødselsnummer"] == personidentifikator.toString()
        }

        internal fun arbeidsgiver(orgnummer: String): AktivitetsloggFilter =
            AktivitetsloggFilter { kontekst ->
                kontekst.kontekstType == "Arbeidsgiver" &&
