...
internal class UtbetalingOgAnnulleringTest : AbstractEndToEndTest() {
    ...
    fun `Kan ikke annullere over en fastlåst annullering`() {
        ...
        assertEquals(2,
            person.personLogg.behov
                .filter {
                    it.detaljer()["fagsystemId"] == inspektør.sisteArbeidsgiveroppdragFagsystemId(
                        1.vedtaksperiode
                    )
                }
    ...
    fun `utbetaling_utbetalt tar med begrunnelse på avviste dager`() {
        ...
        val avvisteDager =
            observatør.utbetalingMedUtbetalingEventer.first().utbetalingsdager.filter { it.type == PersonObserver.Utbetalingsdag.Dagtype.AvvistDag }
