...
) = ArbeidsgiverHendelsefabrikk(orgnummer).lagSykmelding(sykeperioder = sykeperioder, id = id)
...
internal fun AbstractEndToEndTest.søknad(
    ...
    ArbeidsgiverHendelsefabrikk(orgnummer)
        .lagSøknad(
            perioder = perioder,
            ...
            sykmeldingSkrevet =
                sykmeldingSkrevet
                    ?: Søknadsperiode.søknadsperiode(perioder.toList())!!.start.atStartOfDay(),
            id = id,
...
internal fun AbstractEndToEndTest.inntektsmelding(
    ...
    inntektsmeldinger[id] =
        AbstractEndToEndTest.InnsendtInntektsmelding(
            LocalDateTime.now(),
            inntektsmeldinggenerator,
            kontrakten,
        )
    inntekter[id] = beregnetInntekt
    EtterspurtBehov.fjern(
        ikkeBesvarteBehov,
        orgnummer,
...
internal fun AbstractEndToEndTest.inntektsmeldingPortal(
    ...
    return ArbeidsgiverHendelsefabrikk(orgnummer)
        ...
        UtbetalingshistorikkForFeriepenger.Arbeidskategorikoder(
            listOf(
                UtbetalingshistorikkForFeriepenger.Arbeidskategorikoder.KodePeriode(
                    ...
                    UtbetalingshistorikkForFeriepenger.Arbeidskategorikoder.Arbeidskategorikode
                        .Arbeidstaker,
                )
            )
        ),
...
internal fun AbstractEndToEndTest.ytelser(
    ...
    return Ytelser(
        ...
        pleiepenger = Pleiepenger(perioder = pleiepenger),
        omsorgspenger = Omsorgspenger(perioder = omsorgspenger),
        opplæringspenger = Opplæringspenger(perioder = opplæringspenger),
        institusjonsopphold = Institusjonsopphold(perioder = institusjonsoppholdsperioder),
        arbeidsavklaringspenger = Arbeidsavklaringspenger(arbeidsavklaringspenger),
        dagpenger = Dagpenger(dagpenger),
    )
}

internal fun manuellPermisjonsdag(dato: LocalDate) =
    ManuellOverskrivingDag(dato, Dagtype.Permisjonsdag)

internal fun manuellFeriedag(dato: LocalDate) = ManuellOverskrivingDag(dato, Dagtype.Feriedag)

internal fun manuellForeldrepengedag(dato: LocalDate) =
    ManuellOverskrivingDag(dato, Dagtype.Foreldrepengerdag)

internal fun manuellSykedag(dato: LocalDate, grad: Int = 100) =
    ManuellOverskrivingDag(dato, Dagtype.Sykedag, grad)

internal fun manuellArbeidsgiverdag(dato: LocalDate) =
    ManuellOverskrivingDag(dato, Dagtype.Egenmeldingsdag)
...
internal fun AbstractEndToEndTest.simulering(
    ...
    person.personLogg
        .etterspurteBehov(vedtaksperiodeIdInnhenter, orgnummer)
        .filter { it.type == Aktivitet.Behov.Behovtype.Simulering }
        .map { simuleringsBehov ->
            Simulering(
                meldingsreferanseId = UUID.randomUUID(),
                vedtaksperiodeId = vedtaksperiodeIdInnhenter.id(orgnummer).toString(),
                orgnummer = orgnummer,
                ...
                melding = "",
                utbetalingId =
                    UUID.fromString(simuleringsBehov.kontekst().getValue("utbetalingId")),
                simuleringsResultat = simuleringsresultat,
            )
        }

internal fun standardSimuleringsresultat(orgnummer: String, totalbeløp: Int = 2000) =
...
internal fun grunnlag(orgnummer: String, skjæringstidspunkt: LocalDate, inntekter: List<Inntekt>) =
...
) = fom.minusYears(1) til fom.minusMonths(1) inntekter { orgnummer inntekt inntekt }
