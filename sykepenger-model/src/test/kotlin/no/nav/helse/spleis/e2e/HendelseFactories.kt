...
internal fun AbstractEndToEndTest.feriepengeutbetaling(
    ...
    fagsystemId = fagsystemId,
    utbetalingId =
        person.personLogg.sisteBehov(Aktivitet.Behov.Behovtype.Utbetaling).kontekst().getValue("utbetalingId").let {
            UUID.fromString(it)
        },
    status = status,
    melding = "hey",
...
internal fun AbstractEndToEndTest.vilkårsgrunnlag(
    ...
    arbeidsforhold: List<Vilkårsgrunnlag.Arbeidsforhold> =
        listOf(Vilkårsgrunnlag.Arbeidsforhold(orgnummer, 1.januar(2017), type = Arbeidsforholdtype.ORDINÆRT)),
...
internal fun AbstractEndToEndTest.simulering(
    ...
    .filter {
        it.type == Aktivitet.Behov.Behovtype.Simulering
