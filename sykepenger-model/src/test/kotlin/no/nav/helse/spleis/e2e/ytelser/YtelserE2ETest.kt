...
internal class YtelserE2ETest : AbstractEndToEndTest() {
    ...
    fun `masse perioder med med andre ytelser`() {
        ...
        håndterOverstyrTidslinje(
            (1.februar til 15.april).map { ManuellOverskrivingDag(it, Dagtype.Foreldrepengerdag) }
        )
    ...
    fun `Det er fotsatt mulig å bli en AUU som vil omgjøres om man kombinerer snax som begrunnelseForReduksjonEllerIkkeUtbetalt og andre ytelser`() {
        ...
        håndterOverstyrTidslinje(
            forlengelse.map { ManuellOverskrivingDag(it, Dagtype.Foreldrepengerdag) }
        )
        assertSisteTilstand(2.vedtaksperiode, AVSLUTTET_UTEN_UTBETALING)

        observatør.vedtaksperiodeVenter.clear()
        assertUgyldigSituasjon(
            "En vedtaksperiode i AVSLUTTET_UTEN_UTBETALING trenger hjelp fordi VIL_OMGJØRES!"
        ) {
            håndterSøknad(februar)
        }
        val venter =
            observatør.vedtaksperiodeVenter.single {
                it.vedtaksperiodeId == 2.vedtaksperiode.id(ORGNUMMER)
            }
    ...
    fun `Foreldrepenger påvirker skjæringstidspunkt for senere perioder`() {
        ...
        assertEquals(1.januar, inspektør.skjæringstidspunkt(3.vedtaksperiode))
        assertEquals(
            "PPPP PPPPPPP PPPPPPP PPPPPPP PPP",
            inspektør.vedtaksperioder(2.vedtaksperiode).sykdomstidslinje.toShortString(),
        )
        håndterYtelser(
            2.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(1.februar til 1.mars, 100)),
        )
        assertEquals(
            "PPPP PPPPPPP PPPPPPP PPPPPPP PPP",
            inspektør.vedtaksperioder(2.vedtaksperiode).sykdomstidslinje.toShortString(),
    ...
    fun `Foreldrepenger påvirker skjæringstidspunkt annen arbeidsgiver på periode etter`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(januar, 100)),
            orgnummer = a1,
        )
        assertEquals(1.januar, inspektør(a2).skjæringstidspunkt(1.vedtaksperiode))
        assertEquals(
            "SSSSSHH SSSSSHH SSSSSHH SSSSSHH SSS",
            inspektør(a1).vedtaksperioder(1.vedtaksperiode).sykdomstidslinje.toShortString(),
        )
    ...
    fun `Foreldrepenger i halen klemrer ikke vekk skjæringstidspunkt`() {
        ...
        håndterOverstyrTidslinje(
            (1.januar til fredag(26.januar)).map {
                ManuellOverskrivingDag(it, Dagtype.Foreldrepengerdag)
            }
        )
        håndterVilkårsgrunnlag(1.vedtaksperiode)
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(1.januar til søndag(28.januar), 100)),
        )
    ...
    fun `perioden får warnings dersom bruker har fått Dagpenger innenfor 4 uker før skjæringstidspunkt`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            dagpenger = listOf(3.januar.minusDays(14) til 5.januar.minusDays(15)),
        )
    ...
    fun `Foreldrepenger starter mindre enn 4 uker før sykefraværstilfellet`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(3.februar til 20.februar, 100)),
        )
    ...
    fun `Foreldrepenger minst 14 dager før perioden`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(16.september til 30.september, 100)),
        )
    ...
    fun `Overlappende svangerskapspenger`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            svangerskapspenger = listOf(GradertPeriode(3.januar til 20.januar, 100)),
        )
    ...
    fun `Foreldrepenger starter mer enn 4 uker før sykefraværstilfellet`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(3.januar til 20.januar, 100)),
        )
        assertIngenFunksjonelleFeil()
    }
    ...
    fun `Omsorgspenger starter mer enn 4 uker før sykefraværstilfellet`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            omsorgspenger = listOf(GradertPeriode(3.januar til 20.januar, 100)),
        )
        assertIngenFunksjonelleFeil()
    }
    ...
    fun `Foreldrepenger før og etter sykmelding`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(februar, 100), GradertPeriode(mai, 100)),
        )
        assertIngenFunksjonelleFeil()
    ...
    fun `Svangerskapspenger før og etter sykmelding`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            svangerskapspenger =
                listOf(GradertPeriode(20.februar til 28.februar, 100), GradertPeriode(mai, 100)),
        )
        assertIngenFunksjonelleFeil()
    ...
    fun `skal ikke ha funksjonelle feil om andre ytelser ved sammenhengende sykdom etter nådd maksdato`() {
        ...
        håndterYtelser(
            ...
            institusjonsoppholdsperioder =
                listOf(Institusjonsopphold.Institusjonsoppholdsperiode(1.februar, 28.februar)),
        )
    ...
    fun `Annen ytelse i periode som tidligere var sykdom`() {
        ...
        håndterOverstyrTidslinje(
            (20..31).map { ManuellOverskrivingDag(it.januar, Dagtype.Pleiepengerdag) },
            orgnummer = a1,
        )
        håndterOverstyrTidslinje(
            (20..31).map { ManuellOverskrivingDag(it.januar, Dagtype.Pleiepengerdag) },
    ...
    fun `Fullstendig overlapp med foreldrepenger`() {
        ...
        assertVarsel(RV_AY_5, 1.vedtaksperiode.filter())
        assertEquals(
            "YYYYYYY YYYYYYY YYYYYYY YYYYYYY YYY",
            inspektør.sykdomstidslinje.toShortString(),
        )
    }
    ...
    fun `graderte foreldrepenger i halen`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(30.januar til 31.januar, 50)),
        )
        assertVarsel(RV_AY_5, 1.vedtaksperiode.filter())
        assertEquals(
            "SSSSSHH SSSSSHH SSSSSHH SSSSSHH SSS",
    ...
    fun `Overlapp med foreldrepenger i halen og utenfor perioden begrenses av perioden`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(1.januar til 10.februar, 100)),
        )
        assertVarsel(RV_AY_5, 1.vedtaksperiode.filter())
        assertEquals(
            "YYYYYYY YYYYYYY YYYYYYY YYYYYYY YYY",
    ...
    fun `Overlapp med foreldrepenger i halen og før perioden begrenses av perioden`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(1.januar til 28.februar, 100)),
        )
        assertVarsel(RV_AY_5, 1.vedtaksperiode.filter())
    ...
    fun `periode med arbeid i snuten som får omslukende ytelser`() {
        ...
        håndterYtelser(
            1.vedtaksperiode,
            foreldrepenger = listOf(GradertPeriode(31.desember(2017) til 1.februar, 100)),
        )
        assertEquals(
