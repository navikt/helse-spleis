...
internal class RefusjonsopplysningerPåBehandlingE2ETest : AbstractDslTest() {
    ...
    fun `arbeidsgiver opplyser om refusjonsopplysnigner frem i tid, så skal saksbehandler overstyre dem før søknaden kommer - ingen ubrukte etter overstyring`() {
        a1 {
            ...
            val saksbehandlerId = håndterOverstyrArbeidsgiveropplysninger(
                1.januar,
                listOf(OverstyrtArbeidsgiveropplysning(a1, INNTEKT, "forklaring", refusjonsopplysninger = listOf(Triple(1.januar, null, INNTEKT))))
    ...
    fun `arbeidsgiver opplyser om refusjonsopplysnigner frem i tid, så skal saksbehandler overstyre dem før søknaden kommer - nye ubrukte etter overstyring`() {
        a1 {
            ...
            val saksbehandlerId = håndterOverstyrArbeidsgiveropplysninger(
                1.januar,
                listOf(OverstyrtArbeidsgiveropplysning(a1, INNTEKT, "forklaring", refusjonsopplysninger = listOf(Triple(1.januar, 31.januar, INNTEKT), Triple(1.februar, null, INGEN))))
            ).metadata.meldingsreferanseId
    ...
    fun `En overstyring som er kommet etter IM skal vinne på forlengelsen`() {
        a1 {
            ...
            håndterOverstyrArbeidsgiveropplysninger(1.januar, listOf(OverstyrtArbeidsgiveropplysning(a1, INNTEKT, "forklaring", listOf(Triple(1.januar, null, INNTEKT / 2)))), hendelseId = overstyringId)
            håndterYtelser(1.vedtaksperiode)
            assertVarsel(Varselkode.RV_UT_23, 1.vedtaksperiode.filter())
            håndterSimulering(1.vedtaksperiode)
    ...
    fun `arbeidsgiver opplyser om refusjonsopplysninger frem i tid`() {
        a1 {
            ...
            val im = håndterInntektsmelding(listOf(1.januar til 16.januar), refusjon = Refusjon(INNTEKT, opphørsdato = 28.februar), mottatt = mottatt)
    ...
    fun `saksbehandler opplyser om endring i refusjon frem i tid`() {
        a1 {
            ...
            håndterOverstyrArbeidsgiveropplysninger(1.januar, listOf(OverstyrtArbeidsgiveropplysning(a1, INNTEKT, "bla", listOf(Triple(1.februar, null, INGEN)))), hendelseId = overstyringId, tidsstempel = overstyringTidspunkt)
            assertEquals(setOf(1.januar), inspektør.ubrukteRefusjonsopplysninger.refusjonstidslinjer.keys)
    ...
    fun `korrigerte refusjonsopplysninger i AvventerVilkårsprøving`() {
        ...
        val imNy = håndterInntektsmelding(listOf(1.januar til 16.januar), INNTEKT, refusjon = Refusjon(500.daglig, 27.januar), mottatt = tidsstempelNy)
    ...
    fun `korrigerte refusjonsopplysninger i AvventerRevurdering`() {
        ...
        val tidsstempelNy = LocalDateTime.now()
        val imNy = håndterInntektsmelding(listOf(1.mars til 16.mars), INNTEKT, refusjon = Refusjon(500.daglig, 27.mars), mottatt = tidsstempelNy)
        val kildeNy = Kilde(imNy, ARBEIDSGIVER, tidsstempelNy)
