...
private constructor(
    ...
    val låstePerioder
        get() = _låstePerioder.toList()

    // Støtte for at perioden er lengre enn vi har dager for (Map-et er sparse)
    private val periode: Periode? =
        periode ?: if (dager.size > 0) Periode(dager.firstKey(), dager.lastKey()) else null

    internal constructor(dager: Map<LocalDate, Dag> = emptyMap()) : this(dager.toSortedMap())

    internal constructor(
        original: Sykdomstidslinje,
        spanningPeriode: Periode,
    ) : this(
        original.dager,
        original.periode?.plus(spanningPeriode),
        original.låstePerioder.toMutableList(),
    )
    ...
    internal fun merge(other: Sykdomstidslinje, beste: BesteStrategy = default): Sykdomstidslinje {
        ...
        other.dager
            .filter { it.key !in låstePerioder }
    ...
    internal operator fun minus(other: Sykdomstidslinje) =
        Sykdomstidslinje(dager.filterNot { it.key in other.dager.keys }.toSortedMap())
    ...
    internal fun subset(periode: Periode) =
        if (this.periode == null || !periode.overlapperMed(this.periode)) Sykdomstidslinje()
            ...
            Sykdomstidslinje(
                dager.subMap(periode.start, periode.endInclusive.nesteDag),
                this.periode.subset(periode),
            )

    /** Uten å utvide tidslinjen */
    internal fun fremTilOgMed(dato: LocalDate) =
        if (periode == null || dato < førsteDag()) Sykdomstidslinje()
        else subset(førsteDag() til dato)
    ...
    internal fun trim(perioder: List<Periode>): Sykdomstidslinje {
        ...
        return Sykdomstidslinje(
            dager.filterKeys { it !in forkast }.toSortedMap(),
            null,
            låstePerioder.toMutableList(),
        )
    }
    ...
    internal fun låsOpp(periode: Periode) =
        this.also {
            _låstePerioder.removeIf { it == periode } ||
                throw IllegalArgumentException("Kan ikke låse opp periode $periode")
        }
    ...
    internal fun bekreftErÅpen(periode: Periode) {
        check(låstePerioder.none { it.overlapperMed(periode) }) {
            "hele eller deler av $periode er låst"
        }
    }
    ...
    internal fun erRettFør(other: Sykdomstidslinje): Boolean {
        ...
        return this.sisteDag().erRettFør(other.førsteDag()) &&
            !this.erSisteDagOppholdsdag() &&
            !other.erFørsteDagOppholdsdag()
    }
    ...
    internal companion object {
        internal fun List<Sykdomstidslinje>.slåSammenForkastedeSykdomstidslinjer() =
            fold(Sykdomstidslinje()) { acc, sykdomstidslinje ->
                val utenProblemdager =
                    Sykdomstidslinje(
                        sykdomstidslinje.dager
                            .filter { (_, dag) -> dag !is ProblemDag }
                            .toSortedMap(),
        ...
        internal fun arbeidsdager(periode: Periode, kilde: Hendelseskilde) =
            Sykdomstidslinje(
                periode.associateWith {
                    if (it.erHelg()) FriskHelgedag(it, kilde) else Arbeidsdag(it, kilde)
                }
            )
        ...
        internal fun ghostdager(periode: Periode) =
            Sykdomstidslinje(
                periode.associateWith {
                    if (it.erHelg()) UkjentDag(it, INGEN) else UkjentDag(it, INGEN)
                }
            )
                        ...
                        toMap(
                            { it },
                            {
                                Økonomi.sykdomsgrad(grad).let { økonomi ->
                                    if (it.erHelg()) SykHelgedag(it, økonomi, kilde)
                                    else Sykedag(it, økonomi, kilde)
                                }
                            },
                        )
                    )
                    ...
                    .collect(
                        toMap(
                            { it },
                            {
                                Økonomi.sykdomsgrad(grad).let { økonomi ->
                                    if (it.erHelg()) SykHelgedag(it, økonomi, kilde)
                                    else SykedagNav(it, økonomi, kilde)
                                }
                            },
                        )
                    )
        ...
        internal fun ukjent(førsteDato: LocalDate, sisteDato: LocalDate, kilde: Hendelseskilde) =
            Sykdomstidslinje(
                førsteDato
                    .datesUntil(sisteDato.plusDays(1))
                    .collect(toMap<LocalDate, LocalDate, Dag>({ it }, { UkjentDag(it, kilde) }))
            )
                    ...
                    .collect(
                        toMap(
                            { it },
                            {
                                Økonomi.sykdomsgrad(grad).let { økonomi ->
                                    if (it.erHelg()) SykHelgedag(it, økonomi, kilde)
                                    else sykedag(it, avskjæringsdato, økonomi, kilde)
                                }
                            },
                        )
                    )
        ...
        private fun sykedag(
            ...
            if (dato < avskjæringsdato) ForeldetSykedag(dato, økonomi, kilde)
            else Sykedag(dato, økonomi, kilde)
                ...
                førsteDato
                    ...
                    .collect(
                        toMap<LocalDate, LocalDate, Dag>(
                            { it },
                            { ArbeidIkkeGjenopptattDag(it, kilde) },
                        )
                    )
            )
        ...
        internal fun problemdager(
            ...
            Sykdomstidslinje(
                førsteDato
                    .datesUntil(sisteDato.plusDays(1))
                    .collect(
                        toMap<LocalDate, LocalDate, Dag>({ it }, { ProblemDag(it, kilde, melding) })
                    )
            )
