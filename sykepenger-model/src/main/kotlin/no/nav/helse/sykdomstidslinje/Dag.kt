...
sealed class Dag(val dato: LocalDate, val kilde: Hendelseskilde) {
    private fun name() = javaClass.canonicalName.split('.').last()

    internal companion object {
        internal val default: BesteStrategy = { venstre: Dag, høyre: Dag ->
            require(venstre.dato == høyre.dato) {
                "Støtter kun sammenlikning av dager med samme dato"
            }
    ...
    internal fun problem(
        other: Dag,
        melding: String =
            "Kan ikke velge mellom ${name()} fra $kilde og ${other.name()} fra ${other.kilde}.",
    ...
    protected open fun funksjoneltLik(other: Dag) =
        this.dato == other.dato && this::class == other::class

    protected fun sammeGrad(økonomi: Økonomi, other: Økonomi) =
    ...
    internal class UkjentDag(dato: LocalDate, kilde: Hendelseskilde) : Dag(dato, kilde) {
        ...
        internal companion object {
            fun gjenopprett(dto: SykdomstidslinjeDagDto.UkjentDagDto): UkjentDag {
                return UkjentDag(dato = dto.dato, kilde = Hendelseskilde.gjenopprett(dto.kilde))
            }
        }
    }

    internal class Arbeidsdag(dato: LocalDate, kilde: Hendelseskilde) : Dag(dato, kilde) {
        override fun dto(dato: LocalDate, kilde: HendelseskildeDto) =
            SykdomstidslinjeDagDto.ArbeidsdagDto(dato, kilde)

        internal companion object {
            fun gjenopprett(dto: SykdomstidslinjeDagDto.ArbeidsdagDto): Arbeidsdag {
                return Arbeidsdag(dato = dto.dato, kilde = Hendelseskilde.gjenopprett(dto.kilde))
            }
        }
    }

    internal class Arbeidsgiverdag(dato: LocalDate, val økonomi: Økonomi, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is Arbeidsgiverdag && sammeGrad(økonomi, other.økonomi)
    ...
    internal class Feriedag(dato: LocalDate, kilde: Hendelseskilde) : Dag(dato, kilde) {
        override fun dto(dato: LocalDate, kilde: HendelseskildeDto) =
            SykdomstidslinjeDagDto.FeriedagDto(dato, kilde)

        internal companion object {
            fun gjenopprett(dto: SykdomstidslinjeDagDto.FeriedagDto): Feriedag {
                return Feriedag(dato = dto.dato, kilde = Hendelseskilde.gjenopprett(dto.kilde))
            }
        }
    }

    internal class ArbeidIkkeGjenopptattDag(dato: LocalDate, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun dto(dato: LocalDate, kilde: HendelseskildeDto) =
        ...
        internal companion object {
            fun gjenopprett(
                dto: SykdomstidslinjeDagDto.ArbeidIkkeGjenopptattDagDto
    ...
    internal class FriskHelgedag(dato: LocalDate, kilde: Hendelseskilde) : Dag(dato, kilde) {
        override fun dto(dato: LocalDate, kilde: HendelseskildeDto) =
        ...
        internal companion object {
            fun gjenopprett(dto: SykdomstidslinjeDagDto.FriskHelgedagDto): FriskHelgedag {
                return FriskHelgedag(dato = dto.dato, kilde = Hendelseskilde.gjenopprett(dto.kilde))
            }
        }
    }
    ...
    internal class ArbeidsgiverHelgedag(
        ...
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is ArbeidsgiverHelgedag && sammeGrad(økonomi, other.økonomi)
        ...
        internal companion object {
            fun gjenopprett(
                dto: SykdomstidslinjeDagDto.ArbeidsgiverHelgedagDto
    ...
    internal class Sykedag(dato: LocalDate, val økonomi: Økonomi, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is Sykedag && sammeGrad(økonomi, other.økonomi)
    ...
    internal class ForeldetSykedag(dato: LocalDate, val økonomi: Økonomi, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is ForeldetSykedag && sammeGrad(økonomi, other.økonomi)
    ...
    internal class SykHelgedag(dato: LocalDate, val økonomi: Økonomi, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is SykHelgedag && sammeGrad(økonomi, other.økonomi)
    ...
    internal class SykedagNav(dato: LocalDate, val økonomi: Økonomi, kilde: Hendelseskilde) :
        Dag(dato, kilde) {
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is SykedagNav && sammeGrad(økonomi, other.økonomi)
    ...
    internal class Permisjonsdag(dato: LocalDate, kilde: Hendelseskilde) : Dag(dato, kilde) {
        override fun dto(dato: LocalDate, kilde: HendelseskildeDto) =
        ...
        internal companion object {
            fun gjenopprett(dto: SykdomstidslinjeDagDto.PermisjonsdagDto): Permisjonsdag {
                return Permisjonsdag(dato = dto.dato, kilde = Hendelseskilde.gjenopprett(dto.kilde))
            }
        }
    }
    ...
    internal class ProblemDag(
        ...
        internal constructor(
            ...
            kilde: Hendelseskilde,
            melding: String,
        ) : this(dato, kilde, kilde, melding)

        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is ProblemDag && melding == other.melding
    ...
    internal class AndreYtelser(dato: LocalDate, kilde: Hendelseskilde, val ytelse: AnnenYtelse) :
        Dag(dato, kilde) {
        ...
        override fun funksjoneltLik(other: Dag) =
            dato == other.dato && other is AndreYtelser && ytelse == other.ytelse
