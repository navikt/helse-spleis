 ...
 *
 * Gitt en vedtaksperiode [fom, tom] så går vi bakover fra og med 'fom' og tilbake i tid frem til vi
 * finner skjæringstidspunkt. Hvis vedtaksperioden har friskmelding i halen så begynner vi leitingen
 * dagen før friskmeldingen.
 *
 ...
 * til vi kommer på "andre siden". Om det er sykedager på andre siden så fortsetter søket etter
 * skjæringstidspunkt.
 *
 * Ved en oppholdsdag stoppes søket, med mindre det er andre ytelser – da fortsetter søket på samme
 * måte som om det var en feriedag.
 */
internal class Skjæringstidspunkt(private val personsykdomstidslinje: Sykdomstidslinje) {
    ...
    private fun traverserTidslinjenBaklengs(
        søkeperiode: Periode,
        looper: (Søkekontekst, Dag, LocalDate) -> Søkekontekst,
    ): Søkekontekst {
        ...
        while (
            gjeldendeDag >= personsykdomstidslinje.førsteDag() && søkekontekst.skalFortsetteÅSøke()
        ) {
    ...
    private data class Søkekontekst(
        ...
        fun oppholdsdag(søkeperiode: Periode, dagen: LocalDate): Søkekontekst =
            when {
        ...
        fun potensieltSkjæringstidspunkt(dagen: LocalDate): Søkekontekst =
            tilstand.potensieltSkjæringstidspunkt(this, dagen)

        fun potensieltGyldigOppholdsperiode(dagen: LocalDate): Søkekontekst =
            tilstand.potensieltGyldigOppholdsperiode(this, dagen)
    }

    private sealed interface Søketilstand {
        fun oppholdsdag(kontekst: Søkekontekst) = kontekst.avsluttSøk()

        fun potensieltSkjæringstidspunkt(kontekst: Søkekontekst, dagen: LocalDate): Søkekontekst =
            kontekst.copy(tilstand = HarPotensieltSkjæringstidspunkt, skjæringstidspunkt = dagen)

        // en dag som potensielt kan bygge bro mellom to sykdomsperioder (ferie, andre ytelser)
        fun potensieltGyldigOppholdsperiode(kontekst: Søkekontekst, dagen: LocalDate) =
        ...
        fun andreYtelser(kontekst: Søkekontekst, dagen: LocalDate) =
            kontekst.copy(tilstand = AndreYtelser)
        ...
        data object HarIkkeFunnetSkjæringstidspunkt : Søketilstand {
            ...
            override fun potensieltGyldigOppholdsperiode(kontekst: Søkekontekst, dagen: LocalDate) =
                kontekst
        }

        data object HarPotensieltSkjæringstidspunkt : Søketilstand {
            override fun andreYtelser(kontekst: Søkekontekst, dagen: LocalDate) =
                kontekst.avsluttSøk()
        }
        ...
        data object HarSkjæringstidspunkt : Søketilstand {
            override fun potensieltSkjæringstidspunkt(
                kontekst: Søkekontekst,
                dagen: LocalDate,
            ): Søkekontekst = kontekst

            override fun potensieltGyldigOppholdsperiode(
                kontekst: Søkekontekst,
                dagen: LocalDate,
            ): Søkekontekst = kontekst
