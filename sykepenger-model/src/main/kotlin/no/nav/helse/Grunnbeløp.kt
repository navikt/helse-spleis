...
class Grunnbeløp private constructor(private val multiplier: Double) {
    ...
    fun minsteinntekt(dato: LocalDate) =
        HistoriskGrunnbeløp.gjeldendeMinsteinntektGrunnbeløp(grunnbeløp, dato).beløp(multiplier)
    ...
    private class HistoriskGrunnbeløp(
        ...
        init {
            require(virkningsdato >= gyldigFra) {
                "Virkningsdato må være nyere eller lik gyldighetstidspunktet"
            }
            require(gyldigMinsteinntektKrav >= gyldigFra) {
        ...
        private fun gyldighetsperiode(år: Int, andre: List<HistoriskGrunnbeløp>) =
            gyldigFra til
                (andre
                    ...
                    ?.gyldigFra
                    ?.forrigeDag ?: gyldigFra.withMonth(12).withDayOfMonth(31))
        ...
        private fun snitt(år: Int, andre: List<HistoriskGrunnbeløp>): Inntekt {
            ...
            val antallMånederSomDekkesAvGrunnbeløpet =
                gyldighetsperiode.subset(periode).let {
                    it.endInclusive.monthValue - it.start.monthValue + 1
                }
            return beløp(antallMånederSomDekkesAvGrunnbeløpet.toDouble()).månedlig.årlig
        }
            ...
            fun gjeldendeMinsteinntektGrunnbeløp(
                ...
                return grunnbeløper
                    ...
                    ?: throw NoSuchElementException(
                        "Finner ingen grunnbeløp som gyldig som minsteinntektskrav for $dato"
                    )
            }
