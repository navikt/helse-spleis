...
internal data object AvventerInntektsopplysningerForAnnenArbeidsgiver : Vedtaksperiodetilstand {
    ...
    override fun håndterPåminnelse(vedtaksperiode: Vedtaksperiode, eventBus: EventBus, påminnelse: Påminnelse, aktivitetslogg: IAktivitetslogg): Revurderingseventyr? {
        val nesteTilstandEtterInntekt = nesteTilstandEtterInntekt(vedtaksperiode)
        if (nesteTilstandEtterInntekt is AvventerInntektsopplysningerForAnnenArbeidsgiver) return null // Er i riktig tilstand

        vedtaksperiode.tilstand(eventBus, aktivitetslogg, nesteTilstandEtterInntekt)
        vedtaksperiode.person.gjenopptaBehandling(aktivitetslogg)
        return null
    }
}
