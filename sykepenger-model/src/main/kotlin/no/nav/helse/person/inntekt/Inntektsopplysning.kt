...
sealed class Inntektsopplysning(
    ...
    protected open fun kanOverstyresAv(ny: Inntektsopplysning): Boolean {
        // kun saksbehandlerinntekt eller annen inntektsmelding kan overstyre
        // inntektsmelding-inntekt
        if (ny is SkjønnsmessigFastsatt) return true
        if (ny is Saksbehandler) {
            return when {
                // hvis inntekten er skjønnsmessig fastsatt og det overstyres til samme omregnede
                // årsinntekt, så beholdes den skjønnsmessig fastsatte inntekten
                this is SkjønnsmessigFastsatt &&
                    this.omregnetÅrsinntekt().fastsattÅrsinntekt() == ny.fastsattÅrsinntekt() ->
                    false
    ...
    internal companion object {
        ...
        private fun omregnetÅrsinntekt(liste: List<Inntektsopplysning>) =
            liste.map { it.omregnetÅrsinntekt().beløp }.map { it.årlig.toInt() }

        internal fun List<Inntektsopplysning>.markerFlereArbeidsgivere(
            aktivitetslogg: IAktivitetslogg
        ) {
            if (
                distinctBy { it.dato }.size <= 1 &&
                    none { it is SkattSykepengegrunnlag || it is IkkeRapportert }
            )
                return
        ...
        internal fun List<Inntektsopplysning>.validerSkjønnsmessigAltEllerIntet() {
            check(all { it is SkjønnsmessigFastsatt } || none { it is SkjønnsmessigFastsatt }) {
                "Enten så må alle inntektsopplysninger var skjønnsmessig fastsatt, eller så må ingen være det"
            }
        }
        ...
        internal fun gjenopprett(
            ...
            val inntektsopplysning =
                inntekter.getOrPut(dto.id) {
                    when (dto) {
                        is InntektsopplysningInnDto.IkkeRapportertDto ->
                            IkkeRapportert.gjenopprett(dto)
                        is InntektsopplysningInnDto.InfotrygdDto -> Infotrygd.gjenopprett(dto)
                        is InntektsopplysningInnDto.InntektsmeldingDto ->
                            Inntektsmelding.gjenopprett(dto)
                        is InntektsopplysningInnDto.SaksbehandlerDto ->
                            Saksbehandler.gjenopprett(dto, inntekter)
                        is InntektsopplysningInnDto.SkattSykepengegrunnlagDto ->
                            SkattSykepengegrunnlag.gjenopprett(dto)
                        is InntektsopplysningInnDto.SkjønnsmessigFastsattDto ->
                            SkjønnsmessigFastsatt.gjenopprett(dto, inntekter)
                    }
                }
