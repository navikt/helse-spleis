...
data class Refusjonsopplysning(
    ...
    private fun trim(periodeSomSkalFjernes: Periode): List<Refusjonsopplysning> {
        return this.periode.trim(periodeSomSkalFjernes).map {
            Refusjonsopplysning(
                fom = it.start,
    ...
    private fun oppdatertTom(nyTom: LocalDate) =
        if (nyTom < fom) null
        else Refusjonsopplysning(meldingsreferanseId, fom, nyTom, beløp, avsender, tidsstempel)

    private fun begrensTil(dato: LocalDate): Refusjonsopplysning? {
        return if (dekker(dato)) oppdatertTom(dato.forrigeDag) else this
    }
    ...
    internal companion object {
        private fun List<Refusjonsopplysning>.mergeInnNyeOpplysninger(
            ...
            return nyeOpplysninger
                .fold(this) { resultat, nyOpplysning ->
                    resultat.mergeInnNyOpplysning(nyOpplysning, begrensetFra)
                }
                .sortedBy { it.fom }
        }
        ...
        private fun List<Refusjonsopplysning>.mergeInnNyOpplysning(
            ...
            // bevarer eksisterende opplysning hvis ny opplysning finnes fra før (dvs. vi bevarer
            // meldingsreferanseId på forrige)
            if (any { it.funksjoneltLik(nyOpplysningBegrensetStart) }) return this
            // Beholder de delene som ikke dekkes av den nye opplysningen og legger til den nye
            // opplysningen
            return flatMap { eksisterendeOpplysning ->
                    eksisterendeOpplysning.trim(nyOpplysningBegrensetStart.periode)
                }
    ...
    class Refusjonsopplysninger
    private constructor(refusjonsopplysninger: List<Refusjonsopplysning>) {
        ...
        init {
            check(!validerteRefusjonsopplysninger.overlapper()) {
                "Refusjonsopplysninger skal ikke kunne inneholde overlappende informasjon: $refusjonsopplysninger"
            }
        }
        ...
        internal fun lagreTidsnær(
            ...
            val relevanteRefusjonsopplysninger =
        ...
        internal fun beløpstidslinje() =
            validerteRefusjonsopplysninger.fold(Beløpstidslinje()) {
                samletBeløpstidslinje,
                refusjonsopplysning ->
                samletBeløpstidslinje +
                    Beløpstidslinje.fra(
                        periode =
                            refusjonsopplysning.fom til
                                (refusjonsopplysning.tom ?: refusjonsopplysning.fom),
                        beløp = refusjonsopplysning.beløp,
                        kilde =
                            Kilde(
            ...
            when (sisteOppholdsdagFørPerioden) {
                null -> this
                    ...
                    Refusjonsopplysninger(
                        validerteRefusjonsopplysninger.mapNotNull {
                            it.begrensTil(sisteOppholdsdagFørPerioden)
                        }
                    )
            }
        ...
        private fun harNødvendigRefusjonsopplysninger(
            ...
            val førsteRefusjonsopplysning =
                førsteRefusjonsopplysning()
                    ?: return false.also {
                        aktivitetslogg.info(
                            "Mangler refusjonsopplysninger på orgnummer $organisasjonsnummer for hele perioden (${utbetalingsdager.omsluttendePeriode})"
                        )
                    }
            val dekkes = utbetalingsdager.filter { utbetalingsdag -> dekker(utbetalingsdag) }
            val aksepteres =
                utbetalingsdager.filter { utbetalingsdag ->
                    førsteRefusjonsopplysning.aksepterer(skjæringstidspunkt, utbetalingsdag)
                }
            val mangler =
                (utbetalingsdager - dekkes - aksepteres).takeUnless { it.isEmpty() } ?: return true
            aktivitetslogg.info(
                "Mangler refusjonsopplysninger på orgnummer $organisasjonsnummer for periodene ${mangler.grupperSammenhengendePerioder()}"
            )
            return false
        }
        ...
        internal fun harNødvendigRefusjonsopplysninger(
            ...
            hensyntattSisteOppholdagFørPerioden(sisteOppholdsdagFørPerioden)
        ...
        internal fun finnFørsteDatoForEndring(other: Refusjonsopplysninger): LocalDate? {
            // finner alle nye
            val nye =
                this.validerteRefusjonsopplysninger.filter { opplysning ->
                    other.validerteRefusjonsopplysninger.none {
                        it.meldingsreferanseId == opplysning.meldingsreferanseId
                    }
                }
            // fjerner de hvor perioden og beløpet dekkes av forrige
            val nyeUlik =
                nye.filterNot { opplysning ->
        ...
        internal fun overlappendeEllerSenereRefusjonsopplysninger(
            periode: Periode?
        ): List<Refusjonsopplysning> {
        ...
        internal companion object {
            ...
            internal val Refusjonsopplysning.refusjonsopplysninger
                get() = Refusjonsopplysninger(listOf(this))

            internal fun gjenopprett(dto: RefusjonsopplysningerInnDto) =
                Refusjonsopplysninger(
                    refusjonsopplysninger =
                        dto.opplysninger.map { Refusjonsopplysning.gjenopprett(it) }
                )
        }
        ...
        class RefusjonsopplysningerBuilder {
            ...
            private fun sorterteRefusjonsopplysninger() =
                refusjonsopplysninger
                    .sortedWith(
                        compareBy(
            ...
            fun build() =
                Refusjonsopplysninger(
                    emptyList<Refusjonsopplysning>()
                        .mergeInnNyeOpplysninger(sorterteRefusjonsopplysninger())
                )
        }

        internal fun dto() =
            RefusjonsopplysningerUtDto(
                opplysninger = this.validerteRefusjonsopplysninger.map { it.dto() }
            )
    }

    internal fun dto() =
        RefusjonsopplysningUtDto(
            meldingsreferanseId = this.meldingsreferanseId,
            fom = this.fom,
            tom = this.tom,
