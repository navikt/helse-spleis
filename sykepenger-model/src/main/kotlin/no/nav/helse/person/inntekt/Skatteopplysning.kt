...
data class Skatteopplysning(
    ...
    companion object {
        ...
        fun omregnetÅrsinntekt(liste: List<Skatteopplysning>) =
            liste.map { it.beløp }.summer().coerceAtLeast(Inntekt.INGEN).div(3)

        fun rapportertInntekt(liste: List<Skatteopplysning>) =
            liste.map { it.beløp }.summer().div(12)

        fun List<Skatteopplysning>.subsumsjonsformat() =
            this.map {
                Inntektsubsumsjon(
