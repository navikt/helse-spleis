...
class Inntektsmelding internal constructor(
    ...
    override fun erSamme(other: Inntektsopplysning): Boolean =
        other is Inntektsmelding && this.dato == other.dato && other.beløp == this.beløp
    ...
    internal fun kopierTidsnærOpplysning(
        ...
        if (dagerMellom >= 60) {
            aktivitetslogg.info(
                "Det er $dagerMellom dager mellom forrige inntektdato (${this.dato}) og ny inntektdato ($nyDato), dette utløser varsel om gjenbruk.",
            )
            aktivitetslogg.varsel(RV_IV_7)
        } else if (nyArbeidsgiverperiode) {
            aktivitetslogg.info(
                "Det er ny arbeidsgiverperiode, og dette utløser varsel om gjenbruk. Forrige inntektdato var ${this.dato} og ny inntektdato er $nyDato",
            )
            aktivitetslogg.varsel(RV_IV_7)
        }
    ...
    internal companion object {
        ...
        internal fun List<Inntektsmelding>.avklarSykepengegrunnlag(
            ...
            val skatt =
                skattSykepengegrunnlag?.takeIf { it.kanBrukes(skjæringstidspunkt) }?.somSykepengegrunnlag() ?: return inntektsmelding
