...
internal class UtbetalingsdagerBuilder(
    ...
    private val utbetalingsdager = utbetalingstidslinje.map { dag ->
        when (dag) {
            is Utbetalingsdag.Arbeidsdag -> PersonObserver.Utbetalingsdag(
                dag.dato,
                PersonObserver.Utbetalingsdag.Dagtype.Arbeidsdag
            )
            ...
            is Utbetalingsdag.Fridag -> {
                val (dagtype, begrunnelse) = when (sykdomstidslinje[dag.dato]) {
                    ...
                    is Dag.AndreYtelser -> AndreYtelser to eksternBegrunnelse(sykdomstidslinje[dag.dato])?.let {
                        listOf(
                            it
                        )
                    }
            ...
            is Utbetalingsdag.ForeldetDag -> PersonObserver.Utbetalingsdag(
                dag.dato,
                PersonObserver.Utbetalingsdag.Dagtype.ForeldetDag
            )

            is Utbetalingsdag.UkjentDag -> PersonObserver.Utbetalingsdag(
                dag.dato,
                PersonObserver.Utbetalingsdag.Dagtype.UkjentDag
            )
        }
    }
