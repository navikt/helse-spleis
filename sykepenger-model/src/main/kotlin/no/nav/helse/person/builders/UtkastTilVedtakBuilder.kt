...
internal class UtkastTilVedtakBuilder(
    ...
    init {
        if (erForlengelse) tags.add(Tag.Forlengelse) else tags.add(Tag.Førstegangsbehandling)
    }
    ...
    internal fun relevantPeriode(
        ...
        relevantePerioder.add(
            RelevantPeriode(vedtaksperiodeId, behandlingId, skjæringstidspunkt, periode)
        )
    }
    ...
    internal fun periode(arbeidsgiverperiode: List<Periode>, periode: Periode) = apply {
        ...
        val gjennomført =
            NormalArbeidstaker.arbeidsgiverperiodenGjennomført(
                arbeidsgiverperiode.periode()?.count() ?: 0
            )
        val arbeidsgiverperiodePåstartetITidligerePeriode =
            arbeidsgiverperiode.isNotEmpty() &&
                periode.start >= arbeidsgiverperiode.last().endInclusive
    ...
    internal fun hendelseIder(hendelseIder: Set<UUID>) = apply {
    ...
    internal fun vilkårsgrunnlagId(vilkårsgrunnlagId: UUID) = apply {
    ...
    private val Utbetalingstidslinje.behandlingsresultat
        get(): Tag {
            val avvistDag = any {
                it is Utbetalingsdag.AvvistDag || it is Utbetalingsdag.ForeldetDag
            }
    ...
    internal fun arbeidsgiverinntekt(
        ...
        arbeidsgiverinntekter.add(
            Arbeidsgiverinntekt(
                arbeidsgiver,
                omregnedeÅrsinntekt.årlig,
                skjønnsfastsatt?.årlig,
                gjelder,
                skatteopplysning,
            )
        )
    ...
    private inner class Build {
        private val skjønnsfastsatt =
            arbeidsgiverinntekter
                .any { it.skjønnsfastsatt != null }
                .also {
                    if (it)
                        check(
        ...
        private val perioderMedSammeSkjæringstidspunkt =
            relevantePerioder.filter { it.skjæringstidspunkt == skjæringstidspunkt }

        init {
            check(arbeidsgiverinntekter.isNotEmpty()) {
                "Forventet ikke at det ikke er noen arbeidsgivere i sykepengegrunnlaget."
            }
            if (tilkomneArbeidsgivere.isNotEmpty()) tags.add(Tag.TilkommenInntekt)
            if (arbeidsgiverinntekter.size + tilkomneArbeidsgivere.size == 1)
            ...
            if (arbeidsgiverinntekter.single { it.arbeidsgiver == arbeidsgiver }.skatteopplysning)
                tags.add(Tag.InntektFraAOrdningenLagtTilGrunn)
        }

        private val sykepengegrunnlagsfakta:
            PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta =
            when {
                tags.contains(Tag.InngangsvilkårFraInfotrygd) ->
                    PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd(totalOmregnetÅrsinntekt)
                        .also {
                            check(Tag.FlereArbeidsgivere !in tags) {
        ...
        private val omregnetÅrsinntektPerArbeidsgiverForAvsluttedMedVedtak =
            sykepengegrunnlagsfakta.omregnetÅrsinntektPerArbeidsgiverForAvsluttedMedVedtak()

        private val sykepengegrunnlagsbegrensningForAvsluttetMedVedtak =
            sykepengegrunnlagsfakta.sykepengegrunnlagsbegrensningForAvsluttedMedVedtak(tags)

        private val periodetypeForGodkjenningsbehov =
            sykepengegrunnlagsfakta.periodetypeForGodkjenningsbehov(tags)

        private val omregnedeÅrsinntekterForGodkjenningsbehov =
            sykepengegrunnlagsfakta.omregnedeÅrsinntekterForGodkjenningsbehov(arbeidsgiver)
        ...
        val godkjenningsbehov =
            mapOf(
                ...
                "inntektskilde" to
                    if (tags.contains(Tag.EnArbeidsgiver)) "EN_ARBEIDSGIVER"
                    else "FLERE_ARBEIDSGIVERE",
                // Til ettertanke: Her kan det være orgnummer på tilkomnde arbeidsgivere i tillegg
                // til de som er i "sykepengegrunnlagsfakta". Kanskje finne på noe smartere der?
                "orgnummereMedRelevanteArbeidsforhold" to
                    (arbeidsgiverinntekter.map { it.arbeidsgiver }).toSet(),
        ...
        fun avsluttetMedVedtak(vedtakFattet: LocalDateTime, historiskeHendelseIder: Set<UUID>) =
            PersonObserver.AvsluttetMedVedtakEvent(
                organisasjonsnummer = arbeidsgiver,
                vedtaksperiodeId = vedtaksperiodeId,
                behandlingId = behandlingId,
                periode = periode,
                // Til ettertanke: AvsluttetMedVedtak har alle hendelseId'er ever på
                // vedtaksperioden, mens godkjenningsbehov/utkast_til_vedtak har kun behandlingens
                ...
                // Til ettertanke: Denne hentet data fra sykepengegrunnlagsfakta som har to
                // desimaler
                // Til ettertanke: Denne mappes ut i JSON som
                // "grunnlagForSykepengegrunnlagPerArbeidsgiver"
                omregnetÅrsinntektPerArbeidsgiver =
                    omregnetÅrsinntektPerArbeidsgiverForAvsluttedMedVedtak,
                inntekt =
                    inntektForAvsluttetMedVedtak, // TODO: Til ettertanke: What? 👀 Denne håper jeg
                                                  // ingen bruker
                utbetalingId = utbetalingId,
                sykepengegrunnlagsbegrensning = sykepengegrunnlagsbegrensningForAvsluttetMedVedtak,
                vedtakFattetTidspunkt = vedtakFattet,
                // Til ettertanke: Akkurat i avsluttet i vedtak blir beløp i sykepengegrunnlagsfakta
                // avrundet til to desimaler.
                sykepengegrunnlagsfakta =
                    when (val fakta = sykepengegrunnlagsfakta) {
                        is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd ->
                            fakta.copy(omregnetÅrsinntekt = fakta.omregnetÅrsinntekt.toDesimaler)
                        is PersonObserver.UtkastTilVedtakEvent.FastsattEtterHovedregel ->
                            fakta.copy(
    ...
    private companion object {
        private val Double.toDesimaler
            get() = toBigDecimal().setScale(2, RoundingMode.HALF_UP).toDouble()

        private val Set<Tag>.utgående
            get() = map { it.name }.toSet()

        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .beregningsgrunnlagForAvsluttetMedVedtak() =
            when (val fakta = this) {
        ...
        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .inntektForAvsluttetMedVedtak() = beregningsgrunnlagForAvsluttetMedVedtak() / 12

        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .omregnetÅrsinntektPerArbeidsgiverForAvsluttedMedVedtak(): Map<String, Double> =
            when (val fakta = this) {
                is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd -> emptyMap()
                is PersonObserver.UtkastTilVedtakEvent.FastsattEtterHovedregel ->
                    fakta.arbeidsgivere.associate {
                        it.arbeidsgiver to it.omregnetÅrsinntekt.toDesimaler
                    }
                is PersonObserver.UtkastTilVedtakEvent.FastsattEtterSkjønn ->
                    fakta.arbeidsgivere.associate {
                        it.arbeidsgiver to it.skjønnsfastsatt.toDesimaler
                    }
            }

        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .sykepengegrunnlagsbegrensningForAvsluttedMedVedtak(tags: Set<Tag>) =
            when {
                this is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd ->
                    "VURDERT_I_INFOTRYGD"
        ...
        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .periodetypeForGodkjenningsbehov(tags: Set<Tag>): String {
            val erForlengelse = tags.contains(Tag.Forlengelse)
            return when {
                this is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd ->
                    if (erForlengelse) "INFOTRYGDFORLENGELSE" else "OVERGANG_FRA_IT"
                else -> if (erForlengelse) "FORLENGELSE" else "FØRSTEGANGSBEHANDLING"
            }
        }

        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta
            .omregnedeÅrsinntekterForGodkjenningsbehov(
            arbeidsgiver: String
        ): List<Map<String, Any>> =
            when (val fakta = this) {
                is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd ->
