...
internal class UtkastTilVedtakBuilder(
    ...
    internal fun periode(arbeidsgiverperiode: List<Periode>, periode: Periode) = apply {
        ...
        val gjennomført = NormalArbeidstaker.arbeidsgiverperiodenGjennomført(
    ...
    internal fun hendelseIder(hendelseIder: Set<UUID>) =
        apply { this.hendelseIder.addAll(hendelseIder) }
    ...
    internal fun vilkårsgrunnlagId(vilkårsgrunnlagId: UUID) =
        apply { this.vilkårsgrunnlagId = vilkårsgrunnlagId }
    ...
    private val Utbetalingstidslinje.behandlingsresultat
        get(): Tag {
            val avvistDag =
                any { it is Utbetalingsdag.AvvistDag || it is Utbetalingsdag.ForeldetDag }
    ...
    private inner class Build {
        ...
        init {
            ...
            if (arbeidsgiverinntekter.size + tilkomneArbeidsgivere.size == 1) tags.add(Tag.EnArbeidsgiver) else tags.add(
                Tag.FlereArbeidsgivere
            )
            if (arbeidsgiverinntekter.single { it.arbeidsgiver == arbeidsgiver }.skatteopplysning) tags.add(
        ...
        private val sykepengegrunnlagsfakta: PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta =
            when {
                tags.contains(Tag.InngangsvilkårFraInfotrygd) -> PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd(
        ...
        fun avsluttetMedVedtak(vedtakFattet: LocalDateTime, historiskeHendelseIder: Set<UUID>) =
            PersonObserver.AvsluttetMedVedtakEvent(
                ...
                sykepengegrunnlagsfakta = when (val fakta = sykepengegrunnlagsfakta) {
                    is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd -> fakta.copy(
                    ...
                    is PersonObserver.UtkastTilVedtakEvent.FastsattEtterSkjønn -> fakta.copy(
                        omregnetÅrsinntekt = fakta.omregnetÅrsinntekt.toDesimaler,
                        `6G` = fakta.`6G`.toDesimaler,
                        arbeidsgivere = fakta.arbeidsgivere.map {
                            it.copy(
    ...
    private companion object {
        ...
        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta.periodetypeForGodkjenningsbehov(
            tags: Set<Tag>
        ): String {
        ...
        private fun PersonObserver.UtkastTilVedtakEvent.Sykepengegrunnlagsfakta.omregnedeÅrsinntekterForGodkjenningsbehov(
            arbeidsgiver: String
        ): List<Map<String, Any>> = when (val fakta = this) {
            is PersonObserver.UtkastTilVedtakEvent.FastsattIInfotrygd -> listOf(
                mapOf(
            ...
            is PersonObserver.UtkastTilVedtakEvent.FastsattEtterHovedregel -> fakta.arbeidsgivere.map {
                mapOf(
            ...
            is PersonObserver.UtkastTilVedtakEvent.FastsattEtterSkjønn -> fakta.arbeidsgivere.map {
                mapOf(
                    "organisasjonsnummer" to it.arbeidsgiver,
                    "beløp" to it.omregnetÅrsinntekt
                )
