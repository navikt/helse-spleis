...
internal class Arbeidsgiver private constructor(
    ...
    internal companion object {
        ...
        internal fun List<Arbeidsgiver>.tidligsteDato(): LocalDate = mapNotNull { it.sykdomstidslinje().periode()?.start }.minOrNull() ?: LocalDate.now()
        ...
        internal fun List<Arbeidsgiver>.venter(nestemann: Vedtaksperiode) = flatMap { arbeidsgiver -> arbeidsgiver.vedtaksperioder.venter(nestemann) }
        ...
        internal fun List<Arbeidsgiver>.aktiveSkjæringstidspunkter(): Set<LocalDate> = flatMap { it.vedtaksperioder }.aktiveSkjæringstidspunkter()
        ...
        internal fun Iterable<Arbeidsgiver>.vedtaksperioder(filter: VedtaksperiodeFilter) = map { it.vedtaksperioder.filter(filter) }.flatten()
    ...
    internal fun lagRevurdering(
        ...
        gjenståendeSykedager: Int,
        periode: Periode
    ): Utbetaling = lagNyUtbetaling(aktivitetslogg, utbetalingstidslinje, maksdato, forbrukteSykedager, gjenståendeSykedager, periode, Utbetalingtype.REVURDERING)
    ...
    internal fun harIngenSporingTilInntektsmeldingISykefraværet(): Boolean = vedtaksperioder.harIngenSporingTilInntektsmeldingISykefraværet()

    override fun toSpesifikkKontekst(): SpesifikkKontekst = SpesifikkKontekst("Arbeidsgiver", mapOf("organisasjonsnummer" to organisasjonsnummer))
    ...
    fun vedtaksperioderKnyttetTilArbeidsgiverperiode(arbeidsgiverperiode: Arbeidsgiverperiode): List<Vedtaksperiode> = vedtaksperioder.filter { it.periode() in arbeidsgiverperiode }
