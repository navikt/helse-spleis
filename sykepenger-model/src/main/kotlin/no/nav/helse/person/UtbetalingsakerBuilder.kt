...
internal class UtbetalingsakerBuilder(
    ...
    /**
     * utbetalingsaker er den logiske inndelingen som binder sammen flere vedtaksperioder til en sak
     * mot Oppdragsystemet
     *
     * i praksis lager vi en utbetalingssak per arbeidsgiverperiode, men vi bryter de opp om det er
     * infotrygdutbetalinger mellom
     */
    fun lagUtbetalingsaker(): List<Utbetalingsak> {
        ...
        return vedtaksperiodene
            .groupBy {
                finnStartdatoForUtbetalingsakForVedtaksperiode(
                    it,
                    infotrygdbetalinger,
                    vedtaksperiodene,
                )
    ...
    private fun finnStartdatoForUtbetalingsakForVedtaksperiode(
        vedtaksperiode: ArbeidsgiverperiodeForVedtaksperiode,
        infotrygdbetalinger: List<Periode>,
        vedtaksperiodene: List<ArbeidsgiverperiodeForVedtaksperiode>,
    ): LocalDate {
        // eventuell infotrygdutbetaling som ligger mellom arbeidsgiverperioden og vedtaksperioden
        val mellomliggendeInfotrygdutbetaling =
            infotrygdutbetalingEtterArbeidsgiverperiodenOgFørVedtaksperioden(
                infotrygdbetalinger,
                vedtaksperiode,
            )
        val infotrygdutbetalingsakStartdato =
            mellomliggendeInfotrygdutbetaling?.let { infotrygdDag ->
                vedtaksperiodene
                    .first { it.vedtaksperiode.start > infotrygdDag.endInclusive }
                    .vedtaksperiode
                    .start
            }
    ...
    private fun infotrygdutbetalingEtterArbeidsgiverperiodenOgFørVedtaksperioden(
        infotrygdbetalinger: List<Periode>,
        vedtaksperiode: ArbeidsgiverperiodeForVedtaksperiode,
    ) =
        infotrygdbetalinger.lastOrNull { infotrygdperiode ->
            (vedtaksperiode.arbeidsgiverperioder.isEmpty() ||
    ...
    private fun utbetalteInfotrygdperioderMellomVedtaksperioder(
        vedtaksperiodene: List<ArbeidsgiverperiodeForVedtaksperiode>
    ) =
