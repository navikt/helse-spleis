...
internal class Refusjonsservitør(input: Map<LocalDate, Beløpstidslinje> = emptyMap()) {
    ...
    private fun leggTil(dato: LocalDate, beløpstidslinje: Beløpstidslinje) {
        refusjonstidslinjer[dato] =
            refusjonstidslinjer.getOrDefault(dato, Beløpstidslinje()) + beløpstidslinje
        refusjonsrester[dato] =
            refusjonsrester.getOrDefault(dato, Beløpstidslinje()) + beløpstidslinje
    }
    ...
    internal fun servér(startdato: LocalDate, periode: Periode): Beløpstidslinje {
        ...
        aktuelle.keys.forEach { dato ->
            ...
            if (nyVerdi.isEmpty()) refusjonsrester.remove(dato) else refusjonsrester[dato] = nyVerdi
        }
    ...
    internal fun servér(other: Refusjonsservitør, aktivitetslogg: IAktivitetslogg) {
        this.refusjonsrester
            .filterValues { it.isNotEmpty() }
            .forEach { (dato, beløpstidslinje) ->
                aktivitetslogg.info(
                    "Refusjonsservitøren har rester for ${dato.format(formatter)} etter servering: ${beløpstidslinje.perioderMedBeløp.joinToString()}"
                )
    ...
    internal fun dto() =
        RefusjonsservitørDto(
            refusjonsrester.mapValues { (_, beløpstidslinje) -> beløpstidslinje.dto() }
        )
    ...
    internal companion object {
        ...
        internal fun fra(
            ...
            val sisteBit =
                sorterteStartdatoer.last() to
                    refusjonstidslinje.fraOgMed(sorterteStartdatoer.last())
        ...
        internal fun gjenopprett(dto: RefusjonsservitørDto) =
            Refusjonsservitør(
                dto.refusjonstidslinjer
                    .mapValues { (_, beløpstidslinje) ->
                        Beløpstidslinje.gjenopprett(beløpstidslinje)
                    }
                    .toSortedMap()
            )
    }
