...
internal class Refusjonsservitør(
    ...
    internal fun servér(
        ...
        this.refusjonsrester.filterValues { it.isNotEmpty() }.forEach { (dato, beløpstidslinje) ->
            aktivitetslogg.info(
                "Refusjonsservitøren har rester for ${dato.format(
                    formatter,
                )} etter servering: ${beløpstidslinje.perioderMedBeløp.joinToString()}",
            )
    ...
    internal companion object {
        ...
        internal fun gjenopprett(dto: RefusjonsservitørDto) =
            Refusjonsservitør(
                dto.refusjonstidslinjer.mapValues { (_, beløpstidslinje) -> Beløpstidslinje.gjenopprett(beløpstidslinje) }.toSortedMap(),
            )
    }
}
