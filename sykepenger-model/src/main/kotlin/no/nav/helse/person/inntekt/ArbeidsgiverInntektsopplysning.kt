...
data class ArbeidsgiverInntektsopplysning(
    ...
    private fun overstyr(
        overstyringer: List<ArbeidsgiverInntektsopplysning>
    ): ArbeidsgiverInntektsopplysning {
        val overstyring =
            overstyringer.singleOrNull { it.orgnummer == this.orgnummer } ?: return this
    ...
    private fun medBeløpstidslinje(kilde: Kilde): NyInntektUnderveis {
        return NyInntektUnderveis(
            ...
            beløpstidslinje =
                Beløpstidslinje(gjelder.map { Beløpsdag(it, inntektsopplysning.beløp, kilde) }),
        )
    }

    private fun overstyrer(gammel: ArbeidsgiverInntektsopplysning): ArbeidsgiverInntektsopplysning {
        val nyGjelder =
            when (this.inntektsopplysning) {
    ...
    private fun subsummer(subsumsjonslogg: Subsumsjonslogg, opptjening: Opptjening?) {
        inntektsopplysning.subsumerSykepengegrunnlag(
            subsumsjonslogg,
            orgnummer,
            opptjening?.startdatoFor(orgnummer),
        )
    }
    ...
    internal fun arbeidsgiveropplysningerKorrigert(
        ...
        inntektsopplysning.arbeidsgiveropplysningerKorrigert(
            person,
            orgnummer,
    ...
    internal companion object {
        internal fun List<ArbeidsgiverInntektsopplysning>.faktaavklarteInntekter() =
            this.map {
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.finn(orgnummer: String) = firstOrNull {
            it.gjelder(orgnummer)
        }
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.aktiver(
            ...
            val (deaktiverte, aktiverte) =
                this.fjernInntekt(aktiveres, orgnummer, forklaring, false, subsumsjonslogg)
            // Om inntektene i sykepengegrunnlaget var skjønnsmessig fastsatt før aktivering sikrer
            // vi at alle "rulles tilbake" slik at vi ikke lager et sykepengegrunnlag med mix av
            // SkjønnsmessigFastsatt & andre inntektstyper.
            return deaktiverte to
                aktiverte.map {
                    ArbeidsgiverInntektsopplysning(
                        it.orgnummer,
                        it.gjelder,
        ...
        // aktive.deaktiver(deaktiverte, orgnummer) er direkte motsetning til
        // deaktiverte.deaktiver(aktive, orgnummer)
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.overstyrInntekter(
            ...
            val endringen =
                this.map { inntekt -> inntekt.overstyr(other) }
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.overstyrTilkommendeInntekter(
            ...
            val somBeløpstidslinjer =
                this.mapNotNull { inntekt ->
                    if (
                        !nyInntektUnderveis.erRelevantForOverstyring(
                            skjæringstidspunkt,
                            inntekt.gjelder,
                        )
                    )
        ...
        private fun omregnetÅrsinntekter(
            ...
            return opplysninger
                .filter { it.gjelderPåSkjæringstidspunktet(skjæringstidspunkt) }
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.markerFlereArbeidsgivere(
            aktivitetslogg: IAktivitetslogg
        ) {
            return map { it.inntektsopplysning }.markerFlereArbeidsgivere(aktivitetslogg)
        }

        internal fun List<ArbeidsgiverInntektsopplysning>.refusjonsopplysninger(
            organisasjonsnummer: String
        ) =
            singleOrNull { it.gjelder(organisasjonsnummer) }?.refusjonsopplysninger
                ?: Refusjonsopplysninger()
        ...
        >? {
            ...
            val inntekt =
                PersonObserver.FastsattInntekt(
                    fastsattOpplysning.fastsattÅrsinntekt(skjæringstidspunkt)
                )
            val forslag = inntektforslag(skjæringstidspunkt, fastsattOpplysning)
            val refusjon =
                PersonObserver.Refusjon(
                    forslag =
                        fastsattOpplysning.refusjonsopplysninger
                            .overlappendeEllerSenereRefusjonsopplysninger(periode)
                            .map {
        ...
        private fun inntektforslag(
            ...
            val originalInntektsopplysning =
                arbeidsgiverInntektsopplysning.inntektsopplysning.omregnetÅrsinntekt()
            val type =
                when (originalInntektsopplysning) {
                    is no.nav.helse.person.inntekt.Inntektsmelding ->
        ...
        private fun List<ArbeidsgiverInntektsopplysning>.finnEndredeInntektsopplysninger(
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.berik(builder: UtkastTilVedtakBuilder) =
            this.forEach { arbeidsgiver ->
                builder.arbeidsgiverinntekt(
                    ...
                    omregnedeÅrsinntekt =
                        arbeidsgiver.inntektsopplysning.omregnetÅrsinntekt().fastsattÅrsinntekt(),
                    skjønnsfastsatt =
                        if (arbeidsgiver.inntektsopplysning is SkjønnsmessigFastsatt)
                            arbeidsgiver.inntektsopplysning.fastsattÅrsinntekt()
                        else null,
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.ingenRefusjonsopplysninger(
            organisasjonsnummer: String
        ): Boolean {
            val refusjonsopplysninger =
                singleOrNull { it.orgnummer == organisasjonsnummer }?.refusjonsopplysninger
                    ?: Refusjonsopplysninger()
            ...
            val endringsDatoer =
                this.mapNotNull { ny ->
                    ...
                    when {
                        ...
                        ny.inntektsopplysning != gammel.inntektsopplysning ->
                            minOf(ny.gjelder.start, gammel.gjelder.start)
                        else ->
                            ny.refusjonsopplysninger.finnFørsteDatoForEndring(
                                gammel.refusjonsopplysninger
                            )
                    }
                }
        ...
        internal fun List<ArbeidsgiverInntektsopplysning>.harGjenbrukbareOpplysninger(
            organisasjonsnummer: String
        ) =
            singleOrNull { it.orgnummer == organisasjonsnummer }
                ?.inntektsopplysning
                ?.gjenbrukbarInntekt() != null
        ...
        internal fun gjenopprett(
            ...
            return ArbeidsgiverInntektsopplysning(
                ...
                inntektsopplysning =
                    Inntektsopplysning.gjenopprett(dto.inntektsopplysning, inntekter),
