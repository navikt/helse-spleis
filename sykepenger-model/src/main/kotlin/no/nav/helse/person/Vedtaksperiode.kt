...
internal class Vedtaksperiode private constructor(
    ...
    private fun harSammeArbeidsgiverSom(hendelse: SykdomstidslinjeHendelse) = hendelse.organisasjonsnummer() == organisasjonsnummer

    internal fun harUlikFagsystemId(other: Utbetaling) = utbetaling?.hørerSammen(other) == false

    internal fun nyRevurderingFør(revurdert: Vedtaksperiode, hendelse: OverstyrTidslinje) {
        if (revurdert > this || revurdert == this) return

        kontekst(hendelse)

        if (harSammeArbeidsgiverSom(hendelse) && harUlikFagsystemId(revurdert.utbetaling())) {
    ...
    private fun forberedMuligUtbetaling(inntektsmelding: Inntektsmelding) {
        ...
        håndterInntektsmelding(
            inntektsmelding, when {
                overlappendeVedtaksperioder.alleAndreAvventerArbeidsgivere() -> AvventerUtbetalingsgrunnlag
                else -> AvventerArbeidsgivere
            },
    ...
    internal object MottattSykmeldingFerdigForlengelse : Vedtaksperiodetilstand {
        ...
        override fun håndter(vedtaksperiode: Vedtaksperiode, inntektsmelding: Inntektsmelding) {
            inntektsmelding.trimLeft(vedtaksperiode.periode.endInclusive) //Dette var her fra før, vet ikke hva det betyr 😵
            //TODO Ikke lag warning hvis forrige periode er en kort periode... Yikes! 🤯
            if (!false)
                inntektsmelding.warn("Mottatt flere inntektsmeldinger - den første inntektsmeldingen som ble mottatt er lagt til grunn. Utbetal kun hvis det blir korrekt.")
        }
    }
    ...
    internal object AvventerUferdigGap : Vedtaksperiodetilstand {
        ...
        override fun håndter(vedtaksperiode: Vedtaksperiode, gjenopptaBehandling: GjenopptaBehandling) {
            vedtaksperiode.tilstand(gjenopptaBehandling, AvventerUtbetalingsgrunnlag)
        }
    ...
    internal object AvventerUferdigForlengelse : Vedtaksperiodetilstand {
        ...
        override fun håndter(vedtaksperiode: Vedtaksperiode, gjenopptaBehandling: GjenopptaBehandling) {
            return vedtaksperiode.tilstand(gjenopptaBehandling, AvventerUtbetalingsgrunnlag)
        }
    }
    ...
    internal object Avsluttet : Vedtaksperiodetilstand {
        ...
        override fun håndterTidligereUferdigPeriode(vedtaksperiode: Vedtaksperiode, underRevurdering: Vedtaksperiode, hendelse: IAktivitetslogg) {
            if (Toggles.RevurderTidligerePeriode.enabled) vedtaksperiode.tilstand(hendelse, AvventerArbeidsgivereRevurdering)
            else super.håndterTidligereUferdigPeriode(vedtaksperiode, underRevurdering, hendelse)
        }
