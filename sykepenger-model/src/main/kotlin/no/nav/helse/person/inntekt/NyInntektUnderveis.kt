...
data class NyInntektUnderveis(val orgnummer: String, val beløpstidslinje: Beløpstidslinje) {
    fun dto() =
        NyInntektUnderveisDto(orgnummer = orgnummer, beløpstidslinje = beløpstidslinje.dto())
    ...
    companion object {
        internal fun List<NyInntektUnderveis>.finnEndringsdato(
            ...
            if (nyeOrgnr.isNotEmpty())
                return nyeOrgnr.minOfOrNull { it.beløpstidslinje.first().dato }
            return tidligere.førsteEndring(tidligere.merge(this))
        }

        private fun List<NyInntektUnderveis>.førsteEndring(
            others: List<NyInntektUnderveis>
        ): LocalDate? {
            val datoer = mapNotNull { nyInntektUnderveis ->
                val other =
                    others.find { it.orgnummer == nyInntektUnderveis.orgnummer }
        ...
        internal fun List<NyInntektUnderveis>.overstyr(
            hendelse: OverstyrArbeidsgiveropplysninger
        ): List<NyInntektUnderveis> {
            return hendelse.overstyr(this)
        }

        internal fun List<NyInntektUnderveis>.merge(
            nyeInntekter: List<NyInntektUnderveis>
        ): List<NyInntektUnderveis> {
            if (nyeInntekter.isEmpty()) return this
            val periode =
                nyeInntekter.minOf { it.beløpstidslinje.first().dato } til
                    nyeInntekter.maxOf { it.beløpstidslinje.last().dato }
        ...
        private fun List<NyInntektUnderveis>.overskrivTilkommetInntekterForPeriode(
            ...
            return map { tilkommetInntekt ->
                // tom liste/null som resultat tolkes som av søknaden har fjernet inntekten i den
                // angitte perioden
                val nyTidslinje =
                    nyeInntekter
                        .firstOrNull { it.orgnummer == tilkommetInntekt.orgnummer }
                ...
                tilkommetInntekt.copy(
                    beløpstidslinje = tidslinjeFørPerioden + nyTidslinje + tidslinjeEtterPerioden
                )
            }
        }

        fun gjenopprett(dto: NyInntektUnderveisDto) =
            NyInntektUnderveis(
