...
abstract class Utbetalingsperiode(
    ...
    private fun nyDag(builder: Utbetalingstidslinje.Builder, dato: LocalDate) {
        ...
        if (dato.erHelg()) return builder.addHelg(dato, økonomi.inntekt(INGEN, skjæringstidspunkt = dato, `6G` = INGEN, refusjonsbeløp = INGEN))
        builder.addNAVdag(dato, økonomi.inntekt(inntekt, skjæringstidspunkt = dato, `6G` = INGEN, refusjonsbeløp = INGEN))
    }
