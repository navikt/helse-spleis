...
internal class InfotrygdUtbetalingstidslinjedekoratør(
    ...
    private val infotrygdkilde: Hendelseskilde
    ...
    override fun utbetalingsdag(dato: LocalDate, økonomi: Økonomi, kilde: Hendelseskilde) {
        leggTil(dato, kilde) { other.utbetalingsdag(dato, økonomi, kilde) }
    }
    ...
    private fun leggTil(dato: LocalDate, hendelseskilde: Hendelseskilde = INGEN, block: () -> Unit) {
        if (dato !in spleisPeriode) return // Legger kun til dager innenfor Spleis sin sykdomstidslinje
        if (hendelseskilde === infotrygdkilde) return // Er en dag som er utbetalt i Infotrygd
        leggTilUkjenteDagerIForkant(dato)
        block()
        forrigeIkkeUkjenteDag = dato
    }

    private fun leggTilUkjenteDagerIForkant(dato: LocalDate) {
