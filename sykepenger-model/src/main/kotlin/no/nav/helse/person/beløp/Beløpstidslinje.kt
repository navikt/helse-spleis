...
data class Beløpstidslinje(private val dager: SortedMap<LocalDate, Beløpsdag>) :
    ...
    constructor(dager: List<Beløpsdag> = emptyList()) : this(dager.associateBy { it.dato }
        .toSortedMap().also {
    ...
    internal fun fyll(): Beløpstidslinje {
        ...
        val fylteDager = this.map { dag ->
            when (dag) {
                ...
                is UkjentDag -> forrigeBeløpsdag.copy(dato = forrigeBeløpsdag.dato.nesteDag)
                    .also { forrigeBeløpsdag = it }
            }
        }
        ...
        perioder = dager
            ...
            .fold(emptyList()) { result, dag ->
                when {
                    ...
                    result.last().kanUtvidesAv(dag) -> result.dropLast(1) + result.last()
                        .copy(tom = dag.tom)
