...
data class Beløpstidslinje(private val dager: SortedMap<LocalDate, Beløpsdag>) :
    Collection<Dag> by dager.values {
    private constructor(dager: Map<LocalDate, Beløpsdag>) : this(dager.toSortedMap())

    constructor(
        dager: List<Beløpsdag> = emptyList()
    ) : this(
        dager
            .associateBy { it.dato }
            .toSortedMap()
            .also {
                require(dager.size == it.size) {
                    "Forsøkte å opprette en beløpstidslinje med duplikate datoer. Det blir for rart for meg."
                }
            }
    )
    ...
    operator fun plus(other: Beløpstidslinje) =
        merge(other, BevareEksisterendeOpplysningHvisLikeBeløp)

    private fun merge(
        other: Beløpstidslinje,
        strategi: BesteRefusjonsopplysningstrategi,
    ): Beløpstidslinje {
        val results = this.dager.toMutableMap()
        other.dager.forEach { (key, dag) -> results.merge(key, dag, strategi) }
        return Beløpstidslinje(results)
    }

    internal operator fun minus(datoer: Iterable<LocalDate>) =
        Beløpstidslinje(this.dager.filterKeys { it !in datoer })
    ...
    internal operator fun minus(dato: LocalDate) =
        Beløpstidslinje(this.dager.filterKeys { it != dato })
    ...
    internal fun subset(periode: Periode): Beløpstidslinje {
        ...
        return Beløpstidslinje(
            dager.subMap(periode.start, periode.endInclusive.nesteDag).toSortedMap()
        )
    }

    internal fun tilOgMed(dato: LocalDate) =
        Beløpstidslinje(dager.headMap(dato.nesteDag).toSortedMap())
        ...
        val fylteDager =
            this.map { dag ->
                when (dag) {
                    ...
                    is UkjentDag ->
                        forrigeBeløpsdag.copy(dato = forrigeBeløpsdag.dato.nesteDag).also {
                            forrigeBeløpsdag = it
                        }
                }
            }
                ...
                dager
                    ...
                    .fold(emptyList()) { result, dag ->
                        when {
                            ...
                            result.last().kanUtvidesAv(dag) ->
                                result.dropLast(1) + result.last().copy(tom = dag.tom)
    ...
    companion object {
        private val BevareEksisterendeOpplysningHvisLikeBeløp: BesteRefusjonsopplysningstrategi =
            { venstre: Beløpsdag, høyre: Beløpsdag ->
                when {
        ...
        fun fra(periode: Periode, beløp: Inntekt, kilde: Kilde) =
            Beløpstidslinje(periode.map { Beløpsdag(it, beløp, kilde) })

        internal fun gjenopprett(dto: BeløpstidslinjeDto) =
            Beløpstidslinje(
                dager =
                    dto.perioder.flatMap {
...
data class Beløpsdag(
    ...
    fun strekkTilbake(datoFør: LocalDate) =
        if (datoFør > dato) Beløpstidslinje() else strekk(datoFør til dato)

    fun strekkFrem(datoEtter: LocalDate) =
        if (datoEtter < dato) Beløpstidslinje() else strekk(dato til datoEtter)
}
...
data object UkjentDag : Dag {
    ...
    override val kilde
        get() = error("En ukjent dag har ikke et kilde")
}
