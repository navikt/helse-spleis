...
class Vilkårsgrunnlag(
    ...
    private val harInntektMånedenFørSkjæringstidspunkt =
        inntekterForOpptjeningsvurdering.harInntektI(
            YearMonth.from(skjæringstidspunkt).minusMonths(1)
        )
            ...
            grunnlag =
                opptjeningsgrunnlag.map { (orgnummer, ansattPerioder) ->
                    Opptjening.ArbeidsgiverOpptjeningsgrunnlag(
                        orgnummer,
                        ansattPerioder.map { it.tilDomeneobjekt() },
                    )
                },
    ...
    internal fun avklarSykepengegrunnlag(
        ...
        return inntektsvurderingForSykepengegrunnlag.inntekter.avklarSykepengegrunnlag(
            aktivitetslogg,
            person,
            rapporterteArbeidsforhold,
            skjæringstidspunkt,
    ...
    internal fun valider(
        ...
        if (!harInntektMånedenFørSkjæringstidspunkt) {
            ...
            !inntektsvurderingForSykepengegrunnlag.inntekter.harInntektI(
                YearMonth.from(skjæringstidspunkt.minusMonths(1))
            )
        ) {
            aktivitetslogg.info(
                "Har inntekt måneden før skjæringstidspunkt med inntekter for opptjeningsvurdering, men ikke med inntekter for sykepengegrunnlag"
            )
        }
    ...
    class Arbeidsforhold(
        ...
        ) : this(orgnummer, ansattFom til (ansattTom ?: LocalDate.MAX), type)
        ...
        fun validerFrilans(
            ...
            harFrilansinntekterDeSiste3Månedene(
                aktivitetslogg,
                skjæringstidspunkt,
                inntektsvurderingForSykepengegrunnlag,
            )
        ...
        private fun harFrilansinntekterDeSiste3Månedene(
            ...
            val finnerFrilansinntektDeSiste3Månedene =
                (1..3).any { antallMånederFør ->
                    ...
                    ansattIMåneden &&
                        inntektForSykepengegrunnlag.inntekter.harInntektFor(orgnummer, måned)
                }
            ...
            if (
                andre.count {
                    it.orgnummer == this.orgnummer &&
                        it.ansettelseperiode.overlapperMed(this.ansettelseperiode)
                } > 1
            ) {
                aktivitetslogg.info(
                    "Vedkommende har andre overlappende arbeidsforhold i samme virksomhet hvor vedkommende har frilansoppdrag"
                )
            }
        }
        ...
        private fun kanBrukes() =
            type != Arbeidsforholdtype.FRILANSER // filtrerer ut frilans-arbeidsforhold enn så lenge

        internal companion object {
            internal fun Iterable<Arbeidsforhold>.opptjeningsgrunnlag():
                Map<String, List<Arbeidsforhold>> {
                return this.filter { it.kanBrukes() }.groupBy { it.orgnummer }
            }
        }
    }
}
