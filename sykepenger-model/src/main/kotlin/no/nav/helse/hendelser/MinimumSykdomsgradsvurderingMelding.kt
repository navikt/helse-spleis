...
/**
 * Melding om perioder saksbehandler har vurdert dithet at bruker har tapt nok arbeidstid til å ha
 * rett på sykepenger, tross < 20% tapt inntekt
 */
class MinimumSykdomsgradsvurderingMelding(
    ...
    private fun sjekkForOverlapp() {
        perioderMedMinimumSykdomsgradVurdertOK.forEach {
            if (perioderMedMinimumSykdomsgradVurdertIkkeOK.contains(it)) {
                error(
                    "overlappende perioder i MinimumSykdomsgradsvurdering! $it er vurdert OK, men også vurdert til IKKE å være OK"
                )
            }
        }
    }
    ...
    fun valider(): Boolean {
        if (
            perioderMedMinimumSykdomsgradVurdertOK.isEmpty() &&
                perioderMedMinimumSykdomsgradVurdertIkkeOK.isEmpty()
        )
            return false
        if (
            perioderMedMinimumSykdomsgradVurdertOK.containsAll(
