...
class Foreldrepenger(private val foreldrepengeytelse: List<GradertPeriode>) :
    ...
    private val perioder
        get() = foreldrepengeytelse.map { it.periode }
    ...
    internal fun valider(
        ...
        if (foreldrepengeytelse.isEmpty())
            return aktivitetslogg.info("Bruker har ingen foreldrepenger")
    ...
    private fun varselHvisOverlapperMedForeldrepenger(
        ...
        val overlappsperiode =
            if (erForlengelse) sykdomsperiode else sykdomsperiode.familieYtelserPeriode
        val overlapperMedForeldrepenger =
            perioder.any { ytelse -> ytelse.overlapperMed(overlappsperiode) }
        if (overlapperMedForeldrepenger) {
            aktivitetslogg.varsel(Varselkode.`Overlapper med foreldrepenger`)
        } else {
            aktivitetslogg.info(
                "Bruker har foreldrepenger, men det slår ikke ut på overlappsjekken"
            )
        }
    }
    ...
    private fun varselHvisForlengerForeldrepengerMerEnn14Dager(
        ...
        val foreldrepengeperiodeFør =
            foreldrepengeytelse.lastOrNull { it.periode.endInclusive < sykdomsperiode.start }
                ?: return
        val harForeldrepengerAlleDager =
            foreldrepengeperiodeFør.periode.erRettFør(sykdomsperiode) &&
                foreldrepengeperiodeFør.periode.count() > 14 &&
                foreldrepengeperiodeFør.grad == 100
    ...
    override fun sykdomstidslinje(
        ...
        require(foreldrepengeytelse.map { it.periode }.grupperSammenhengendePerioder().size == 1) {
            "Ikke trygt å kalle sykdomstidslinjen til ${this.javaClass.simpleName} når det er huller i ytelser"
        }
        val hendelseskilde =
            SykdomshistorikkHendelse.Hendelseskilde(Ytelser::class, meldingsreferanseId, registrert)
    ...
    ): Pair<Boolean, Companion.HvorforIkkeOppdatereHistorikk?> {
        return foreldrepengeytelse.skalOppdatereHistorikkIHalen(
            vedtaksperiode,
            skjæringstidspunkt,
