...
internal class DagerFraInntektsmelding(
    ...
    private companion object {
        ...
        private const val MAKS_ANTALL_DAGER_MELLOM_FØRSTE_FRAVÆRSDAG_OG_AGP_FOR_HÅNDTERING_AV_DAGER =
            20
    }

    // TODO: kilden må være av en type som arver SykdomshistorikkHendelse; altså
    // BitAvInntektsmelding
    // krever nok at vi json-migrerer alle "Inntektsmelding" til "BitAvInntektsmelding" først
    internal val kilde =
        Hendelseskilde("Inntektsmelding", hendelse.metadata.meldingsreferanseId, mottatt)
    ...
    private val begrunnelseForReduksjonEllerIkkeUtbetalt =
        begrunnelseForReduksjonEllerIkkeUtbetalt.takeUnless { it.isNullOrBlank() }
    private val arbeidsgiverperiode = arbeidsgiverperioder.periode()
    private val overlappsperiode =
        when {
            // første fraværsdag er oppgitt etter arbeidsgiverperioden
            arbeidsgiverperiode == null ||
                førsteFraværsdag != null &&
                    førsteFraværsdag > arbeidsgiverperiode.endInclusive.nesteDag ->
                førsteFraværsdag?.somPeriode()
            // kant-i-kant
            førsteFraværsdag?.forrigeDag == arbeidsgiverperiode.endInclusive ->
                arbeidsgiverperiode.oppdaterTom(arbeidsgiverperiode.endInclusive.nesteDag)
    ...
    private val validator =
        if (
            avsendersystem is NavPortal &&
                avsendersystem.forespurt &&
                arbeidsgiverperioder.isEmpty()
        )
            ForespurtPortalinntektsmeldingUtenArbeidsgiverperiodeValidering
    ...
    val gjenståendeDager
        get() = _gjenståendeDager.toSet()
    ...
    private val ignorerDager: Boolean
        get() {
            ...
            if (begrunnelseForReduksjonEllerIkkeUtbetalt in ikkeStøttedeBegrunnelserForReduksjon)
                return true
    ...
    private fun tidslinjeForFørsteFraværsdag(): Sykdomstidslinje {
        if (begrunnelseForReduksjonEllerIkkeUtbetalt == null || førsteFraværsdag == null)
    ...
    private fun tidslinjeForArbeidsgiverperioden(): Sykdomstidslinje? {
        ...
        val arbeidsdager =
            arbeidsgiverperiode?.let { Sykdomstidslinje.arbeidsdager(arbeidsgiverperiode, kilde) }
                ?: Sykdomstidslinje()
    ...
    private fun arbeidsgiverperiodenKanIkkeTolkes(): Boolean {
        ...
        return periodeMellom != null &&
            periodeMellom.count() >=
                MAKS_ANTALL_DAGER_MELLOM_FØRSTE_FRAVÆRSDAG_OG_AGP_FOR_HÅNDTERING_AV_DAGER
    }
    ...
    private fun friskHelgMellomFørsteFraværsdagOgArbeidsgiverperioden(): Sykdomstidslinje {
        ...
        val helgen =
            arbeidsgiverperiode.periodeMellom(førsteFraværsdag) ?: return Sykdomstidslinje()
    ...
    private fun lagArbeidsgivertidslinje(): Sykdomstidslinje {
        ...
        return agp.merge(
            arbeidsgiverperiodeNavSkalUtbetale().map(::sykedagerNav).merge(),
            Dag.replace,
        )
    }
    ...
    private fun arbeidsgiverperiodeNavSkalUtbetale(): List<Periode> {
        if (
            arbeidsgiverperiode != null &&
                (førsteFraværsdag == null || førsteFraværsdag in arbeidsgiverperiode)
        )
            return arbeidsgiverperioder
        return listOfNotNull(førsteFraværsdag?.somPeriode())
    }

    private fun arbeidsgiverdager(periode: Periode) =
        Sykdomstidslinje.arbeidsgiverdager(periode.start, periode.endInclusive, 100.prosent, kilde)

    private fun sykedagerNav(periode: Periode) =
        Sykdomstidslinje.sykedagerNav(periode.start, periode.endInclusive, 100.prosent, kilde)

    internal fun alleredeHåndtert(behandlinger: Behandlinger) =
        behandlinger.dokumentHåndtert(dokumentsporing)
    ...
    internal fun skalHåndteresAv(periode: Periode): Boolean {
        val vedtaksperiodeRettFør =
            _gjenståendeDager.isNotEmpty() &&
                periode.endInclusive.erRettFør(_gjenståendeDager.first())
        return skalHåndtere(periode) ||
            vedtaksperiodeRettFør ||
            tomSykdomstidslinjeMenSkalValidere(periode) ||
            egenmeldingerIForkantAvPerioden(periode)
    }
    ...
    // om vedtaksperioden ikke direkte overlapper med gjenståendeDager, men gjenståendeDager er ikke
    // tom, og vedtaksperioden overlapper
    // med første fraværsdag, betyr det at inntektsmeldingen informerer om egenmeldinger vi ikke har
    // søknad for.
    ...
    private fun egenmeldingerIForkantAvPerioden(periode: Periode) =
        (overlappsperiode != null &&
            overlappsperiode.overlapperMed(periode) &&
            _gjenståendeDager.isNotEmpty())

    internal fun skalHåndteresAvRevurdering(
        periode: Periode,
        sammenhengende: Periode,
        arbeidsgiverperiode: Arbeidsgiverperiode?,
    ): Boolean {
        if (skalHåndtere(periode)) return true
        // vedtaksperiodene før dagene skal bare håndtere dagene om de nye opplyste dagene er
        // nærmere enn 10 dager fra forrige AGP-beregning
        if (opprinneligPeriode == null || arbeidsgiverperiode == null) return false
        val periodeMellomForrigeAgpOgNyAgp =
            arbeidsgiverperiode.omsluttendePeriode?.periodeMellom(opprinneligPeriode.start)
                ?: return false
        return periodeMellomForrigeAgpOgNyAgp.count() <=
            MAKS_ANTALL_DAGER_MELLOM_TIDLIGERE_OG_NY_AGP_FOR_HÅNDTERING_AV_DAGER &&
            sammenhengende.contains(periodeMellomForrigeAgpOgNyAgp)
    }

    internal fun harBlittHåndtertAv(periode: Periode) = håndterteDager.any { it in periode }

    internal fun bitAvInntektsmelding(
    ...
    internal fun tomBitAvInntektsmelding(
        aktivitetslogg: IAktivitetslogg,
        vedtaksperiode: Periode,
    ): BitAvInntektsmelding {
    ...
    private fun håndterDager(
        ...
        if (periode.start != vedtaksperiode.start)
            aktivitetslogg.info(
                "Perioden ble strukket tilbake fra ${vedtaksperiode.start} til ${periode.start} (${ChronoUnit.DAYS.between(periode.start, vedtaksperiode.start)} dager)"
            )
    ...
    private fun skalValideresAv(periode: Periode) =
        overlappsperiode?.overlapperMed(periode) == true || ikkeUtbetaltAGPOgAGPOverlapper(periode)

    private fun ikkeUtbetaltAGPOgAGPOverlapper(periode: Periode): Boolean {
        if (begrunnelseForReduksjonEllerIkkeUtbetalt == null) return false
        if (arbeidsgiverperiode == null) return false
        if (
            førsteFraværsdag != null && førsteFraværsdag > arbeidsgiverperiode.endInclusive.nesteDag
        )
            return false
    ...
    internal fun valider(
        aktivitetslogg: IAktivitetslogg,
        periode: Periode,
        gammelAgp: Arbeidsgiverperiode? = null,
    ) {
    ...
    private fun validerBegrunnelseForReduksjonEllerIkkeUtbetalt(aktivitetslogg: IAktivitetslogg) {
        ...
        aktivitetslogg.info(
            "Arbeidsgiver har redusert utbetaling av arbeidsgiverperioden på grunn av: %s"
                .format(begrunnelseForReduksjonEllerIkkeUtbetalt)
        )
        if (!kjenteBegrunnelserForReduksjon.contains(begrunnelseForReduksjonEllerIkkeUtbetalt)) {
            aktivitetslogg.info(
                "Kjenner ikke til betydning av $begrunnelseForReduksjonEllerIkkeUtbetalt"
            )
        }
    ...
    private fun hulleteArbeidsgiverperiode(): Boolean {
        return arbeidsgiverperioder.size > 1 &&
            (førsteFraværsdag == null || førsteFraværsdag in arbeidsgiverperiode!!)
    }
    ...
    internal fun validerArbeidsgiverperiode(
        aktivitetslogg: IAktivitetslogg,
        periode: Periode,
        beregnetArbeidsgiverperiode: Arbeidsgiverperiode?,
    ) {
        if (!skalValideresAv(periode)) return
        if (_gjenståendeDager.isNotEmpty())
            return validator.validerFeilaktigNyArbeidsgiverperiode(
                aktivitetslogg,
                periode,
                beregnetArbeidsgiverperiode,
            )
        if (beregnetArbeidsgiverperiode != null)
            validerArbeidsgiverperiode(aktivitetslogg, beregnetArbeidsgiverperiode)
    ...
    private fun validerArbeidsgiverperiode(
        aktivitetslogg: IAktivitetslogg,
        arbeidsgiverperiode: Arbeidsgiverperiode,
    ) {
    ...
    internal fun overlappendeSykmeldingsperioder(
    ...
    internal fun perioderInnenfor16Dager(sykmeldingsperioder: List<Periode>): List<Periode> {
        ...
        return sykmeldingsperioder.mapNotNull { sykmeldingsperiode ->
            ...
            val dagerMellom =
                overlappsperiode.periodeMellom(sykmeldingsperiode.start)?.count()
    ...
    private fun validerOverstigerMaksimaltTillatAvstandMellomTidligereAGP(
        aktivitetslogg: IAktivitetslogg,
        gammelAgp: Arbeidsgiverperiode?,
    ) {
        if (!overstigerMaksimaltTillatAvstandMellomTidligereAGP(gammelAgp)) return
        aktivitetslogg.varsel(
            Varselkode.RV_IM_24,
            "Ignorerer dager fra inntektsmelding fordi perioden mellom gammel agp og opplyst agp er mer enn 10 dager",
        )
    }

    private fun overstigerMaksimaltTillatAvstandMellomTidligereAGP(
        gammelAgp: Arbeidsgiverperiode?
    ): Boolean {
        if (opprinneligPeriode == null) return false
        val periodeMellom =
            gammelAgp?.omsluttendePeriode?.periodeMellom(opprinneligPeriode.start) ?: return false
        return periodeMellom.count() >
            MAKS_ANTALL_DAGER_MELLOM_TIDLIGERE_OG_NY_AGP_FOR_HÅNDTERING_AV_DAGER
    }

    internal fun erKorrigeringForGammel(
        aktivitetslogg: IAktivitetslogg,
        gammelAgp: Arbeidsgiverperiode?,
    ): Boolean {
        ...
        aktivitetslogg.info(
            "Håndterer dager fordi perioden mellom gammel agp og opplyst agp er mindre enn 10 dager"
        )
        return false
    }
    ...
    internal class BegrunnelseForReduksjonEllerIkkeUtbetalt {
        companion object {
            private val støttedeBegrunnelserForReduksjon =
                setOf(
                    "LovligFravaer",
                    "ArbeidOpphoert",
                    "ManglerOpptjening",
            ...
            internal val ikkeStøttedeBegrunnelserForReduksjon =
                setOf(
            ...
            internal val kjenteBegrunnelserForReduksjon =
                støttedeBegrunnelserForReduksjon + ikkeStøttedeBegrunnelserForReduksjon
            ...
            internal fun funksjonellBetydningAvBegrunnelseForReduksjonEllerIkkeUtbetalt(
                ...
                return when {
                    antallDagerIOpplystArbeidsgiverperiode in (1..15) ->
                        ARBEIDSGIVER_VIL_BARE_DEKKE_DELVIS_AGP
                    førsteFraværsdagStarterMerEnn16DagerEtterEtterSisteDagIAGP ->
                        ARBEIDSGIVER_VIL_IKKE_DEKKE_NY_AGP_TROSS_GAP
                    antallDagerIOpplystArbeidsgiverperiode == 0 ->
                        ARBEIDSGIVER_SIER_AT_DET_IKKE_ER_NOE_AGP_Å_SNAKKE_OM_I_DET_HELE_TATT
                    begrunnelseForReduksjonEllerIkkeUtbetalt in støttedeBegrunnelserForReduksjon ->
                        ARBBEIDSGIVER_VIL_AT_NAV_SKAL_DEKKE_AGP_FRA_FØRSTE_DAG
                    else -> UKJENT
                }
            }
    ...
    private sealed interface Validator {
        fun validerFeilaktigNyArbeidsgiverperiode(
            aktivitetslogg: IAktivitetslogg,
            vedtaksperiode: Periode,
            beregnetArbeidsgiverperiode: Arbeidsgiverperiode?,
        )
    ...
    private data object ForespurtPortalinntektsmeldingUtenArbeidsgiverperiodeValidering :
        Validator {
        override fun validerFeilaktigNyArbeidsgiverperiode(
            aktivitetslogg: IAktivitetslogg,
            vedtaksperiode: Periode,
            beregnetArbeidsgiverperiode: Arbeidsgiverperiode?,
        ) {}
    ...
    private data object DefaultValidering : Validator {
        override fun validerFeilaktigNyArbeidsgiverperiode(
            ...
            beregnetArbeidsgiverperiode?.validerFeilaktigNyArbeidsgiverperiode(
                vedtaksperiode,
                aktivitetslogg,
            )
        }
