...
internal class DagerFraInntektsmelding(
    ...
    private val overlappsperiode =
        when {
            ...
            arbeidsgiverperiode == null || førsteFraværsdag != null && førsteFraværsdag > arbeidsgiverperiode.endInclusive.nesteDag ->
                førsteFraværsdag
                    ?.somPeriode()
            // kant-i-kant
            førsteFraværsdag?.forrigeDag == arbeidsgiverperiode.endInclusive ->
                arbeidsgiverperiode.oppdaterTom(
    ...
    private val validator =
        if (avsendersystem is NavPortal &&
    ...
    private fun lagSykdomstidslinje(): Sykdomstidslinje = tidslinjeForArbeidsgiverperioden() ?: tidslinjeForFørsteFraværsdag()
    ...
    internal fun skalHåndteresAvRevurdering(
        ...
        return periodeMellomForrigeAgpOgNyAgp.count() <= MAKS_ANTALL_DAGER_MELLOM_TIDLIGERE_OG_NY_AGP_FOR_HÅNDTERING_AV_DAGER &&
    ...
    private fun håndterDager(
        ...
        if (periode.start !=
            ...
            aktivitetslogg.info(
                "Perioden ble strukket tilbake fra ${vedtaksperiode.start} til ${periode.start} (${ChronoUnit.DAYS.between(
                    periode.start,
                    vedtaksperiode.start,
                )} dager)",
            )
    ...
    private fun samletSykdomstidslinje(periode: Periode) = (arbeidsdagertidslinje() + sykdomstidslinje).subset(periode)

    private fun arbeidsdagertidslinje() = arbeidsdager.map { Sykdomstidslinje.arbeidsdager(it, it, kilde) }.merge()
    ...
    private fun validerBegrunnelseForReduksjonEllerIkkeUtbetalt(aktivitetslogg: IAktivitetslogg) {
        ...
        aktivitetslogg.info(
            "Arbeidsgiver har redusert utbetaling av arbeidsgiverperioden på grunn av: %s".format(begrunnelseForReduksjonEllerIkkeUtbetalt),
        )
    ...
    private fun hulleteArbeidsgiverperiode(): Boolean =
        arbeidsgiverperioder.size > 1 && (førsteFraværsdag == null || førsteFraværsdag in arbeidsgiverperiode!!)
    ...
    internal class BitAvInntektsmelding(
        ...
        override fun oppdaterFom(other: Periode) = other.oppdaterFom(sykdomstidslinje().periode() ?: other)
