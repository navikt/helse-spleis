...
internal class DagerFraInntektsmelding(
    ...
    private val overlappsperiode = when {
        ...
        // kant-i-kant
        førsteFraværsdag?.forrigeDag == arbeidsgiverperiode.endInclusive -> arbeidsgiverperiode.oppdaterTom(
            arbeidsgiverperiode.endInclusive.nesteDag
        )
    ...
    internal fun skalHåndteresAv(periode: Periode): Boolean {
        val vedtaksperiodeRettFør =
            _gjenståendeDager.isNotEmpty() && periode.endInclusive.erRettFør(_gjenståendeDager.first())
        return skalHåndtere(periode) || vedtaksperiodeRettFør || tomSykdomstidslinjeMenSkalValidere(
            periode
    ...
    internal fun skalHåndteresAvRevurdering(
        ...
        val periodeMellomForrigeAgpOgNyAgp =
            arbeidsgiverperiode.omsluttendePeriode?.periodeMellom(opprinneligPeriode.start)
                ?: return false
        return periodeMellomForrigeAgpOgNyAgp.count() <= MAKS_ANTALL_DAGER_MELLOM_TIDLIGERE_OG_NY_AGP_FOR_HÅNDTERING_AV_DAGER && sammenhengende.contains(
            periodeMellomForrigeAgpOgNyAgp
        )
    }
    ...
    private fun håndterDager(
        ...
        if (periode.start != vedtaksperiode.start) aktivitetslogg.info(
            "Perioden ble strukket tilbake fra ${vedtaksperiode.start} til ${periode.start} (${
                ChronoUnit.DAYS.between(
                    periode.start,
                    vedtaksperiode.start
                )
            } dager)"
        )
    ...
    private fun validerBegrunnelseForReduksjonEllerIkkeUtbetalt(aktivitetslogg: IAktivitetslogg) {
        ...
        aktivitetslogg.info(
            "Arbeidsgiver har redusert utbetaling av arbeidsgiverperioden på grunn av: %s".format(
    ...
    internal fun validerArbeidsgiverperiode(
        ...
        if (!skalValideresAv(periode)) return
        if (_gjenståendeDager.isNotEmpty()) return validator.validerFeilaktigNyArbeidsgiverperiode(
            aktivitetslogg,
            periode,
            beregnetArbeidsgiverperiode
        )
        if (beregnetArbeidsgiverperiode != null) validerArbeidsgiverperiode(
            aktivitetslogg,
            beregnetArbeidsgiverperiode
        )
