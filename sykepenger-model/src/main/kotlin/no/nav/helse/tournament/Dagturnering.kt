...
internal class Dagturnering private constructor(private val source: String) {
    ...
    private fun readStrategies(): Map<Turneringsnøkkel, Map<Turneringsnøkkel, Strategy>> {
        ...
        return csv.drop(1).associate { (key, row) ->
            enumValueOf<Turneringsnøkkel>(key) to
                columnHeaders
                    .zip(row)
                    .filter { (_, cell) -> cell.isNotBlank() }
                    .associate { (columnHeader, cell) ->
                        enumValueOf<Turneringsnøkkel>(columnHeader) to strategyFor(cell)
                    }
        }
    }
        ...
        when (cellValue) {
            ...
            "L" -> Latest
                ...
                throw RuntimeException(
                    "$cellValue is not a known strategy for deciding between days"
                )
        }
}
...
internal object Impossible : Strategy() {
    override fun decide(row: Dag, column: Dag): Dag =
        throw RuntimeException(
            "Nøklene ${Turneringsnøkkel.fraDag(row)} + ${Turneringsnøkkel.fraDag(column)} er en ugyldig sammenligning"
        )
