...
internal class Feriepengeutbetaling private constructor(
    ...
    companion object {
        fun List<Feriepengeutbetaling>.gjelderFeriepengeutbetaling(hendelse: UtbetalingHendelse) =
            any {
                hendelse.fagsystemId ==
                    it.oppdrag.fagsystemId ||
                    hendelse.fagsystemId == it.personoppdrag.fagsystemId
            }

        internal fun gjenopprett(
            alder: Alder,
            dto: FeriepengeInnDto,
    ...
    fun håndter(
        ...
        if (utbetalingHendelse.utbetalingId != this.utbetalingId ||
            utbetalingHendelse.fagsystemId !in setOf(oppdrag.fagsystemId, personoppdrag.fagsystemId)
        ) {
            return
        }
    ...
    internal class Builder(
        ...
        private val List<Periode>.finere get() =
            joinToString(
                separator = "\n\t\t\t\t\t",
                prefix = "\n\t\t\t\t\t",
            ) { "$it (${it.count()} dager)" }
        ...
        internal fun build(aktivitetslogg: IAktivitetslogg): Feriepengeutbetaling {
            ...
            val hvaViHarBeregnetAtInfotrygdHarUtbetaltTilArbeidsgiver =
                feriepengeberegner
                    .beregnUtbetalteFeriepengerForInfotrygdArbeidsgiver(
                        orgnummer,
                    )
            ...
            val differanseMellomTotalOgAlleredeUtbetaltAvInfotrygd: Double =
                feriepengeberegner.beregnFeriepengedifferansenForArbeidsgiver(
                    orgnummer,
                )
            ...
            val arbeidsgiveroppdrag =
                oppdrag(aktivitetslogg, Fagområde.SykepengerRefusjon, forrigeSendteArbeidsgiverOppdrag, arbeidsgiverbeløp)
            ...
            val differanseMellomTotalOgAlleredeUtbetaltAvInfotrygdTilPerson =
                feriepengeberegner.beregnFeriepengedifferansenForPerson(
                    orgnummer,
                )
            ...
            val hvaViHarBeregnetAtInfotrygdHarUtbetaltTilPersonForDenneAktuelleArbeidsgiver =
                feriepengeberegner
                    .beregnUtbetalteFeriepengerForInfotrygdPersonForEnArbeidsgiver(
                        orgnummer,
                    )
            ...
            if (differanseMellomTotalOgAlleredeUtbetaltAvInfotrygdTilPerson < -499 ||
            ...
            val arbeidsgiveroppdragdetaljer =
                PersonObserver.FeriepengerUtbetaltEvent.OppdragEventDetaljer
                    .mapOppdrag(
                        arbeidsgiveroppdrag,
                    ).toString()
            ...
            aktivitetslogg.info(
                ...
                - GENERELT:         
                ${feriepengeberegner.feriepengedatoer().let { datoer ->
                    "Datoer vi skal utbetale feriepenger for (${datoer.size}): ${datoer.grupperSammenhengendePerioder().finere}"
                }}
                
                - OPPDRAG:
                Skal sende arbeidsgiveroppdrag til OS: $sendArbeidsgiveroppdrag
                Differanse fra forrige sendte arbeidsgoiveroppdrag: ${forrigeSendteArbeidsgiverOppdrag?.totalbeløp()?.minus(
                    arbeidsgiveroppdrag.totalbeløp(),
                )}
