...
internal class UtbetalingstidslinjeBuilder(utbetalingstidslinje: Utbetalingstidslinje) {
    ...
    private val navdager =
        utbetalingstidslinje.mapNotNull { dag ->
            when (dag) {
                ...
                is Utbetalingsdag.NavHelgDag -> {
                    if (forrigeDag?.erAvvistDag() == true) visit(dag.dato, "AVVISTDAG", dag.økonomi)
                    else visit(dag.dato, "NAVDAG", dag.økonomi)
                }
    ...
    companion object {
        ...
        internal fun List<Utbetalingstidslinje>.subsumsjonsformat(): List<List<Tidslinjedag>> =
            map { it.subsumsjonsformat() }.filter { it.isNotEmpty() }
    }
}
