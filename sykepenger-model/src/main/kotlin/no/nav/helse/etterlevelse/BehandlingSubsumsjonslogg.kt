...
class BehandlingSubsumsjonslogg(
    ...
    private fun sjekkKontekster() {
        ...
        check(
            kritiskeTyper.all { kritiskType -> kontekster.count { it.type == kritiskType } == 1 }
        ) {
            "en av $kritiskeTyper mangler/har duplikat:\n${kontekster.joinToString(separator = "\n")}"
        }
        // todo: sjekker for mindre enn 1 også ettersom noen subsumsjoner skjer på arbeidsgivernivå.
        // det burde vi forsøke å flytte/fikse slik at
    ...
    fun medFødselsnummer(personidentifikator: String) =
        kopierMedKontekst(
            listOf(Subsumsjonskontekst(KontekstType.Fødselsnummer, personidentifikator))
        )

    fun medOrganisasjonsnummer(organisasjonsnummer: String) =
        kopierMedKontekst(
            listOf(Subsumsjonskontekst(KontekstType.Organisasjonsnummer, organisasjonsnummer))
        )

    fun medVedtaksperiode(vedtaksperiodeId: UUID, hendelseIder: List<Subsumsjonskontekst>) =
        kopierMedKontekst(
            listOf(Subsumsjonskontekst(KontekstType.Vedtaksperiode, vedtaksperiodeId.toString())) +
                hendelseIder
        )

    fun medInntektsmelding(inntektsmeldingId: UUID) =
        kopierMedKontekst(
            listOf(Subsumsjonskontekst(KontekstType.Inntektsmelding, inntektsmeldingId.toString()))
        )
