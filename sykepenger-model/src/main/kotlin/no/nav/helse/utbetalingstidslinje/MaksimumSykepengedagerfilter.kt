    ...
    override fun filter(
        ...
        val begrunnelser =
            (maksdatosaker.plusElement(sisteVurdering))
                .flatMap { maksdatosak ->
                    maksdatosak.begrunnelseForAvslåtteDager(alder, arbeidsgiverRegler, TILSTREKKELIG_OPPHOLD_I_SYKEDAGER)
    ...
    private fun håndterBetalbarDag(dagen: LocalDate) {
        ...
        when {
            sisteVurdering.erDagerUnder67ÅrForbrukte(alder, arbeidsgiverRegler) ||
                sisteVurdering.erDagerOver67ÅrForbrukte(alder, arbeidsgiverRegler) -> state(Karantene)
            else -> state(Syk)
        }
    }
