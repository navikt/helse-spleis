...
internal data class Maksdatokontekst(
    ...
    internal fun gjenståendeDagerUnder67År(alder: Alder, regler: ArbeidsgiverRegler) =
        regler.maksSykepengedager() - forbrukteDager
    ...
    internal fun fremdelesSykEtterTilstrekkeligOpphold(
        ...
        val datoForTilstrekkeligOpphold =
            datoForTilstrekkeligOppholdOppnådd(tilstrekkeligOpphold) ?: return false
    ...
    internal fun begrunnelseForAvslåtteDager(
        ...
        val datoForTilstrekkeligOppholdOppnådd =
            datoForTilstrekkeligOppholdOppnådd(tilstrekkeligOpphold)
        return avslåtteDager.map { avslåttDag ->
            val begrunnelseForAvslåttDag =
                when {
                    alder.mistetSykepengerett(avslåttDag) -> Begrunnelse.Over70
                    datoForTilstrekkeligOppholdOppnådd != null &&
                        avslåttDag > datoForTilstrekkeligOppholdOppnådd ->
                        Begrunnelse.NyVilkårsprøvingNødvendig
    ...
    internal fun avgrensTil(vurderingTilOgMed: LocalDate): Maksdatokontekst {
        /**
         * etter at vi har telt ferdig så skal hver vedtaksperiode få et maksdatoresultat som tar
         * utgangspunkt i siste dag i sin periode. enten så "spoler vi tilbake" saken, eller så vyer
         * vi fremover.
         */
    ...
    private fun vyFremover(vurderingTilOgMed: LocalDate) = copy(vurdertTilOgMed = vurderingTilOgMed)
    ...
    internal fun medOppholdsdag(dato: LocalDate) =
        copy(vurdertTilOgMed = dato, oppholdsdager = oppholdsdager + dato)
    ...
    internal fun beregnMaksdatoOgSubsummer(
        ...
        val førSyttiårsdagen =
            fun(subsumsjonslogg: Subsumsjonslogg, utfallTom: LocalDate) {
        ...
        val harNåddMaks =
            erDagerOver67ÅrForbrukte(alder, regler) || erDagerUnder67ÅrForbrukte(alder, regler)
        ...
        val maksdatoOrdinærRett =
            forrigeVirkedag + gjenståendeDagerUnder67År(alder, regler).ukedager
        val maksdatoBegrensetRett =
            maxOf(forrigeVirkedag, alder.redusertYtelseAlder.sisteVirkedagInklusiv()) +
                gjenståendeDagerOver67År(alder, regler).ukedager

        val hjemmelsbegrunnelse: Maksdatoresultat.Bestemmelse
        val maksdato: LocalDate
        val gjenståendeDager: Int
        // maksdato er den dagen som først inntreffer blant ordinær kvote, 67-års-kvoten og
        // 70-årsdagen,
        ...
        when {
            ...
            maksdatoBegrensetRett <= alder.syttiårsdagen.forrigeVirkedagFør() -> {
                ...
                val avvisteDagerFraOgMedSøtti =
                    avslåtteDager.filter { alder.mistetSykepengerett(it) }
                if (avvisteDagerFraOgMedSøtti.isNotEmpty()) {
                    subsumsjonslogg.logg(
                        `§ 8-3 ledd 1 punktum 2`(
                            ...
                            tidslinjeTom = vedtaksperiode.endInclusive,
                            avvistePerioder =
                                avvisteDagerFraOgMedSøtti.grupperSammenhengendePerioder(),
                        )
                    )
                }
            }
        ...
        val tidligsteDag =
            if (startdatoSykepengerettighet == LocalDate.MIN) startdatoTreårsvindu
            else minOf(startdatoTreårsvindu, startdatoSykepengerettighet)
        return Maksdatoresultat(
            ...
            startdatoSykepengerettighet =
                startdatoSykepengerettighet.takeUnless { it == LocalDate.MIN },
