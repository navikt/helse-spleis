...
data class Arbeidsgiverperioderesultat(
    ...
    internal fun subsummering(
        ...
        arbeidsgiverperiode
            ...
            .map { it.start }
            .also { subsumsjonslogg.logg(`§ 8-19 tredje ledd`(it, sykdomstidslinjesubsumsjon)) }
        ...
        if (fullstendig && arbeidsgiverperiode.last().endInclusive in vedtaksperiode)
            subsumsjonslogg.logg(
                `§ 8-19 første ledd`(
                    arbeidsgiverperiode.last().endInclusive,
                    sykdomstidslinjesubsumsjon,
                )
            )
        ...
        utbetalingsperioder
            .firstOrNull()
            ?.firstOrNull { !it.erHelg() }
            ?.takeIf { it in vedtaksperiode }
            ?.also {
                subsumsjonslogg.logg(
                    `§ 8-17 ledd 1 bokstav a`(
                        oppfylt = true,
                        dagen = listOf(it.rangeTo(it)),
                        sykdomstidslinjesubsumsjon,
                    )
                )
            }
    ...
    internal fun somArbeidsgiverperiode(): Arbeidsgiverperiode {
        val agp =
            ...
            else Arbeidsgiverperiode(arbeidsgiverperiode)
