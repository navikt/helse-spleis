...
internal class Arbeidsgiverperiode
private constructor(private val perioder: List<Periode>, førsteUtbetalingsdag: LocalDate?) :
    ...
    private val arbeidsgiverperioden
        get() = perioder.first().start til perioder.last().endInclusive
    ...
    private val innflytelseperioden
        get() = førsteKjente til sisteKjente
    ...
    override fun compareTo(other: LocalDate) = førsteKjente.compareTo(other)

    operator fun contains(dato: LocalDate) = dato in innflytelseperioden

    operator fun contains(periode: Periode) = innflytelseperioden.overlapperMed(periode)
    ...
    internal fun forventerOpplysninger(periode: Periode): Boolean {
        if (dekkesAvArbeidsgiver(periode))
            return false // trenger ikke opplysninger om perioden er innenfor agp
    ...
    private fun utbetalingsperiodeForPeriode(periode: Periode) =
        utbetalingsdager.firstOrNull { utbetalingsperiode -> utbetalingsperiode.start in periode }

    private fun forrigeUtbetalingsperiode(periode: Periode) =
        utbetalingsdager.lastOrNull { utbetalingsperiode ->
            utbetalingsperiode.endInclusive < periode.start
        }

    // krever at det foreligger opphold mellom utbetalingsperiodene for at vi skal forvente nye
    // opplysninger
    private fun erOppholdMellom(a: Periode, b: Periode): Boolean {
        val mellomliggendePeriode =
            checkNotNull(a.periodeMellom(b.start)) {
        ...
        return oppholdsdager.any { oppholdsperiode ->
            oppholdsperiode.overlapperMed(mellomliggendePeriode)
        }
    }
    ...
    internal fun dekkesAvArbeidsgiver(periode: Periode): Boolean {
        ...
        return (periode.overlapperMed(arbeidsgiversAnsvar) &&
            arbeidsgiversAnsvar.slutterEtter(periode.endInclusive))
    }
    ...
    internal fun sammenlign(other: List<Periode>): Boolean {
        ...
        return otherSiste == thisSiste ||
            (thisSiste.erHelg() && otherSiste.erRettFør(thisSiste)) ||
            (otherSiste.erHelg() && thisSiste.erRettFør(otherSiste))
    }
    ...
    internal fun erFørsteUtbetalingsdagFørEllerLik(periode: Periode): Boolean {
        ...
        val forrigeUtbetaling =
            utbetalingsdager.lastOrNull { other -> other.endInclusive < periode.endInclusive }
                ?: return false
        return oppholdsdager.none {
            it.overlapperMed(forrigeUtbetaling.endInclusive til periode.endInclusive)
        }
    }
    ...
    internal fun validerFeilaktigNyArbeidsgiverperiode(
        ...
        // Om det er én eller fler ukedager mellom beregnet AGP og vedtaksperioden som overlapper
        // med dager fra inntektsmeldingen
        // tyder det på at arbeidsgiver tror det er ny arbeidsgiverperiode, men vi har beregnet at
        // det _ikke_ er ny arbeidsgiverperiode.
    ...
    private fun utbetalingsdagerI(periode: Periode) =
        periode.filter { dag ->
            utbetalingsdager.any { utbetalingsperiode -> dag in utbetalingsperiode }
        }
    ...
    internal companion object {
        ...
        private fun Periode.justerForHelg() =
            when (endInclusive.dayOfWeek) {
        ...
        internal fun harNødvendigeRefusjonsopplysninger(
            ...
            harNødvendigeRefusjonsopplysninger(
                ...
                arbeidsgiverperiode.oppholdsdager.flatten().lastOrNull { oppholdsdag ->
                    oppholdsdag < periode.start
        ...
        internal fun harNødvendigeRefusjonsopplysningerEtterInntektsmelding(
            ...
            harNødvendigeRefusjonsopplysninger(
                ...
                null
            } // Ved revurderinger hensyntar vi ikke oppholdsdager før utbetalignsdager
