...
internal class VilkårsprøvdSkjæringstidspunkt(
    ...
    internal fun forArbeidsgiver(organisasjonsnummer: String): ArbeidsgiverFaktaavklartInntekt? = inntekter[organisasjonsnummer]

    internal fun medGhostOgNyeInntekterUnderveis(
        utbetalingstidslinjer: Map<String, List<Utbetalingstidslinje>>,
    ): Map<String, Utbetalingstidslinje> = nyeInntekterUnderveis(ghosttidslinjer(utbetalingstidslinjer))
...
internal class ArbeidsgiverFaktaavklartInntekt(
    ...
    ): Økonomi = medInntekt(dato, økonomi, regler, lagDefaultRefusjonsbeløpHvisMangler, refusjonstidslinje)
    ...
    internal fun medInntektEllersVarsel(
        ...
        regler: ArbeidsgiverRegler,
        refusjonstidslinje: Beløpstidslinje,
    ): Økonomi = medInntekt(dato, økonomi, regler, krevRefusjonsbeløpHvisMangler, refusjonstidslinje)
    ...
    internal fun ghosttidslinje(
        ...
        val aktueltGhostområde =
            if (gjelder.start <=
                beregningsperiode.endInclusive
            ) {
                ...
                Utbetalingstidslinje
                    ...
                    .apply {
                        periode.forEach { dag ->
                            ...
                            if (dag.erHelg()) {
                                addFridag(dag, Økonomi.ikkeBetalt())
                            } else {
                                addArbeidsdag(
                                    dag,
                                    Økonomi.ikkeBetalt().inntekt(
...
internal class UtbetalingstidslinjeBuilderVedtaksperiode(
        ...
        sykdomstidslinje.forEach { dag ->
            when (dag) {
                ...
                is Dag.ArbeidsgiverHelgedag -> {
                    if (erAGP(dag.dato)) {
                        arbeidsgiverperiodedag(builder, dag.dato, dag.økonomi)
                    } else {
                        helg(builder, dag.dato, dag.økonomi)
                    }
                }
                is Dag.Arbeidsgiverdag -> {
                    if (erAGP(dag.dato)) {
                        ...
                        avvistDag(builder, dag.dato, dag.økonomi.ikkeBetalt(), Begrunnelse.EgenmeldingUtenforArbeidsgiverperiode)
                    }
                }
                is Dag.Sykedag -> {
                    if (erAGP(dag.dato)) {
                        ...
                        navDag(builder, dag.dato, dag.økonomi)
                    }
                }
                is Dag.SykHelgedag -> {
                    if (erAGP(dag.dato)) {
                ...
                is Dag.SykedagNav -> {
                    if (erAGP(dag.dato)) {
                        builder.addArbeidsgiverperiodedagNav(
                            dag.dato,
                            faktaavklarteInntekter.medInntektEllersVarsel(dag.dato, dag.økonomi, regler, refusjonstidslinje),
                        )
                    } else {
                ...
                is Dag.ForeldetSykedag -> {
                    if (erAGP(dag.dato)) {
                        ...
                        builder.addForeldetDag(
                            dag.dato,
                            faktaavklarteInntekter.medInntektHvisFinnes(dag.dato, dag.økonomi, regler, refusjonstidslinje),
                        )
                    }
                }
    ...
    private fun arbeidsgiverperiodedag(
        ...
        builder.addArbeidsgiverperiodedag(
            dato,
            faktaavklarteInntekter.medInntektHvisFinnes(dato, økonomi.ikkeBetalt(), regler, refusjonstidslinje),
        )
    }
