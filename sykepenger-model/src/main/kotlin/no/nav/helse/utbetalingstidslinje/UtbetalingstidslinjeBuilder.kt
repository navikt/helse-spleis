...
internal class VilkårsprøvdSkjæringstidspunkt(
    ...
    internal fun medGhostOgNyeInntekterUnderveis(utbetalingstidslinjer: Map<String, List<Utbetalingstidslinje>>): Map<String, Utbetalingstidslinje> = nyeInntekterUnderveis(ghosttidslinjer(utbetalingstidslinjer))
    ...
    internal fun result(sykdomstidslinje: Sykdomstidslinje): Utbetalingstidslinje {
        ...
        sykdomstidslinje.forEach { dag ->
            when (dag) {
                ...
                is Dag.SykedagNav -> {
                    if (erAGP(dag.dato)) {
                        builder.addArbeidsgiverperiodedagNav(dag.dato, faktaavklarteInntekter.medInntektEllersVarsel(dag.dato, dag.økonomi, regler, refusjonstidslinje))
                    } else {
                ...
                is Dag.ForeldetSykedag -> {
                    if (erAGP(dag.dato)) {
                        ...
                        builder.addForeldetDag(dag.dato, faktaavklarteInntekter.medInntektHvisFinnes(dag.dato, dag.økonomi, regler, refusjonstidslinje))
                    }
                }
